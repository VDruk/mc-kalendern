/*
 * MC Kalendern - Event Data
 * Copyright (c) 2026 Slava Druk, Uppsala, Sweden
 * https://druk.se/
 *
 * This data is collected, normalized and maintained by MC Kalendern.
 * All rights reserved. Commercial use, redistribution or republication
 * of this data requires written permission from the copyright holder.
 * Contact: slava.druk@gmail.com
 *
 * Data contains integrity markers for copy detection.
 */
const EVENTS_DATA = {
  "lastUpdated": "2026-03-16T17:27:11Z",
  "events": [
    {
      "id": "svedea-mc-forsakring-ad-2026",
      "name": "Svedea MC-försäkring",
      "date": "2026-01-01",
      "dateEnd": "2026-12-31",
      "location": "svedea.se/mc-forsakring",
      "type": "AD",
      "organizer": "Svedea",
      "description": "Sveriges mest sålda mc-försäkring. Över 130 000 nöjda hojägare. Marknadens längsta maskinförsäkring och 10% SMC-rabatt.",
      "descriptionFull": "Svedea är Sveriges största mc-försäkring. Idag väljer 40% av landets motorcykelägare att försäkra sin hoj hos Svedea.\n\nÖver 130 000 nöjda kunder och betyg 4.9 av 5 baserat på 1 782 omdömen.\n\nMarknadens längsta maskinförsäkring: skador på motor och kraftöverföring i hela 12 år eller 8 000 mil.\n\n10% rabatt för dig som är medlem i SMC (Sveriges Motorcyklister).\n\n30% rabatt på Detecht Premium: krachdetektering, rullgrusvarningar och SOS-funktion direkt i mobilen.\n\nSe ditt pris på svedea.se/mc-försäkring",
      "backImage": "ads/svedea-mc-forsakring-back-2026.jpg",
      "frontImage": "ads/svedea-mc-forsakring-front-2026.jpg",
      "links": [
        {
          "label": "svedea.se",
          "url": "https://www.svedea.se/mc-forsakring"
        }
      ],
      "link": "https://www.svedea.se/mc-forsakring",
      "region": "Sverige",
      "source": "AD",
      "_ad": true
    },
    {
      "id": "if-mc-forsakring-ad-2026",
      "name": "If MC-försäkring",
      "date": "2026-01-01",
      "dateEnd": "2026-12-31",
      "location": "if.se/mc-forsakring",
      "type": "AD",
      "organizer": "If Skadeförsäkring",
      "description": "MC-försäkring från If med fria mil, vägassistans dygnet runt och 10% SMC-rabatt. Månadsbetalning som hjälper mycket.",
      "descriptionFull": "If är ett av Nordens största försäkringsbolag och erbjuder mc-försäkring i tre nivåer: Trafik, Halv och Hel.\n\nFria mil: kör så mycket du vill utan att priset påverkas.\n\nVägassistans dygnet runt: om något akut händer kan du ringa If, även mitt i natten.\n\nLägre självrisk med spårsändare: upp till 6 000 kr lägre självrisk med godkänd spårsändare.\n\nIngen garagkrav: hojen behöver inte stå inlåst i garage för att du ska få ersättning vid stöld.\n\n10% rabatt för SMC-medlemmar.\n\nMånadsbetalning tillgänglig så du slipper betala allt på en gång.\n\nBeräkna ditt pris på if.se/mc-försäkring",
      "backImage": "ads/if-mc-forsakring-back-2026.jpg",
      "frontImage": "ads/if-mc-forsakring-front-2026.jpg",
      "links": [
        {
          "label": "if.se",
          "url": "https://www.if.se/privat/forsakringar/fordonsforsakringar/mc-forsakring"
        }
      ],
      "link": "https://www.if.se/privat/forsakringar/fordonsforsakringar/mc-forsakring",
      "region": "Sverige",
      "source": "AD",
      "_ad": true
    },
    {
      "id": "sponsor-harley-davidson-ad-2026",
      "name": "Harley-Davidson 2026",
      "date": "2026-01-01",
      "dateEnd": "2026-12-31",
      "location": "harley-davidson.com",
      "type": "AD",
      "organizer": "Harley-Davidson",
      "description": "Upptäck Harley-Davidson 2026 års utbud. Nya modeller och klassiska favoriter. Besök harley-davidson.com för mer info.",
      "descriptionFull": "Harley-Davidson 2026 års modeller. Upptäck hela utbudet av nya motorcyklar, från klassiska cruisers till moderna touring-hojar. Besök din lokala återförsäljare eller harley-davidson.com.",
      "frontImage": "ads/hd-2026-utbudet-2026-04.jpg",
      "backImage": "ads/hd-2026-utbudet-2026-04.jpg",
      "links": [
        {
          "label": "harley-davidson.com",
          "url": "https://www.harley-davidson.com/"
        }
      ],
      "link": "https://www.harley-davidson.com/",
      "region": "Sverige",
      "source": "AD",
      "_ad": true
    },
    {
      "id": "sponsor-bsa-motorcycles-ad-2026",
      "name": "BSA Motorcycles 2026",
      "date": "2026-01-01",
      "dateEnd": "2026-12-31",
      "location": "bsacompany.co.uk",
      "type": "AD",
      "organizer": "BSA Motorcycles",
      "description": "BSA är tillbaka i Sverige med tre modeller 2026: Gold Star 650, Scrambler 650 och nya Bantam 350. Klassiskt brittiskt arv i modern form.",
      "descriptionFull": "BSA Motorcycles 2026 - tre modeller i Sverige:\n\nGold Star 650 - 652cc DOHC-motor, 45 hk. Klassisk stil med modern teknik. Fem växlar, över 70 mpg.\n\nScrambler 650 - Samma kraftfulla 652cc-motor i en äventyrlig förpackning. Finns i gult, grått och svart.\n\nBantam 350 - Helt ny! Vätskekyld 344cc encylindrig motor, 29 hk. Lätt och smidig för stadskörning. Tre färger: Black UJ, Grey & White, Red & Grey.\n\nBSA grundades 1861 i Birmingham och är ett av motorcykelhistoriens mest ikoniska märken. Nu tillbaka med modernt utbud för europeiska förare.",
      "frontImage": "ads/bsa-scrambler-2026-08-06.jpg",
      "backImage": "ads/bsa-scrambler-2026-08-06.jpg",
      "links": [
        {
          "label": "bsacompany.co.uk",
          "url": "https://www.bsacompany.co.uk/"
        }
      ],
      "link": "https://www.bsacompany.co.uk/",
      "region": "Sverige",
      "source": "AD",
      "_ad": true
    },
    {
      "id": "sponsor-indian-motorcycle-ad-2026",
      "name": "Indian Motorcycle",
      "date": "2026-01-01",
      "dateEnd": "2026-12-31",
      "location": "indianmotorcycle.com",
      "type": "AD",
      "organizer": "Indian Motorcycle",
      "description": "Indian Motorcycle. Amerikanskt arv, modern prestanda. Upptäck hela sortimentet på indianmotorcycle.com.",
      "descriptionFull": "Indian Motorcycle. Amerikanskt arv sedan 1901, modern prestanda och tidlös design. Upptäck hela sortimentet av cruisers, baggers och touring-hojar på indianmotorcycle.com.",
      "frontImage": "ads/indian-motorcycle-2026.jpg",
      "backImage": "ads/indian-motorcycle-2026.jpg",
      "links": [
        {
          "label": "indianmotorcycle.com",
          "url": "https://www.indianmotorcycle.com/sv-se/"
        }
      ],
      "link": "https://www.indianmotorcycle.com/sv-se/",
      "region": "Sverige",
      "source": "AD",
      "_ad": true
    },
    {
      "id": "sponsor-dwr-uppsala-ad-2026",
      "name": "DWR V-Twin Motorcycle Workshop",
      "date": "2026-01-01",
      "dateEnd": "2026-12-31",
      "location": "Uppsala",
      "type": "AD",
      "organizer": "DWR Uppsala",
      "description": "DWR V-Twin Motorcycle Workshop i Uppsala. Service, reparation och customizing av V-Twin motorcyklar.",
      "descriptionFull": "DWR V-Twin Motorcycle Workshop i Uppsala. Specialiserade på service, reparation och customizing av V-Twin motorcyklar. Kontakta oss för mer info.",
      "frontImage": "ads/dwr-uppsala-2026.jpg",
      "backImage": "ads/dwr-uppsala-2026.jpg",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/dwruppsala"
        }
      ],
      "link": "https://www.facebook.com/dwruppsala",
      "region": "Sverige",
      "source": "AD",
      "_ad": true
    },
    {
      "id": "mc-kalendern-druk-se-ad-2026",
      "name": "MC Kalendern",
      "date": "2026-09-28",
      "dateEnd": "2026-09-28",
      "location": "druk.se",
      "type": "AD",
      "organizer": "MC Kalendern / Slava Druk",
      "description": "Sveriges mest kompletta MC-kalender. 770+ event, 8 scrapers, en passion. Gratis, utan reklam, byggd av en hojare for alla hojare.",
      "descriptionFull": "MC Kalendern samlar hela Sveriges MC-event pa ett stalle.\n\nVi har 770+ event fran traeffar och koerningar till shower, racing och fester. All data haemtas automatiskt fran 8 olika kaellor och uppdateras varje dag.\n\nProjektet aer 100% gratis och byggt av ren passion for tvahjuliga aeventyr. Ingen inloggning, inga betalmurer, inga spammail.\n\nSaknar du ett event? Hojta till sa laegger vi till det!\n\nBesok oss pa druk.se och hitta din naesta tur.",
      "frontImage": "ads/mc-kalendern-front-2026-09-28.jpg",
      "backImage": "ads/mc-kalendern-back-2026-09-28.jpg",
      "links": [
        {
          "label": "druk.se",
          "url": "https://druk.se"
        }
      ],
      "link": "https://druk.se",
      "region": "Sverige",
      "source": "AD",
      "_ad": true
    },
    {
      "id": "jonkoping-mcmassan-2026",
      "name": "MC-Mässan Jönköping (Elmia)",
      "date": "2026-01-23",
      "dateEnd": "2026-01-25",
      "location": "Elmiamässan, Jönköping",
      "type": "Show",
      "organizer": "MC-Mässan / Elmia",
      "description": "Sveriges största MC-mässa flyttar till Elmia. Nyheter, provkörningar, tillbehör och Customhoj Bike Show.",
      "link": "https://www.elmia.se/mcmassan/",
      "region": "Jönköping",
      "source": "elmia.se",
      "descriptionFull": "MC-Mässan Jönköping (Elmia) arrangeras av MC-Mässan / Elmia.\n\nSveriges största MC-mässa flyttar till Elmia. Nyheter, provkörningar, tillbehör och Customhoj Bike Show.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Elmiamässan, Jönköping.",
      "links": [
        {
          "label": "elmia.se",
          "url": "https://www.elmia.se/mcmassan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Elmiam%C3%A4ssan%2C+J%C3%B6nk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "halmstad-mcmassa-2026",
      "name": "MC-Mässan Halmstad",
      "date": "2026-03-07",
      "dateEnd": "2026-03-08",
      "location": "Halmstad Arena, Halmstad",
      "type": "Show",
      "organizer": "BVN Event / MC-Veteranerna",
      "description": "MC-arrangemang i Halmstad Arena, Halmstad. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bvnevent.se/",
      "region": "Halland",
      "source": "bvnevent.se",
      "descriptionFull": "MC-Mässan Halmstad arrangeras av BVN Event / MC-Veteranerna.\n\nMC-arrangemang i Halmstad Arena, Halmstad. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Halmstad Arena, Halmstad.",
      "links": [
        {
          "label": "bvnevent.se",
          "url": "https://www.bvnevent.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halmstad+Arena%2C+Halmstad",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-norrbotten-årsmöte-2026",
      "name": "SMC Norrbottens Årsmöte 2026",
      "date": "2026-03-07",
      "location": "Biografen Royal, Luleå",
      "region": "Norrbotten",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Norrbottens årsmöte med gast Kristina 'Krikko' Lundstrom från Boden, kand aventyrsmotorcyklist med stor erfarenhet av resor runt om i världen.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Norrbottens Årsmöte 2026 arrangeras av SMC.\n\nSMC Norrbottens årsmöte med gast Kristina 'Krikko' Lundstrom från Boden, kand aventyrsmotorcyklist med stor erfarenhet av resor runt om i världen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biografen Royal, Luleå.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biografen+Royal%2C+Lule%C3%A5",
          "type": "map"
        }
      ]
    },
    {
      "id": "moon-bike-show-avesta-2026",
      "name": "Moon Bike Show Avesta",
      "date": "2026-03-07",
      "dateEnd": "2026-03-07",
      "location": "Verket, Kanalvägen 1, Avesta",
      "type": "Show",
      "organizer": "Mooneyes Sweden",
      "description": "Choppers, bobbers, caféracers och klassiska motorcyklar i den gamla bruksbyggnaden Verket i Avesta. Nordens mest atmosfäriska MC-show.",
      "descriptionFull": "Moon Bike Show i Avesta den 7 mars 2026.\n\nÅrlig MC-show med choppers, bobbers, caféracers och klassiska motorcyklar. Visas i den historiska bruksbyggnaden Verket i Avesta, vilket ger en unik atmosfär.\n\nÖppet kl 10:00-18:00. Entré 180 kr, barn under 12 gratis.\n\nArrangeras av Mooneyes Sweden.\n\nPlats: Verket, Kanalvägen 1, Avesta.",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/p/Mooneyes-Sweden-100057805594083/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Verket+Kanalv%C3%A4gen+1+Avesta",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/p/Mooneyes-Sweden-100057805594083/",
      "region": "Dalarna",
      "source": "glinder.se"
    },
    {
      "id": "smc-skane-årsmöte-2026",
      "name": "SMC Skånes Årsmöte 2026",
      "date": "2026-03-08",
      "location": "Skåne",
      "region": "Skåne",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Skanes årsmöte. Kl 13. Anmalan sker via Mitt Konto (konto.svmc.se). Plats och ytterligare information meddelas via hemsida, SMC Boken och Facebook.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Skånes Årsmöte 2026 arrangeras av SMC.\n\nSMC Skanes årsmöte. Kl 13. Anmalan sker via Mitt Konto (konto.svmc.se). Plats och ytterligare information meddelas via hemsida, SMC Boken och Facebook.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-uppsala-årsmöte-2026",
      "name": "SMC Uppsala Årsmöte 2026",
      "date": "2026-03-08",
      "location": "Östra Aros MC klubblokal, Stångby 1, Uppsala",
      "region": "Uppsala",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Uppsala årsmöte, preliminärt kl 15. Kontakta styrelsen på info@smcuppsala.se för mer info eller om du vill hjälpa till i verksamheten.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Uppsala Årsmöte 2026 arrangeras av SMC.\n\nSMC Uppsala årsmöte, preliminärt kl 15. Kontakta styrelsen på info@smcuppsala.se för mer info eller om du vill hjälpa till i verksamheten.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Östra Aros MC klubblokal, Stångby 1, Uppsala.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96stra+Aros+MC+klubblokal%2C+St%C3%A5ngby+1%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-orebro-årsmöte-2026",
      "name": "SMC Örebros Årsmöte 2026",
      "date": "2026-03-10",
      "location": "Idrottens Hus, Karlsgatan 28, Örebro",
      "region": "Örebro",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Örebros årsmöte. Tid: 18.30. Plats: Idrottens Hus, Karlsgatan 28 Örebro. Hall da koll på sociala medier, hemsidan och appen för senaste information.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Örebros Årsmöte 2026 arrangeras av SMC.\n\nSMC Örebros årsmöte. Tid: 18.30. Plats: Idrottens Hus, Karlsgatan 28 Örebro. Hall da koll på sociala medier, hemsidan och appen för senaste information.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Idrottens Hus, Karlsgatan 28, Örebro.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Idrottens+Hus%2C+Karlsgatan+28%2C+%C3%96rebro",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-03-11",
      "dateEnd": "2026-03-11",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-03-11/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-03-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-roliga-timmen-med-ronnie-2026",
      "name": "DOA - Roliga timmen med Ronnie",
      "date": "2026-03-12",
      "dateEnd": "2026-03-12",
      "location": "Ronnies garage, Skärholmen",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "12 Mars 18.15 i garaget Vi pratar och delar tankar om däck till Harley, max 5 personer. Biljett, ett paket bryggkaffe eller 100kr swish!",
      "link": "https://hdcs.se/evenemang/doa-roliga-timmen-med-ronnie/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Roliga timmen med Ronnie arrangeras av H-DCS.\n\n12 Mars 18.15 i garaget Vi pratar och delar tankar om däck till Harley, max 5 personer. Biljett, ett paket bryggkaffe eller 100kr swish!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Ronnies garage, Skärholmen.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-roliga-timmen-med-ronnie/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ronnies+garage%2C+Sk%C3%A4rholmen",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-vill-du-bli-turledare-i-d11-introduktion-2026",
      "name": "Vill du bli turledare i D11? Introduktion",
      "date": "2026-03-12",
      "dateEnd": "2026-03-12",
      "location": "Online / Digitalt",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D11 från Online / Digitalt. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/vill-du-bli-turledare-i-d11-introduktion/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "Vill du bli turledare i D11? Introduktion arrangeras av BMW MC-klubben.\n\nKörning med D11 från Online / Digitalt. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Online / Digitalt.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/vill-du-bli-turledare-i-d11-introduktion/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Online+%2F+Digitalt",
          "type": "map"
        }
      ]
    },
    {
      "id": "uppsala-mcmassa-2026",
      "name": "Uppsala MC-Mässa & Motorshow",
      "date": "2026-03-13",
      "dateEnd": "2026-03-15",
      "location": "Fyrishov, Uppsala",
      "type": "Show",
      "organizer": "BVN Event",
      "description": "Mässan fylls med motorcyklar, byggare, handlare, klubbar och mc-entusiaster. Stuntshow med Sveriges bästa stuntförare.",
      "link": "https://www.bvnevent.se/Uppsala",
      "region": "Uppsala",
      "source": "bvnevent.se",
      "descriptionFull": "Uppsala MC-Mässa & Motorshow arrangeras av BVN Event.\n\nMässan fylls med motorcyklar, byggare, handlare, klubbar och mc-entusiaster. Stuntshow med Sveriges bästa stuntförare.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Fyrishov, Uppsala.",
      "links": [
        {
          "label": "bvnevent.se",
          "url": "https://www.bvnevent.se/Uppsala"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Fyrishov%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-pubkvall-varberg-2026",
      "name": "DO-C pubkväll varberg",
      "date": "2026-03-13",
      "dateEnd": "2026-03-13",
      "location": "Varbergs ölhall, Varberg",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Nu är det klart med Pubkväll i Varberg.",
      "link": "https://hdcs.se/evenemang/do-c-pubkvall-varberg-3/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C pubkväll varberg arrangeras av H-DCS.\n\nNu är det klart med Pubkväll i Varberg.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Varbergs ölhall, Varberg.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-pubkvall-varberg-3/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Varbergs+%C3%B6lhall%2C+Varberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-lordagsfika-pa-biltema-2026",
      "name": "DOA - Lördagsfika på Biltema",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Biltema, Haninge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Varje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.",
      "link": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-03-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Lördagsfika på Biltema arrangeras av H-DCS.\n\nVarje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Haninge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-03-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+Haninge",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikafraff-i-hedemora-2026",
      "name": "DOG - Fikafräff i Hedemora",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Wahlmans Cafe, Hedemora",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Fikafräff på Wahlmans cafe i Hedemora Lördag 10 januari kl. 12.",
      "link": "https://hdcs.se/evenemang/dog-fikafraff-i-falun-2/2026-03-14/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG - Fikafräff i Hedemora arrangeras av H-DCS.\n\nFikafräff på Wahlmans cafe i Hedemora Lördag 10 januari kl. 12.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Wahlmans Cafe, Hedemora.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikafraff-i-falun-2/2026-03-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Wahlmans+Cafe%2C+Hedemora",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-årsmöte-2026",
      "name": "DOA - Årsmöte",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Pub Anchor, Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Alla får gärna ta med förslag på utflyktsmål till sommarens turer. Antingen till kvällsturer, dagsturer eller helgturer.",
      "link": "https://hdcs.se/evenemang/doa-årsmöte/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Årsmöte arrangeras av H-DCS.\n\nAlla får gärna ta med förslag på utflyktsmål till sommarens turer. Antingen till kvällsturer, dagsturer eller helgturer.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Pub Anchor, Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-årsmöte/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Pub+Anchor%2C+Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-pubkvall-stockholm-2026",
      "name": "DOA - Pubkväll Stockholm",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Pub Anchor, Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll med H-DCS DOA i Pub Anchor, Stockholm. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!",
      "link": "https://hdcs.se/evenemang/doa-pubkvall-stockholm/2026-03-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Pubkväll Stockholm arrangeras av H-DCS.\n\nPubkväll med H-DCS DOA i Pub Anchor, Stockholm. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Pub Anchor, Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-pubkvall-stockholm/2026-03-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Pub+Anchor%2C+Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "lob-pubkvall-i-kristianstad-installt-2026",
      "name": "LOB- Pubkväll i Kristianstad INSTÄLLT",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Kristianstad",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ställer in detta evenemang endast 3 som anmält sig Pubkväll i Kristianstad den 14 mars. Vi träffas på O´Learys kl. 17.00.",
      "link": "https://hdcs.se/evenemang/lob-pubkvall-i-kristianstad/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "LOB- Pubkväll i Kristianstad INSTÄLLT arrangeras av H-DCS.\n\nVi ställer in detta evenemang endast 3 som anmält sig Pubkväll i Kristianstad den 14 mars. Vi träffas på O´Learys kl. 17.00.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kristianstad.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lob-pubkvall-i-kristianstad/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kristianstad",
          "type": "map"
        }
      ]
    },
    {
      "id": "bagskytte-2026",
      "name": "Bågskytte",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Bågskytte, plats oklart än, mer info kommer. Efter går dom som vill och tar något att äta och kanske en öl.",
      "link": "https://hdcs.se/evenemang/bagskytte/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "Bågskytte arrangeras av H-DCS.\n\nBågskytte, plats oklart än, mer info kommer. Efter går dom som vill och tar något att äta och kanske en öl.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/bagskytte/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-gps-och-navigationsdag-2026",
      "name": "D7 GPS- och Navigationsdag",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Landsvägen 49",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "GPS- och navigationskurs med D7. Lär dig navigera mellan waypoints. Anmälan på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-gps-och-navigationsdag/",
      "region": "Västmanland",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 GPS- och Navigationsdag arrangeras av BMW MC-klubben.\n\nGPS- och navigationskurs med D7. Lär dig navigera mellan waypoints. Anmälan på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Landsvägen 49.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-gps-och-navigationsdag/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Landsv%C3%A4gen+49",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-gps-och-navigationsdag-stegra-passet-over-teams-2026",
      "name": "GPS- och Navigationsdag - Stegra-passet över Teams",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Online / Digitalt",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Vi öppnar upp ett av passen under GPS- och Navigationsdagen för alla klubbmedlemmar - oavsett var i landet du befinner dig!",
      "link": "https://www.bmwklubben.se/aktiviteter/gps-och-navigationsdag-stegra-passet-over-teams/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "GPS- och Navigationsdag - Stegra-passet över Teams arrangeras av BMW MC-klubben.\n\nVi öppnar upp ett av passen under GPS- och Navigationsdagen för alla klubbmedlemmar - oavsett var i landet du befinner dig!\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Online / Digitalt.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/gps-och-navigationsdag-stegra-passet-over-teams/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Online+%2F+Digitalt",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-alands-x-2026",
      "name": "Alands-X med Eckerolinjen",
      "date": "2026-03-14",
      "dateEnd": "2026-03-14",
      "location": "Grisslehamn",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Grisslehamn. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/alands-x-med-eckerolinjen-2/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "Alands-X med Eckerolinjen arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Grisslehamn. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Grisslehamn.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/alands-x-med-eckerolinjen-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Grisslehamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-sondagsvandring-stockamollan-2026",
      "name": "D15 Söndagsvandring Stockamöllan",
      "date": "2026-03-15",
      "dateEnd": "2026-03-15",
      "location": "Stenbocksliden 2, Stockamöllan",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Stenbocksliden 2, 241 96 Stockamöllan, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-sondagsvandring/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Söndagsvandring Stockamöllan arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Stenbocksliden 2, 241 96 Stockamöllan, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stenbocksliden 2, Stockamöllan.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-sondagsvandring/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stenbocksliden+2%2C+Stockam%C3%B6llan",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-fikatraff-sormland-mar-2026",
      "name": "Sormlandswingarnas fikatraff",
      "date": "2026-03-15",
      "dateEnd": "2026-03-15",
      "location": "Sigridslunds Cafe, Ardala",
      "type": "Träff",
      "organizer": "Sormlandswingarna / GWCS",
      "description": "Träffpunkt i Sigridslunds Cafe, Ardala. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://gwcs.se/events/sormlandswingarnas-fikatraff-sigridslunds-cafe-2/?occurrence=2026-03-15",
      "region": "Södermanland",
      "source": "gwcs.se",
      "descriptionFull": "Sormlandswingarnas fikatraff arrangeras av Sormlandswingarna / GWCS.\n\nTräffpunkt i Sigridslunds Cafe, Ardala. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sigridslunds Cafe, Ardala.",
      "links": [
        {
          "label": "gwcs.se",
          "url": "https://gwcs.se/events/sormlandswingarnas-fikatraff-sigridslunds-cafe-2/?occurrence=2026-03-15"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sigridslunds+Cafe%2C+Ardala",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-kronoberg-årsmöte-2026",
      "name": "SMC Kronobergs Årsmöte 2026",
      "date": "2026-03-15",
      "location": "Hjorts klubbstuga, Kronoberg",
      "region": "Kronoberg",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Kronobergs årsmöte kl 15 i Hjorts klubbstuga. Motioner maste vara styrelsen tillhanda senast den 14/2 (info@smckronoberg.se).",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Kronobergs Årsmöte 2026 arrangeras av SMC.\n\nSMC Kronobergs årsmöte kl 15 i Hjorts klubbstuga. Motioner maste vara styrelsen tillhanda senast den 14/2 (info@smckronoberg.se).\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Hjorts klubbstuga, Kronoberg.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hjorts+klubbstuga%2C+Kronoberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-03-17",
      "dateEnd": "2026-03-17",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-03-17/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-03-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-03-18",
      "dateEnd": "2026-03-18",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-03-18/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-03-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-aw-med-filmvisning-2026",
      "name": "D7 AW med filmvisning",
      "date": "2026-03-18",
      "dateEnd": "2026-03-18",
      "location": "Landsvägen 49",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "En kväll för dig som älskar friheten på två hjul och berättelser som doftar både bensin och guldrusch.",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-aw-med-filmvisning/",
      "region": "Stockholm",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 AW med filmvisning arrangeras av BMW MC-klubben.\n\nEn kväll för dig som älskar friheten på två hjul och berättelser som doftar både bensin och guldrusch.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Landsvägen 49.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-aw-med-filmvisning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Landsv%C3%A4gen+49",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-03-19",
      "dateEnd": "2026-03-19",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-03-19/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-03-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-sasongsuppstart-motorrad-center-upplands-vasby-2026",
      "name": "D7 Säsongsuppstart, Motorrad Center Upplands Väsby",
      "date": "2026-03-19",
      "dateEnd": "2026-03-19",
      "location": "Slånbärsvägen 6, Upplands Väsby",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D7 från Slånbärsvägen 6, 194 65 Upplands Väsby, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-sasongsuppstart-motorrad-center-upplands-vasby/",
      "region": "Stockholm",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 Säsongsuppstart, Motorrad Center Upplands Väsby arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D7 från Slånbärsvägen 6, 194 65 Upplands Väsby, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Slånbärsvägen 6, Upplands Väsby.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-sasongsuppstart-motorrad-center-upplands-vasby/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sl%C3%A5nb%C3%A4rsv%C3%A4gen+6%2C+Upplands+V%C3%A4sby",
          "type": "map"
        }
      ]
    },
    {
      "id": "harley-davidson-night-pa-ikoniska-babel-2026",
      "name": "Harley-Davidson Night på ikoniska Babel",
      "date": "2026-03-20",
      "dateEnd": "2026-03-20",
      "location": "Babel, Malmö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Evenemang av Probike Malmö Babel Malmö Längd: 3 h Offentlig · Alla på och utanför Facebook One Night Only. One Hell of a Party. Everyone’s Welcome.",
      "link": "https://hdcs.se/evenemang/dob-harley-night-probike/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "Harley-Davidson Night på ikoniska Babel arrangeras av H-DCS.\n\nEvenemang av Probike Malmö Babel Malmö Längd: 3 h Offentlig · Alla på och utanför Facebook One Night Only. One Hell of a Party. Everyone’s Welcome.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Babel, Malmö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-harley-night-probike/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Babel%2C+Malm%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-harley-night-med-probike-pa-anchor-2026",
      "name": "DOA - Harley Night med ProBike på Anchor",
      "date": "2026-03-20",
      "dateEnd": "2026-03-20",
      "location": "Pub Anchor, Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "ProBike bjuder in till Harley-Night på Pub Anchor 20 mars kl.",
      "link": "https://hdcs.se/evenemang/doa-harley-night-med-probike-pa-anchor/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Harley Night med ProBike på Anchor arrangeras av H-DCS.\n\nProBike bjuder in till Harley-Night på Pub Anchor 20 mars kl.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Pub Anchor, Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-harley-night-med-probike-pa-anchor/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Pub+Anchor%2C+Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-pubkvall-boras-2026",
      "name": "DO-C pubkväll borås",
      "date": "2026-03-20",
      "dateEnd": "2026-03-20",
      "location": "Bowlers i Borås, Borås",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Välkomna alla Boråsare med omnejd! Till pubkväll på Bowlers i Borås för att umgås en stund med gött snack, & !",
      "link": "https://hdcs.se/evenemang/do-c-pubkvall-boras-9/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C pubkväll borås arrangeras av H-DCS.\n\nVälkomna alla Boråsare med omnejd! Till pubkväll på Bowlers i Borås för att umgås en stund med gött snack, & !\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Bowlers i Borås, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-pubkvall-boras-9/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bowlers+i+Bor%C3%A5s%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "harley-davidson-night-bikeman-helsingborg-2026",
      "name": "Harley-Davidson Night",
      "date": "2026-03-20",
      "dateEnd": "2026-03-20",
      "location": "Kroppavägen 40, Mörarp",
      "type": "Träff",
      "organizer": "Bikeman Helsingborg",
      "description": "Bikeman Helsingborg välkomnar våren med Harley-Davidson Night! Livemusik, burgare, barberare och tatuering av Tattoo Royal.",
      "descriptionFull": "Bikeman Helsingborg bjuder in till en kväll fylld av härlig stämning, gemenskap och äkta Harley-känsla. Damma av skinnjackan, samla vännerna och kom och kickstarta säsongen!\n\nAnmälan sker via Harley-Davidsons hemsida.\n\nUnder kvällen bjuds det på livemusik och burgare. Barberare på plats. Dessutom får ni besök av Andreas från Tattoo Royal för er som är sugna på en ny tatuering eller vill snacka motiv.\n\nTid: 16:00-20:00\nLängd: ca 4 timmar\n\nPlats: Kroppavägen 40, 253 55 Mörarp.",
      "backImage": "ads/harley-davidson-night-bikeman-back-2026-03-20.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/3426762137474597/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bikeman+AB+M%C3%B6rarp",
          "type": "map"
        }
      ],
      "link": "",
      "region": "Skåne",
      "source": "FB"
    },
    {
      "id": "hd-nights-2026-03-20",
      "name": "Harley-Davidson Nights 20 Mars",
      "date": "2026-03-20",
      "dateEnd": "2026-03-20",
      "location": "Malmö, Stockholm, Borås, Helsingborg",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Harley-Davidson Nights den 20 mars på flera platser i Sverige. Probike, H-DCS och Bikeman arrangerar kvällsevent för alla Harley-entusiaster.",
      "descriptionFull": "Harley-Davidson Nights den 20 mars 2026. H-DCS distrikt över hela Sverige arrangerar kvällsevent för alla MC-entusiaster.\n\nPlatser:\n- DOB Harley Night på Babel, Malmö (Probike Malmö)\n- DOA Harley Night på Pub Anchor, Stockholm (ProBike)\n- DO-C Pubkväll på Bowlers, Borås\n- Harley-Davidson Night hos Bikeman, Mörarp/Helsingborg\n\nAlla är välkomna oavsett vilken hoj du kör. One Night Only. One Hell of a Party.",
      "frontImage": "ads/hd-nights-2026-03-20.jpg",
      "backImage": "ads/hd-nights-2026-03-20.jpg",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/"
        }
      ],
      "link": "https://hdcs.se/",
      "region": "Sverige",
      "source": "hdcs.se"
    },
    {
      "id": "after-st-patrick-day-sundsvall-2026",
      "name": "After St.Patrick Day",
      "date": "2026-03-20",
      "dateEnd": "2026-03-20",
      "location": "Adas MC Sundsvall, Regementsvägen 12, Sundsvall",
      "type": "Fest",
      "organizer": "Adas MC Sundsvall",
      "description": "Kom och umgas på en AW med St Patrick's-tema hos Adas MC Sundsvall. Irlandska gosaker och kaffe serveras. Kl 17-24.",
      "descriptionFull": "Adas MC Sundsvall bjuder in till After St.Patrick Day den 20 mars 2026.\n\nKom och umgas på en AW med tema St Patrick's. Det serveras massa irlandska gosaker av olika slag, och det kommer även finnas vanligt kaffe.\n\nKl 17:00 - 23:59.\n\nPlats: Regementsvägen 12, SE-852 38 Sundsvall.",
      "backImage": "ads/after-st-patrick-day-back-2026-03-20.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1212626780853112/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Adas+MC+Sundsvall",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1212626780853112/",
      "region": "Västernorrland",
      "source": "FB"
    },
    {
      "id": "doa-lordagsfika-pa-biltema-2026",
      "name": "DOA - Lördagsfika på Biltema",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "Biltema, Haninge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Varje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.",
      "link": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-03-21/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Lördagsfika på Biltema arrangeras av H-DCS.\n\nVarje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Haninge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-03-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+Haninge",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-03-21/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-03-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-i-grillning-solleftea-2026",
      "name": "DO I Grillning Sollefteå",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "Granvågsniporna, Sollefteå",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Som vanligt så inleds våren med grillning i Granvågsniporna. Kom med glatt humör och kläder efter väder.",
      "link": "https://hdcs.se/evenemang/do-i-grillning-solleftea/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "DO I Grillning Sollefteå arrangeras av H-DCS.\n\nSom vanligt så inleds våren med grillning i Granvågsniporna. Kom med glatt humör och kläder efter väder.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Granvågsniporna, Sollefteå.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-i-grillning-solleftea/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Granv%C3%A5gsniporna%2C+Sollefte%C3%A5",
          "type": "map"
        }
      ]
    },
    {
      "id": "pubkvall-pa-hamlet-i-linkoping-2026",
      "name": "Pubkväll på Hamlet i Linköping",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "Hamlet, Storgatan 37, Linköping",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Nu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.",
      "link": "https://hdcs.se/evenemang/pubkvall-pa-hamlet-i-linkoping-4/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "Pubkväll på Hamlet i Linköping arrangeras av H-DCS.\n\nNu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Hamlet, Storgatan 37, Linköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/pubkvall-pa-hamlet-i-linkoping-4/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hamlet%2C+Storgatan+37%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "lob-pubkvall-malmo-2026",
      "name": "LOB- Pubkväll Malmö",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "Babel, Malmö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll med H-DCS Distrikt B Välkommen till pubkväll med Harley-Davidson Club Sweden - Distrikt B tillsammans med vårt lokala ombud i Malmö.",
      "link": "https://hdcs.se/evenemang/lob-pubkvall-malmo/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "LOB- Pubkväll Malmö arrangeras av H-DCS.\n\nPubkväll med H-DCS Distrikt B Välkommen till pubkväll med Harley-Davidson Club Sweden - Distrikt B tillsammans med vårt lokala ombud i Malmö.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Babel, Malmö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lob-pubkvall-malmo/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Babel%2C+Malm%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-stockholm-årsmöte-2026",
      "name": "SMC Stockholms Årsmöte 2026",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "SMC Stockholm",
      "description": "Årsmöte för SMC i Stockholm. Viktigt möte om kommande säsong. Alla medlemmar uppmanas att närvara!",
      "link": "https://www.svmc.se/stockholm/nyheter/aarsmoete-2026/",
      "region": "Stockholm",
      "source": "svmc.se",
      "descriptionFull": "SMC Stockholms Årsmöte 2026 arrangeras av SMC Stockholm.\n\nÅrsmöte för SMC i Stockholm. Viktigt möte om kommande säsong. Alla medlemmar uppmanas att närvara!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se/stockholm/nyheter/aarsmoete-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-vasternorrland-årsmöte-2026",
      "name": "SMC Västernorrlands Årsmöte 2026",
      "date": "2026-03-21",
      "location": "Västernorrland",
      "region": "Västernorrland",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Vasternorrlands årsmöte med motionsskrivning och planering på agendan. Tid för inskick av motioner publiceras på distriktets hemsida och sociala medier.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Västernorrlands Årsmöte 2026 arrangeras av SMC.\n\nSMC Vasternorrlands årsmöte med motionsskrivning och planering på agendan. Tid för inskick av motioner publiceras på distriktets hemsida och sociala medier.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Västernorrland.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4sternorrland",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-vastmanland-årsmöte-2026",
      "name": "SMC Västmanlands Årsmöte 2026",
      "date": "2026-03-21",
      "location": "Västerås",
      "region": "Västmanland",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Västmanlands årsmöte kl 16 i Västerås. Mer info via www.svmc.se/vastmanland och sociala medier.",
      "link": "https://svmc.se/vastmanland",
      "descriptionFull": "SMC Västmanlands Årsmöte 2026 arrangeras av SMC.\n\nSMC Västmanlands årsmöte kl 16 i Västerås. Mer info via www.svmc.se/vastmanland och sociala medier.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Västerås.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se/vastmanland"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ster%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-vastra-gotaland-årsmöte-2026",
      "name": "SMC Västra Götalands Årsmöte 2026",
      "date": "2026-03-21",
      "location": "Västra Götaland",
      "region": "Västra Götaland",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Vast haller årsmöte med sedvanliga årsmötesforhandlingar. Tid och plats på hemsidan. Motioner senast den 7 februari.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Västra Götalands Årsmöte 2026 arrangeras av SMC.\n\nSMC Vast haller årsmöte med sedvanliga årsmötesforhandlingar. Tid och plats på hemsidan. Motioner senast den 7 februari.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Västra Götaland.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4stra+G%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "mtl-royal-enfield-125-monesteras-2026",
      "name": "Royal Enfield 125 år",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "MTL Powersport, Mönsterås",
      "type": "Träff",
      "organizer": "MTL Powersport",
      "description": "Royal Enfield fyller 125 år! MTL Powersport firar med extra fokus, nyheter och demohojar att provköra. Servicekampanj med gratis första service vid köp.",
      "descriptionFull": "Royal Enfield fyller 125 år och MTL Powersport firar!\n\nKom och kolla in nyheter samt provkör demohojar. MTL Powersport har haft Royal Enfield i sitt sortiment i ett år och responsen har varit fantastisk.\n\nServicekampanj: Gratis första service vid köp av en Royal Enfield denna dag!\n\nTid: Lördag 21 mars, kl. 10:00 - 13:00.\n\nPlats: MTL Powersport, Västra Oknebäcksvägen 1, Mönsterås.",
      "backImage": "ads/royal-enfield-125-back-2026-03-21.jpg",
      "links": [
        { "label": "Facebook", "url": "https://www.facebook.com/mtlpowersport" },
        { "label": "mtlpowersport.se", "url": "https://www.mtlpowersport.se" },
        { "label": "Karta", "url": "https://www.google.com/maps/search/?api=1&query=MTL+Powersport+V%C3%A4stra+Okneb%C3%A4cksv%C3%A4gen+1+M%C3%B6nster%C3%A5s", "type": "map" }
      ],
      "link": "https://www.mtlpowersport.se",
      "region": "Kalmar",
      "source": "facebook.com"
    },
    {
      "id": "smc-gotland-årsmöte-2026",
      "name": "SMC Gotlands Årsmöte 2026",
      "date": "2026-03-22",
      "location": "Uncle Joes, Visby",
      "region": "Gotland",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Gotlands årsmöte kl 14 på Uncle Joes i Visby. Motioner och förslag behöver vara styrelsen tillhanda sex veckor före årsmötet. Efter årsmötet informerar vi om SMCs arbete.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Gotlands Årsmöte 2026 arrangeras av SMC.\n\nSMC Gotlands årsmöte kl 14 på Uncle Joes i Visby. Motioner och förslag behöver vara styrelsen tillhanda sex veckor före årsmötet. Efter årsmötet informerar vi om SMCs arbete.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Uncle Joes, Visby.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Uncle+Joes%2C+Visby",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-jonkoping-årsmöte-2026",
      "name": "SMC Jönköpings Årsmöte 2026",
      "date": "2026-03-22",
      "location": "Jönköping",
      "region": "Jönköping",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Jonkopings årsmöte kl 13. Motet inleds med ett föredrag av Björn Gunnarsson. Därefter årsmöte! SMC bjuder på fika. Plats meddelas på hemsida och Facebook.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Jönköpings Årsmöte 2026 arrangeras av SMC.\n\nSMC Jonkopings årsmöte kl 13. Motet inleds med ett föredrag av Björn Gunnarsson. Därefter årsmöte! SMC bjuder på fika. Plats meddelas på hemsida och Facebook.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Jönköping.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=J%C3%B6nk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-kalmar-årsmöte-2026",
      "name": "SMC Kalmars Årsmöte 2026",
      "date": "2026-03-22",
      "location": "Kalmar",
      "region": "Kalmar",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Kalmar Lans årsmöte kl 12. Plats meddelas senare. Obligatorisk anmalan till info@smc-kalmar.com senast den 8 mars 2026.",
      "link": "https://svmc.se",
      "descriptionFull": "SMC Kalmars Årsmöte 2026 arrangeras av SMC.\n\nSMC Kalmar Lans årsmöte kl 12. Plats meddelas senare. Obligatorisk anmalan till info@smc-kalmar.com senast den 8 mars 2026.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kalmar.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kalmar",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-03-24",
      "dateEnd": "2026-03-24",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-03-24/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-03-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-uppdatering-for-turledare-i-d11-2026",
      "name": "Uppdatering för turledare i D11",
      "date": "2026-03-24",
      "dateEnd": "2026-03-24",
      "location": "Online / Digitalt",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D11 från Online / Digitalt. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/uppdatering-for-turledare-i-d11/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "Uppdatering för turledare i D11 arrangeras av BMW MC-klubben.\n\nKörning med D11 från Online / Digitalt. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Online / Digitalt.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/uppdatering-for-turledare-i-d11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Online+%2F+Digitalt",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-medlems-kvall-probike-malmo-2026",
      "name": "DOB- Medlems kväll Probike Malmö",
      "date": "2026-03-25",
      "dateEnd": "2026-03-25",
      "location": "Probike, Malmö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "DOB- Medlemskväll Probike Malmö Endast för H-DCS medlemmar har ni respektive som inte är medlem så är de välkomna. Tisdagen den 25 mars kl.",
      "link": "https://hdcs.se/evenemang/dob-medlems-kvall-probike-malmo/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Medlems kväll Probike Malmö arrangeras av H-DCS.\n\nDOB- Medlemskväll Probike Malmö Endast för H-DCS medlemmar har ni respektive som inte är medlem så är de välkomna. Tisdagen den 25 mars kl.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Probike, Malmö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-medlems-kvall-probike-malmo/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike%2C+Malm%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-03-25",
      "dateEnd": "2026-03-25",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-03-25/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-03-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d12-planeringsmote-2026-2026",
      "name": "D12 Planeringsmöte 2026",
      "date": "2026-03-25",
      "dateEnd": "2026-03-26",
      "location": "Sjudarevägen 42b, Visby",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Möte för D12. Planering av säsongens turer och klubbaktiviteter. Viktigt att närvara för medlemmar.",
      "link": "https://www.bmwklubben.se/aktiviteter/d12-planeringsmote-2026/",
      "region": "Gotland",
      "source": "bmwklubben.se",
      "descriptionFull": "D12 Planeringsmöte 2026 arrangeras av BMW MC-klubben.\n\nMöte för D12. Planering av säsongens turer och klubbaktiviteter. Viktigt att närvara för medlemmar.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sjudarevägen 42b, Visby.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d12-planeringsmote-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sjudarev%C3%A4gen+42b%2C+Visby",
          "type": "map"
        }
      ]
    },
    {
      "id": "ttc-oppet-hus-posa-falun-2026",
      "name": "Öppet hus hos TTC - med Posa!",
      "date": "2026-03-25",
      "dateEnd": "2026-03-25",
      "location": "Ingarvsvägen 8 B, Falun",
      "type": "Träff",
      "organizer": "The Twin Club Falun",
      "description": "Öppet hus i klubbstugan med gäst Per-Olof \"Posa\" Serenius, en av världens bästa isracingförare. Spännande berättelser om MC-äventyr. Gratis entré, fika ingår.",
      "descriptionFull": "Vi håller öppet hus i klubbstugan!\n\nGäst denna kväll är en riktig legend, Per-Olof \"Posa\" Serenius. Posa, en av världens genom tiderna bästa isracingförare, kommer och berättar om sina äventyr på motorcykel, både på landsväg och inom isracing.\n\nVi kan garantera att han har mycket intressant att berätta! Posa har lovat att underhålla länge denna kväll, så var beredd på flera timmars spännande berättelser!\n\nEvenemanget är gratis och klubben bjuder på fika.\n\nVälkomna!\n\n18:00-21:00",
      "backImage": "ads/ttc-oppet-hus-back-2026-03-25.jpg",
      "links": [
        {
          "label": "thetwinclub.se",
          "url": "https://www.thetwinclub.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ingarvsvägen+8B+Falun",
          "type": "map"
        }
      ],
      "link": "https://www.thetwinclub.se/",
      "region": "Dalarna",
      "source": "FB"
    },
    {
      "id": "smc-grundkurs-underhall-eskilstuna-2026",
      "name": "Grundkurs i MC Underhåll",
      "date": "2026-03-25",
      "dateEnd": "2026-03-25",
      "location": "Drottninggatan 15B, Eskilstuna",
      "type": "Träff",
      "organizer": "SMC Södermanland",
      "description": "Lär dig kontrollera din MC inför säsongen, besiktning eller resa. Motorcyklar på plats att inspektera tillsammans. SMC-medlem 100 kr, ej medlem 150 kr. Kl 18-20.",
      "descriptionFull": "Varm välkommen på Grundkurs i MC Underhåll.\n\nDenna kurs riktar sig till dig som vill lära dig mer om underhåll av din motorcykel, oavsett om du övningskör och vill förbereda dig inför kommande körprov, haft körkort eller precis tagit körkort!\n\nUnder kursen går vi igenom viktig information om hur du gör en bra kontroll av din MC innan du tar ut den för säsongen, åker till besiktningen eller om du ska ut och resa.\n\nDet kommer att finnas några motorcyklar på plats som vi inspekterar tillsammans.\n\nPlats: Isaks Trafikskola, Drottninggatan 15B, Eskilstuna.\n\nInskrivning 17:50\nKursstart 18:00\nKursavslut ca 20:00\n\nAnmälningsavgift:\nSMC-medlem: 100 kr (skriv medlemsnr i meddelande vid bokningen)\nEj medlem: 150 kr\n\nSwish: 0736-34 25 60 (Maria N)\nMeddelande: 25 mars kurs (ev medl nr)\n\nOBS! Begränsat antal platser!",
      "backImage": "ads/smc-grundkurs-underhall-back-2026-03-25.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1189508803258568"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Drottninggatan+15B+Eskilstuna",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1189508803258568",
      "region": "Södermanland",
      "source": "FB"
    },
    {
      "id": "bmw-d10-teater-goteborgskravallerna-2026",
      "name": "D10 Teater - Göteborgskravallerna",
      "date": "2026-03-26",
      "dateEnd": "2026-03-26",
      "location": "Götaplatsen, Göteborg",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D10 från Götaplatsen, 412 56 Göteborg, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-teater-goteborgskravallerna/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Teater - Göteborgskravallerna arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D10 från Götaplatsen, 412 56 Göteborg, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Götaplatsen, Göteborg.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-teater-goteborgskravallerna/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=G%C3%B6taplatsen%2C+G%C3%B6teborg",
          "type": "map"
        }
      ]
    },
    {
      "id": "pub-kvall-pa-murphys-i-jonkoping-2026",
      "name": "PUB-KVÄLL på Murphys i Jönköping",
      "date": "2026-03-27",
      "dateEnd": "2026-03-27",
      "location": "Murphys Jönköping, Jönköping",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "PUB KVÄLL på Murphys fredag 27 mars , Lundströms plats 2 i Jönköping som ligger alldeles vid Resecenrum.",
      "link": "https://hdcs.se/evenemang/av-pa-murphys-i-jonkoping/",
      "region": "Jönköping",
      "source": "hdcs.se",
      "descriptionFull": "PUB-KVÄLL på Murphys i Jönköping arrangeras av H-DCS.\n\nPUB KVÄLL på Murphys fredag 27 mars , Lundströms plats 2 i Jönköping som ligger alldeles vid Resecenrum.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Murphys Jönköping, Jönköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/av-pa-murphys-i-jonkoping/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Murphys+J%C3%B6nk%C3%B6ping%2C+J%C3%B6nk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-pubkvall-i-halmstad-2026",
      "name": "DO-C pubkväll i halmstad",
      "date": "2026-03-27",
      "dateEnd": "2026-03-27",
      "location": "Cyrano, Halmstad",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll med H-DCS i Cyrano halmstad, Halmstad. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!",
      "link": "https://hdcs.se/evenemang/do-c-pubkvall-i-halmstad-5/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C pubkväll i halmstad arrangeras av H-DCS.\n\nPubkväll med H-DCS i Cyrano halmstad, Halmstad. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Cyrano, Halmstad.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-pubkvall-i-halmstad-5/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Cyrano%2C+Halmstad",
          "type": "map"
        }
      ]
    },
    {
      "id": "pubkvall-pa-cromwell-house-i-norrkoping-2026",
      "name": "Pubkväll på Cromwell House i Norrköping",
      "date": "2026-03-27",
      "dateEnd": "2026-03-27",
      "location": "Cromwell House, Kungsgatan 36, Norrköping",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Nu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.",
      "link": "https://hdcs.se/evenemang/pubkvall-pa-cromwell-house-i-norrkoping-3/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "Pubkväll på Cromwell House i Norrköping arrangeras av H-DCS.\n\nNu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Cromwell House, Kungsgatan 36, Norrköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/pubkvall-pa-cromwell-house-i-norrkoping-3/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Cromwell+House%2C+Kungsgatan+36%2C+Norrk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "sulas-mc-kladesrea-2026",
      "name": "Sulas MC Klädesrea",
      "date": "2026-03-27",
      "dateEnd": "2026-03-28",
      "location": "Sulas MC, Säva 17, Uppsala",
      "type": "Fest",
      "organizer": "Sulas MC",
      "description": "Stor klädesrea hos Sulas MC. 15-50% rabatt på all personlig utrustning. Fre 9-18, Lör 10-14.",
      "link": "https://sulas.se/",
      "region": "Uppsala",
      "source": "sulas.se",
      "descriptionFull": "Sulas MC Klädesrea arrangeras av Sulas MC.\n\nStor klädesrea hos Sulas MC. 15-50% rabatt på all personlig utrustning. Fre 9-18, Lör 10-14.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Sulas MC, Säva 17, Uppsala.",
      "links": [
        {
          "label": "sulas.se",
          "url": "https://sulas.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sulas+MC%2C+S%C3%A4va+17%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-lordagsfika-pa-biltema-2026",
      "name": "DOA - Lördagsfika på Biltema",
      "date": "2026-03-28",
      "dateEnd": "2026-03-28",
      "location": "Biltema, Haninge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Varje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.",
      "link": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-03-28/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Lördagsfika på Biltema arrangeras av H-DCS.\n\nVarje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Haninge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-03-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+Haninge",
          "type": "map"
        }
      ]
    },
    {
      "id": "pub-kvall-med-tura-2026",
      "name": "PUB kväll med tura",
      "date": "2026-03-28",
      "dateEnd": "2026-03-28",
      "location": "Skåne",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "LOB- Ulf PUB kväll med tura Den 28/3 är det dags för en PUB tura Helsingborg/Helsingör. Vi träffas vid terminalen och åker med färjan Aurora kl 17:00.",
      "link": "https://hdcs.se/evenemang/lob-ulf-pub-kvall-med-tura/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "PUB kväll med tura arrangeras av H-DCS.\n\nLOB- Ulf PUB kväll med tura Den 28/3 är det dags för en PUB tura Helsingborg/Helsingör. Vi träffas vid terminalen och åker med färjan Aurora kl 17:00.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lob-ulf-pub-kvall-med-tura/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "pub-kvall-ludvika-2026",
      "name": "Pub-kväll Ludvika",
      "date": "2026-03-28",
      "dateEnd": "2026-03-28",
      "location": "China Town i Ludvika",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll med H-DCS i china town Ludvika, Ludvika. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!",
      "link": "https://hdcs.se/evenemang/pub-kvall-ludvika/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "Pub-kväll Ludvika arrangeras av H-DCS.\n\nPubkväll med H-DCS i china town Ludvika, Ludvika. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: China Town i Ludvika.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/pub-kvall-ludvika/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=China+Town+i+Ludvika",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-hojslappet-2026-2026",
      "name": "D10 Hojsläppet 2026",
      "date": "2026-03-28",
      "dateEnd": "2026-03-28",
      "location": "R40, Dalsjöfors Pendelparkering, Borås",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D10 från Borås, Västra Götalands län, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-hojslappet-2026/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Hojsläppet 2026 arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D10 från Borås, Västra Götalands län, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: R40, Dalsjöfors Pendelparkering, Borås.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-hojslappet-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R40%2C+Dalsj%C3%B6fors+Pendelparkering%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "ava-mc-dagen-gustavsberg-2026",
      "name": "AVA MC Dagen",
      "date": "2026-03-28",
      "dateEnd": "2026-03-28",
      "location": "Mörtnäs Hagväg 13, Gustavsberg",
      "type": "Träff",
      "organizer": "AVA MC",
      "description": "Provkör nya modeller, träffa Voge Sverige, fynda tillbehör och kläder till kampanjpriser. IMRG grillar korv. Finansiering och servicebokning på plats.",
      "descriptionFull": "Lördagen den 28 mars slår AVA MC upp portarna för årets stora öppet hus!\n\nIMRG är på plats och sköter korvgrillning till svältpriser. Vi har kaffe och kakor samt visar nyheter av respektive märke.\n\nBeroende på väder bokar vi kommande provkörningar på märke och modell som du är intresserad av att testa. Voge Sverige på plats för att svara på frågor och kanske en provkörning?\n\nFinansieringserbjudanden utlovas samt kvarvarande nya motorcyklar till kampanjpriser.\n\nTillbehörsbutiken är full av rabatterbjudanden gällande kläder och tillbehör. Vår serviceavdelning är självklart på plats för bokning av service eller reparation, bara du som saknas!\n\nVi syns lördagen den 28e mars från klockan 10 till 14, möjligen längre om ni håller ut!",
      "backImage": "ads/ava-mc-dagen-back-2026-03-28.jpg",
      "bgColor": "#9F9B90",
      "links": [
        {
          "label": "avamc.se",
          "url": "https://avamc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Mörtnäs+Hagväg+13+Gustavsberg",
          "type": "map"
        }
      ],
      "link": "https://avamc.se",
      "region": "Stockholm",
      "source": "FB"
    },
    {
      "id": "bmw-d15-mc-fest-hos-claesson-i-bromolla-2026",
      "name": "MC-Fest i Bromölla 2026",
      "date": "2026-03-28",
      "dateEnd": "2026-03-29",
      "location": "Industrigatan 11, Bromölla",
      "type": "Fest",
      "organizer": "Claessons Motor Bromölla",
      "description": "Claessons Motor firar 50 år med storslaget MC-fest! Generalagenter och leverantörer på plats. Unika erbjudanden, tävlingar och massor av utställare. 28-29 mars.",
      "descriptionFull": "Helgen den 28-29 mars är det äntligen dags för årets MC-Fest hos Claessons Motor i Bromölla!\n\n2026 firar vi dessutom 50 år vilket betyder att årets upplaga blir en fest med guldkant!\n\nDet blir ett storslaget firande och vi kommer att ha våra generalagenter, leverantörer och samarbetspartners på plats hos oss.\n\nDu kommer att kunna ta del av unika erbjudanden och kampanjer samt delta i tävlingar som endast gäller under dessa dagar.\n\nDet kommer även att finnas extra många utställare med allt som hör hojlivet till.\n\nDet här vill du inte missa! Boka in en tur till Bromölla helgen den 28-29 mars!",
      "frontImage": "ads/claessons-mcfest-2026-03-28.jpg",
      "backImage": "ads/claessons-mc-fest-back-2026-03-28.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1077891221149564"
        },
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-mc-fest-hos-claesson-i-bromolla/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Industrigatan+11+Bromölla",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1077891221149564",
      "region": "Skåne",
      "source": "FB"
    },
    {
      "id": "smc-varmland-årsmöte-2026",
      "name": "SMC Värmlands Årsmöte 2026",
      "date": "2026-03-28",
      "location": "Värmlands TrafikCenter, Grums",
      "region": "Värmland",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Varmlands årsmöte på Varmlands TrafikCenter i Grums. SMC bjuder som vanligt på fika. Motioner skickas till ordforande@svmc.se/varmland.",
      "link": "https://svmc.se/varmland",
      "descriptionFull": "SMC Värmlands Årsmöte 2026 arrangeras av SMC.\n\nSMC Varmlands årsmöte på Varmlands TrafikCenter i Grums. SMC bjuder som vanligt på fika. Motioner skickas till ordforande@svmc.se/varmland.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Värmlands TrafikCenter, Grums.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://svmc.se/varmland"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4rmlands+TrafikCenter%2C+Grums",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-vasterbotten-årsmöte-2026",
      "name": "SMC Västerbottens Årsmöte 2026",
      "date": "2026-03-28",
      "location": "Västerbotten",
      "region": "Västerbotten",
      "type": "Träff",
      "organizer": "SMC",
      "description": "SMC Vasterbottens årsmöte. Forerasning kl 11-13 om resor med SMC Travel-guide John Paakkonen. Arsmote kl 14-16. SMC bjuder på fika. Lunch bestalls på egen bekostnad. Boka via grundkurser.svmc.se.",
      "link": "https://grundkurser.svmc.se",
      "descriptionFull": "SMC Västerbottens Årsmöte 2026 arrangeras av SMC.\n\nSMC Vasterbottens årsmöte. Forerasning kl 11-13 om resor med SMC Travel-guide John Paakkonen. Arsmote kl 14-16. SMC bjuder på fika. Lunch bestalls på egen bekostnad. Boka via grundkurser.svmc.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Västerbotten.",
      "links": [
        {
          "label": "grundkurser.svmc.se",
          "url": "https://grundkurser.svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4sterbotten",
          "type": "map"
        }
      ]
    },
    {
      "id": "ttc-årsmöte-smc-dalarna-2026",
      "name": "Årsmöte SMC Dalarna",
      "date": "2026-03-29",
      "dateEnd": "2026-03-29",
      "location": "Leksands gymnasium, Rättviksvägen 29, Leksand",
      "type": "Träff",
      "organizer": "SMC Dalarna",
      "description": "SMC Dalarna håller årsmöte i Leksand. Internt möte för medlemmar med genomgång av verksamhetsåret och planering inför säsongen. Kl 15:00-17:00.",
      "descriptionFull": "SMC Dalarna håller årsmöte i Leksand, 29 mars kl 15:00.\n\nPlats: Leksands gymnasium, Rättviksvägen 29.\n\nInternt möte för SMC Dalarnas medlemmar med genomgång av verksamhetsåret, ekonomi och val av styrelse.\n\n15:00-17:00",
      "links": [
        {
          "label": "thetwinclub.se",
          "url": "https://www.thetwinclub.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Rättviksvägen+29+Leksand",
          "type": "map"
        }
      ],
      "link": "https://www.thetwinclub.se/",
      "region": "Dalarna",
      "source": "thetwinclub.se"
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-03-31",
      "dateEnd": "2026-03-31",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-03-31/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-03-31/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-04-01",
      "dateEnd": "2026-04-01",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-01/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "ttc-manadsmotet-april-2026",
      "name": "Månadsmöte april - The Twin Club",
      "date": "2026-04-01",
      "dateEnd": "2026-04-01",
      "location": "Klubbstugan, Ingarvsvägen 8 B, Falun",
      "type": "Träff",
      "organizer": "The Twin Club Falun",
      "description": "Månadsmöte för nya, kommande och redan invalda medlemmar samt andra intresserade. Klubben bjuder på fika. Protokoll publiceras på medlemssidorna.",
      "descriptionFull": "Månadsmöte i klubbstugan.\n\nFör nya, kommande och redan invalda medlemmar samt andra intresserade. Vi avslutar mötet med fika som klubben bjuder på.\n\nVarmt välkommen!\n\nProtokollet från mötet publiceras på medlemssidorna.\n\n19:00-20:30",
      "links": [
        {
          "label": "thetwinclub.se",
          "url": "https://www.thetwinclub.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ingarvsvägen+8B+Falun",
          "type": "map"
        }
      ],
      "link": "https://www.thetwinclub.se/",
      "region": "Dalarna",
      "source": "thetwinclub.se"
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-04-02",
      "dateEnd": "2026-04-02",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-04-02/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-04-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "custom-motor-show-2026",
      "name": "Custom Motor Show",
      "date": "2026-04-03",
      "dateEnd": "2026-04-06",
      "location": "Elmia, Jönköping",
      "type": "Show",
      "organizer": "Elmia",
      "description": "Custom Motor Show med fokus på specialbyggda bilar och motorcyklar. Utställare, tävlingar och livemusik.",
      "link": "https://www.elmia.se/",
      "region": "Jönköping",
      "source": "elmia.se",
      "descriptionFull": "Custom Motor Show arrangeras av Elmia.\n\nCustom Motor Show med fokus på specialbyggda bilar och motorcyklar. Utställare, tävlingar och livemusik.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Elmia, Jönköping.",
      "links": [
        {
          "label": "elmia.se",
          "url": "https://www.elmia.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Elmia%2C+J%C3%B6nk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-pubkvall-goteborg-2026",
      "name": "DO-C Pubkväll, Göteborg",
      "date": "2026-04-03",
      "dateEnd": "2026-04-03",
      "location": "The Golden Days, Södra Hamngatan 31, Göteborg",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll med H-DCS i The Golden Days, Södra Hamngatan 31, 411 14 Göteborg, Sverige. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!",
      "link": "https://hdcs.se/evenemang/do-c-pubkvall-goteborg/2026-04-03/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Pubkväll, Göteborg arrangeras av H-DCS.\n\nPubkväll med H-DCS i The Golden Days, Södra Hamngatan 31, 411 14 Göteborg, Sverige. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: The Golden Days, Södra Hamngatan 31, Göteborg.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-pubkvall-goteborg/2026-04-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=The+Golden+Days%2C+S%C3%B6dra+Hamngatan+31%2C+G%C3%B6teborg",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-lordagsfika-pa-biltema-2026",
      "name": "DOA - Lördagsfika på Biltema",
      "date": "2026-04-04",
      "dateEnd": "2026-04-04",
      "location": "Biltema, Haninge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Varje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.",
      "link": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-04/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Lördagsfika på Biltema arrangeras av H-DCS.\n\nVarje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Haninge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+Haninge",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-04-04",
      "dateEnd": "2026-04-04",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-04-04/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-04-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "kastberga-paskarallyt-2026",
      "name": "Påskarallyt",
      "date": "2026-04-04",
      "dateEnd": "2026-04-04",
      "location": "Kastberga 209, Eslöv",
      "type": "Körning",
      "organizer": "Kastberga MC",
      "description": "Påskrally till förmån för Min Stora Dag. Härlig tur längs Skånes vårvägar med sillamacka efteråt. Startavgift minst 100 kr per hjälm, allt går till välgörenhet.",
      "descriptionFull": "Kastberga MC arrangerar tillsammans med MC FÖR ALLA - SKÅNE Påskarallyt till förmån för Min Stora Dag.\n\nHäng med på en härlig tur längs Skånes vårvägar innan ni firar påskafton med släkt och vänner!\n\nVi bjuder på trevligt sällskap, lite roliga överraskningar och en go' sillamacka när vi samlas efter turen.\n\nStartavgiften är minst 100 kr per hjälm, och hela beloppet går oavkortat till Min Stora Dag.\n\nEn perfekt chans att göra något gott samtidigt som vi kickar igång MC-säsongen tillsammans.\n\nTävling: Vinnaren får en kostnadsfri årlig bas-service hos Kastberga MC. Tävlingen är helt frivillig att delta i.\n\nKl 11:00-14:00",
      "backImage": "ads/kastberga-paskarallyt-back-2026-04-04.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/872482305175445"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kastberga+209+Eslöv",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/872482305175445",
      "region": "Skåne",
      "source": "FB"
    },
    {
      "id": "pask-rundan-med-hakan-warnemyr-5-4-2026",
      "name": "Påsk rundan med Håkan Warnemyr 5/4",
      "date": "2026-04-05",
      "dateEnd": "2026-04-05",
      "location": "Staffanstorp",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Påsk rundan 5/4 Även i år tar Håkan Warnemyr ut oss på en härlig runda.",
      "link": "https://hdcs.se/evenemang/hakan-w-pask-rundan/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "Påsk rundan med Håkan Warnemyr 5/4 arrangeras av H-DCS.\n\nPåsk rundan 5/4 Även i år tar Håkan Warnemyr ut oss på en härlig runda.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Staffanstorp.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/hakan-w-pask-rundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Staffanstorp",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-annandagsturen-2026",
      "name": "Annandagsturen",
      "date": "2026-04-06",
      "dateEnd": "2026-04-06",
      "location": "Småland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Gemensam körning från Småland. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/annandagsturen/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "Annandagsturen arrangeras av BMW MC-klubben.\n\nGemensam körning från Småland. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Småland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/annandagsturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-04-07",
      "dateEnd": "2026-04-07",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-07/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-04-07",
      "dateEnd": "2026-04-07",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-07/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-04-08",
      "dateEnd": "2026-04-08",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-08/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-klubbafton-hos-handelsboden-2026",
      "name": "Klubbafton hos Handelsboden",
      "date": "2026-04-08",
      "dateEnd": "2026-04-08",
      "location": "Vigfastgatan 6, Linköping",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Vigfastgatan 6, 582 78 Linköping, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/klubbafton-hos-handelsboden/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "Klubbafton hos Handelsboden arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Vigfastgatan 6, 582 78 Linköping, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vigfastgatan 6, Linköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/klubbafton-hos-handelsboden/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Vigfastgatan+6%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-mcq-bodils-tur-med-drejning-och-spa-2026",
      "name": "D15 MCQ Bodils tur med drejning och spa",
      "date": "2026-04-10",
      "dateEnd": "2026-04-12",
      "location": "Vemmentorp 520, Örkelljunga, Sweden",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D15 från Vemmentorp 520, 286 92 Örkelljunga, Sweden. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-mcq-bodoils-tur-med-drejning-och-spa/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 MCQ Bodils tur med drejning och spa arrangeras av BMW MC-klubben.\n\nKörning med D15 från Vemmentorp 520, 286 92 Örkelljunga, Sweden. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vemmentorp 520, Örkelljunga, Sweden.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-mcq-bodoils-tur-med-drejning-och-spa/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Vemmentorp+520%2C+%C3%96rkelljunga%2C+Sweden",
          "type": "map"
        }
      ]
    },
    {
      "id": "sasongsoppning-smc-sthlm-2026",
      "name": "Säsongsöppning SMC Stockholm",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "SMC Stockholm",
      "description": "Välkommen på säsongsöppning! Gemensam utfärd och fika för att fira att mc-säsongen är igång.",
      "link": "https://www.svmc.se/",
      "region": "Stockholm",
      "source": "svmc.se",
      "descriptionFull": "Säsongsöppning SMC Stockholm arrangeras av SMC Stockholm.\n\nVälkommen på säsongsöppning! Gemensam utfärd och fika för att fira att mc-säsongen är igång.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-i-falun-2026",
      "name": "DOG - Fikaträff i Falun",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "Focus, Falun",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Falun. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-i-falun-3/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG - Fikaträff i Falun arrangeras av H-DCS.\n\nTräffpunkt för DOG i Falun. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Focus, Falun.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-i-falun-3/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Focus%2C+Falun",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-lordagsfika-pa-biltema-2026",
      "name": "DOA - Lördagsfika på Biltema",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "Biltema, Haninge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Varje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.",
      "link": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-11/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Lördagsfika på Biltema arrangeras av H-DCS.\n\nVarje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Haninge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+Haninge",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-pubkvall-ekero-2026",
      "name": "DOA - Pubkväll Ekerö",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "Lilla Venedig, Mälarö torg 9, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll på Lilla Venedig, Mälarö torg, Ekerö C För er som inte bor på någon av öarna tar ni er hit med SL-kort via.",
      "link": "https://hdcs.se/evenemang/doa-pubkvall-ekero-4/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Pubkväll Ekerö arrangeras av H-DCS.\n\nPubkväll på Lilla Venedig, Mälarö torg, Ekerö C För er som inte bor på någon av öarna tar ni er hit med SL-kort via.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Lilla Venedig, Mälarö torg 9, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-pubkvall-ekero-4/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lilla+Venedig%2C+M%C3%A4lar%C3%B6+torg+9%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-hojslapp-2026-2026",
      "name": "D11 Hojsläpp 2026",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "Korrö, Linneryd",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Då var det äntligen dags att starta upp årets mc säsong med årets Hojsläpp i D11. Välkommen!",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-hojslapp-2026/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Hojsläpp 2026 arrangeras av BMW MC-klubben.\n\nDå var det äntligen dags att starta upp årets mc säsong med årets Hojsläpp i D11. Välkommen!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Korrö, Linneryd.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-hojslapp-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Korr%C3%B6%2C+Linneryd",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-grusslappet-2026-2026",
      "name": "D10 Grussläppet 2026",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "R40 Dalsjöfors Pendelparkering, Borås",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D10. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-grusslappet-2026/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Grussläppet 2026 arrangeras av BMW MC-klubben.\n\nGrustour med D10. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: R40 Dalsjöfors Pendelparkering, Borås.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-grusslappet-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R40+Dalsj%C3%B6fors+Pendelparkering%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-mcq-brudar-bagar-bubbel-babbel-2026-2026",
      "name": "D11 mcQ Brudar, Bågar, Bubbel & Babbel 2026",
      "date": "2026-04-11",
      "dateEnd": "2026-04-12",
      "location": "Gamla Slottsvägen 1b, Lyckeby",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Efter hojsläpp kör mcQ gemensamt mot Karlskrona/Lyckeby att att fortsätta dagen med champagneprovning.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-mcq-bubbel-babbel-2026/",
      "region": "Blekinge",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 mcQ Brudar, Bågar, Bubbel & Babbel 2026 arrangeras av BMW MC-klubben.\n\nEfter hojsläpp kör mcQ gemensamt mot Karlskrona/Lyckeby att att fortsätta dagen med champagneprovning.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Gamla Slottsvägen 1b, Lyckeby.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-mcq-bubbel-babbel-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gamla+Slottsv%C3%A4gen+1b%2C+Lyckeby",
          "type": "map"
        }
      ]
    },
    {
      "id": "mc-dag-besikta-orebro-2026",
      "name": "MC-dag hos Besikta",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "Skottvägen 5, 703 84 Örebro",
      "type": "Träff",
      "organizer": "Besikta Bilprovning",
      "description": "MC-dag hos Besikta med besiktning till specialpris. Boka din MC-besiktning med 100 kr rabatt och gör hojen redo för säsongen.",
      "backImage": "ads/mc-dag-besikta-orebro-back-2026-04-11.jpg",
      "descriptionFull": "MC-dag hos Besikta i Örebro den 11 april 2026.\n\nNu är det dags att väcka hojen ur vintervilan! Under MC-dagen erbjuder Besikta 100 kr rabatt på besiktning av din motorcykel. Använd kampanjkoden MC2A2026 när du bokar din tid.\n\nÖppet kl 09:00-15:00.\n\nPlats: Besikta Örebro, Skottvägen 5, 703 84 Örebro.",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/2706683493034329/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skottv%C3%A4gen+5+%C3%96rebro",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/2706683493034329/",
      "region": "Örebro",
      "source": "facebook.com"
    },
    {
      "id": "start-of-season-holmegaard-2026",
      "name": "Start of Season 2026",
      "date": "2026-04-11",
      "dateEnd": "2026-04-11",
      "location": "Kalbyvej 32, 4684 Holmegaard, Danmark",
      "type": "Träff",
      "organizer": "Lazy Bones, Awesome Bikers & The Others MC",
      "description": "Säsongsstart i Danmark. Tre MC-klubbar bjuder in till en heldag med morgonmad, frokost, aftensmad, hygge och körtur. 150 kr.",
      "backImage": "ads/start-of-season-holmegaard-back-2026-04-11.jpg",
      "descriptionFull": "Start of Season 2026 den 11 april i Holmegaard, Danmark.\n\nFör femte gången arrangerar Lazy Bones, Awesome Bikers och The Others MC sin legendariska säsongsstart. Alla på två hjul är välkomna, oavsett om du kommer med klubb, familj eller ensam.\n\nMorgenmad, frokost och aftensmad ingår. Pris: 150 kr per person. Dessutom en fantastisk körtur.\n\nMax 125 platser, säljer ofta slut snabbt.\n\nPlats: Kalbyvej 32, 4684 Holmegaard, Danmark.",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/728281653052000/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kalbyvej+32+4684+Holmegaard+Danmark",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/728281653052000/",
      "region": "Danmark",
      "source": "facebook.com"
    },
    {
      "id": "bmw-d15-sondagsvandring-gyllebosjon-runt-2026",
      "name": "D15 Söndagsvandring Gyllebosjön Runt",
      "date": "2026-04-12",
      "dateEnd": "2026-04-12",
      "location": "Gyllebosjöns Badplats, Simrishamn",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Söndagsvandring runt Gyllebosjön, Österlens största sjö. Vandring genom vitsippor och ramslök med grillning efteråt.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-sondagsvandring-3/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Söndagsvandring Gyllebosjön Runt arrangeras av BMW MC-klubben.\n\nSöndagsvandring runt Gyllebosjön, Österlens största sjö. Vandring genom vitsippor och ramslök med grillning efteråt.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Gyllebosjöns Badplats, Simrishamn.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-sondagsvandring-3/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gyllebosj%C3%B6ns+Badplats%2C+Simrishamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-medlemsmote-apr-2026",
      "name": "Sormlandswingarnas Medlemsmote",
      "date": "2026-04-12",
      "dateEnd": "2026-04-12",
      "location": "Sigridslunds Cafe, Ardala",
      "type": "Träff",
      "organizer": "Sormlandswingarna / GWCS",
      "description": "MC-arrangemang i Sigridslunds Cafe, Ardala. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://gwcs.se/events/sormlandswingarnas-medlemsmote-2/?occurrence=2026-04-12",
      "region": "Södermanland",
      "source": "gwcs.se",
      "descriptionFull": "Sormlandswingarnas Medlemsmote arrangeras av Sormlandswingarna / GWCS.\n\nMC-arrangemang i Sigridslunds Cafe, Ardala. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sigridslunds Cafe, Ardala.",
      "links": [
        {
          "label": "gwcs.se",
          "url": "https://gwcs.se/events/sormlandswingarnas-medlemsmote-2/?occurrence=2026-04-12"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sigridslunds+Cafe%2C+Ardala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-04-14",
      "dateEnd": "2026-04-14",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-04-14",
      "dateEnd": "2026-04-14",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-14/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-04-15",
      "dateEnd": "2026-04-15",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-15/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-onsdagstur-med-blandat-underlag-2026",
      "name": "D15 Onsdagstur med blandat underlag",
      "date": "2026-04-15",
      "dateEnd": "2026-04-15",
      "location": "Ringsjövägen 40, Hörby",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Onsdagstur med D15 på blandade vägar. Samling enligt schema. Se bmwklubben.se för tider och plats.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-onsdagstur-med-blandat-underlag/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Onsdagstur med blandat underlag arrangeras av BMW MC-klubben.\n\nOnsdagstur med D15 på blandade vägar. Samling enligt schema. Se bmwklubben.se för tider och plats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ringsjövägen 40, Hörby.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-onsdagstur-med-blandat-underlag/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ringsj%C3%B6v%C3%A4gen+40%2C+H%C3%B6rby",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-04-16",
      "dateEnd": "2026-04-16",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-04-16/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-04-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "adv-expo-2026",
      "name": "ADV Motorcycle Expo 2026",
      "date": "2026-04-18",
      "dateEnd": "2026-04-19",
      "location": "Upplands Väsby",
      "type": "Show",
      "organizer": "Backroads AB / ADV Expo",
      "description": "Nordens största mässa för äventyrsmotorcyklister. Utställare, föreläsningar och provkörningar.",
      "link": "https://www.backroads.eu/adv-motorcycle-expo/",
      "region": "Stockholm",
      "source": "advmotorcycleexpo.com",
      "backImage": "ads/adv-motorcycle-expo-back-2026-04-18.jpg",
      "descriptionFull": "ADV Motorcycle Expo 2026 arrangeras av Backroads AB / ADV Expo.\n\nNordens största mässa för äventyrsmotorcyklister. Utställare, föreläsningar och provkörningar.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Upplands Väsby.",
      "links": [
        {
          "label": "backroads.eu",
          "url": "https://www.backroads.eu/adv-motorcycle-expo/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Upplands+V%C3%A4sby",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-avrostning-2026",
      "name": "DOA - Avrostning",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-avrostning/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Avrostning arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-avrostning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-lordagsfika-pa-biltema-2026",
      "name": "DOA - Lördagsfika på Biltema",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Biltema, Haninge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Varje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.",
      "link": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-18/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Lördagsfika på Biltema arrangeras av H-DCS.\n\nVarje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Haninge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+Haninge",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-04-18/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-04-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "medlemsmote-i-distrikt-f-med-quiz-och-bowling-2026",
      "name": "Medlemsmöte i distrikt F med Quiz och Bowling",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Main Event Lindesberg, Lindesberg",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Efter några ändringar av dag så är det bestämt att NU KÖR VI!Närmare bestämt lördag 18/4 på Main Event i Lindesberg.Vi samlas ca 16.",
      "link": "https://hdcs.se/evenemang/medlemsmote-i-distrikt-f-med-quiz-och-bowling/",
      "region": "Gotland",
      "source": "hdcs.se",
      "descriptionFull": "Medlemsmöte i distrikt F med Quiz och Bowling arrangeras av H-DCS.\n\nEfter några ändringar av dag så är det bestämt att NU KÖR VI!Närmare bestämt lördag 18/4 på Main Event i Lindesberg.Vi samlas ca 16.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Main Event Lindesberg, Lindesberg.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/medlemsmote-i-distrikt-f-med-quiz-och-bowling/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Main+Event+Lindesberg%2C+Lindesberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "pubkvall-pa-hamlet-i-linkoping-2026",
      "name": "Pubkväll på Hamlet i Linköping",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Hamlet, Storgatan 37, Linköping",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Nu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.",
      "link": "https://hdcs.se/evenemang/pubkvall-pa-hamlet-i-linkoping-5/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "Pubkväll på Hamlet i Linköping arrangeras av H-DCS.\n\nNu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Hamlet, Storgatan 37, Linköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/pubkvall-pa-hamlet-i-linkoping-5/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hamlet%2C+Storgatan+37%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "sulas-mc-invigning-2026",
      "name": "Sulas MC Stor Invigning",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Sulas MC, Säva 17, Uppsala",
      "type": "Fest",
      "organizer": "Sulas MC",
      "description": "Stor invigning av Sulas MC:s nya butik och showroom. Visning av nya modeller från BSA, Honda, Indian, Kawasaki, Suzuki och QJ Motor. Lör 10-16.",
      "link": "https://sulas.se/",
      "region": "Uppsala",
      "source": "sulas.se",
      "descriptionFull": "Sulas MC Stor Invigning arrangeras av Sulas MC.\n\nStor invigning av Sulas MC:s nya butik och showroom. Visning av nya modeller från BSA, Honda, Indian, Kawasaki, Suzuki och QJ Motor. Lör 10-16.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Sulas MC, Säva 17, Uppsala.",
      "backImage": "ads/sulas-mc-invigning-back-2026-04-18.jpg",
      "links": [
        {
          "label": "sulas.se",
          "url": "https://sulas.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sulas+MC%2C+S%C3%A4va+17%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "jsm-mx-tranas-2026",
      "name": "JSM MX2 J19, USM 85cc & Svemo Cup - Tranås",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Bredtorpsbanan, Tranås",
      "type": "Racing",
      "organizer": "SVEMO / Tranås MS",
      "description": "Junior-SM MX2 J19, Ungdoms-SM 85cc och Svemo Cup. Säsongens första stora MX-tävling.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Jönköping",
      "source": "svemo.se",
      "descriptionFull": "JSM MX2 J19, USM 85cc & Svemo Cup - Tranås arrangeras av SVEMO / Tranås MS.\n\nJunior-SM MX2 J19, Ungdoms-SM 85cc och Svemo Cup. Säsongens första stora MX-tävling.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Bredtorpsbanan, Tranås.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bredtorpsbanan%2C+Tran%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "probike-harley-day-provkorning-2026",
      "name": "Harley Day / Provkörningsevent",
      "date": "2026-04-18",
      "dateEnd": "2026-04-18",
      "location": "Probike Stockholm, Göteborg & Malmö",
      "type": "Träff",
      "organizer": "Probike",
      "description": "Kom in och provkör mängder av hojar. Eventet gäller alla tre Probikebutikerna.",
      "descriptionFull": "Harley Day hos Probike!\n\nKom in och provkör mängder av hojar hos Probike. Eventet gäller alla tre butikerna i Stockholm, Göteborg och Malmö.\n\nPerfekt tillfälle att testa nya Harley-Davidson modeller.",
      "links": [
        {
          "label": "Probike",
          "url": "https://www.probikebutiken.se/event/harley-day-provkorningsevent/"
        },
        {
          "label": "Probike butiker",
          "url": "https://www.probikebutiken.se/butiker/"
        }
      ],
      "link": "https://www.probikebutiken.se/event/harley-day-provkorningsevent/",
      "region": "Sverige",
      "source": "probikebutiken.se"
    },
    {
      "id": "bsa-sverige-sulas-2026",
      "name": "BSA hos Sulas MC",
      "date": "2026-04-19",
      "dateEnd": "2026-04-19",
      "location": "Sulas MC, Säva 17, Uppsala",
      "type": "Fest",
      "organizer": "BSA Sverige AB",
      "description": "BSA finns nu hos Sulas MC i Uppsala. Provkör Gold Star 650, Scrambler 650 och nya Bantam 350 på plats.",
      "descriptionFull": "BSA grundades 1861 i Birmingham och var ett av världens största motorcykelmärken. Produktionen upphörde 1973 men 2021 gjorde BSA en efterlängtad comeback.\n\nNya BSA-modeller finns nu hos Sulas MC i Uppsala:\n\nGold Star 650 - 652cc, 45 hk. Klassisk stil.\nScrambler 650 - Äventyrlig, finns i gult, grått och svart.\nBantam 350 - Helt ny! 344cc, 29 hk. Lätt stadshoj.\n\nBesök bsasverige.se eller Sulas MC för provkörning.",
      "frontImage": "ads/bsa-sulas-2026-04-18.jpg",
      "backImage": "ads/bsa-sulas-2026-04-18.jpg",
      "links": [
        {
          "label": "bsasverige.se",
          "url": "https://bsasverige.se/"
        },
        {
          "label": "sulas.se",
          "url": "https://sulas.se/vara-fordon/motorcyklar/bsa-motorcycles/"
        }
      ],
      "link": "https://bsasverige.se/",
      "region": "Uppsala",
      "source": "bsasverige.se"
    },
    {
      "id": "bmw-d15-kul-pa-hjul-2026-avrostning-2026",
      "name": "D15 Kul på Hjul 2026 - Avrostning",
      "date": "2026-04-19",
      "dateEnd": "2026-04-19",
      "location": "Nötesjövägen 293-12, Svedala",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Avrostningstur med D15. Säsongen drar igång med gemensam körning. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-avrostning/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Kul på Hjul 2026 - Avrostning arrangeras av BMW MC-klubben.\n\nAvrostningstur med D15. Säsongen drar igång med gemensam körning. Samling enligt schema på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nötesjövägen 293-12, Svedala.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-avrostning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=N%C3%B6tesj%C3%B6v%C3%A4gen+293-12%2C+Svedala",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-tur-till-ostersjon-2026",
      "name": "D10 Tur till Östersjön",
      "date": "2026-04-19",
      "dateEnd": "2026-04-19",
      "location": "Lilla Kongahällavägen, Kungälv",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D10 från Lilla Kongahällavägen, 442 90 Kungälv, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-tur-till-ostersjon/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Tur till Östersjön arrangeras av BMW MC-klubben.\n\nKörning med D10 från Lilla Kongahällavägen, 442 90 Kungälv, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lilla Kongahällavägen, Kungälv.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-tur-till-ostersjon/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lilla+Kongah%C3%A4llav%C3%A4gen%2C+Kung%C3%A4lv",
          "type": "map"
        }
      ]
    },
    {
      "id": "probike-malmo-avrostning-2026",
      "name": "Erikssons Trafikskolas Avrostning",
      "date": "2026-04-19",
      "dateEnd": "2026-04-19",
      "location": "Rimfrostgatan 5, 212 23 Malmö",
      "type": "Träff",
      "organizer": "Probike Malmö",
      "description": "Avrosta din mc-körning på manöverbanan, med din egen hoj eller prova en annan. Träffa återförsäljare och mingla med mc-förare.",
      "descriptionFull": "Probike Malmö på Erikssons Trafikskolas Avrostning!\n\nKom och avrosta din mc-körning på manöverbanan, med din egen hoj eller prova en annan hoj.\n\nTräffa återförsäljare.\nMingla med nya och erfarna mc-förare.\n\nPlats: Rimfrostgatan 5, Malmö. Start kl. 10:00.",
      "links": [
        {
          "label": "Probike",
          "url": "https://www.probikebutiken.se/event/probike-malmo-at-erikssons-trafikskolas-avrostning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Rimfrostgatan+5+Malmö",
          "type": "map"
        }
      ],
      "link": "https://www.probikebutiken.se/event/probike-malmo-at-erikssons-trafikskolas-avrostning/",
      "region": "Skåne",
      "source": "probikebutiken.se"
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-04-20",
      "dateEnd": "2026-04-20",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-04-20/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-04-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-04-21",
      "dateEnd": "2026-04-21",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-21/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-04-21",
      "dateEnd": "2026-04-21",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-21/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-04-21",
      "dateEnd": "2026-04-21",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-04-21/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-04-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-04-22",
      "dateEnd": "2026-04-22",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-04-22/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-04-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo, udda veckor",
      "date": "2026-04-22",
      "dateEnd": "2026-04-22",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-04-22/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo, udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-04-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-04-22",
      "dateEnd": "2026-04-22",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-22/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-04-22",
      "dateEnd": "2026-04-22",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-04-22/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-04-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-04-23",
      "dateEnd": "2026-04-23",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-04-23/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-04-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-04-23",
      "dateEnd": "2026-04-23",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-04-23/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-04-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-04-23",
      "dateEnd": "2026-04-23",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-04-23/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-04-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "pubkvall-pa-cromwell-house-i-norrkoping-2026",
      "name": "Pubkväll på Cromwell House i Norrköping",
      "date": "2026-04-24",
      "dateEnd": "2026-04-24",
      "location": "Cromwell House, Kungsgatan 36, Norrköping",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Nu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.",
      "link": "https://hdcs.se/evenemang/pubkvall-pa-cromwell-house-i-norrkoping-4/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "Pubkväll på Cromwell House i Norrköping arrangeras av H-DCS.\n\nNu är det åter dags för en trevlig pubkväll med lite mat och dryck samt trevligt sällskap. Hjärtligt välkomna.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Cromwell House, Kungsgatan 36, Norrköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/pubkvall-pa-cromwell-house-i-norrkoping-4/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Cromwell+House%2C+Kungsgatan+36%2C+Norrk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "bikedagarna-trollhattan-2026",
      "name": "Bikedagarna 2026",
      "date": "2026-04-24",
      "dateEnd": "2026-04-25",
      "location": "Lextorpsvägen 997, Trollhättan",
      "type": "Fest",
      "organizer": "Bike Trollhättan",
      "description": "Två dagar med stuntshow, provkörningar och bra deals på MC-kläder, hjälmar och tillbehör. Honda är tillbaka med nya modeller. Gratis entré, mat och fika på plats.",
      "descriptionFull": "24-25 april är det dags för Bikedagarna!\n\nTvå dagar fyllda av stuntshow, provkörningar och riktigt bra deals. Oavsett om du redan kör, planerar att ta kort eller bara älskar känslan runt hojar och gemenskap, så är det här helgen du inte vill missa.\n\nUnder två fullmatade dagar bjuder vi på riktigt starka erbjudanden på MC-kläder, hjälmar och tillbehör. Det blir även förmånliga kampanjer på motorcyklar och möjlighet att provköra många av årets hetaste nyheter.\n\nExtra roligt i år är att Honda äntligen är tillbaka hos oss, vilket innebär ännu fler spännande modeller att uppleva.\n\nDet händer mer än bara affärer. Under helgen blir det stuntshow, god mat och aktiviteter för barnen, vilket gör detta till en perfekt utflykt för både hojåkare och hela familjen.\n\nEvenemanget äger rum hos oss på Lextorpsvägen 997 i Trollhättan.\n\nDet är gratis att besöka oss på Bikedagarna!\n\nVi ses på Bikedagarna!",
      "backImage": "ads/bikedagarna-back-2026-04-24.jpg",
      "links": [
        {
          "label": "biketrollhattan.se",
          "url": "https://biketrollhattan.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lextorpsvägen+997+Trollhättan",
          "type": "map"
        }
      ],
      "link": "https://biketrollhattan.se",
      "region": "Västra Götaland",
      "source": "FB"
    },
    {
      "id": "mc-collection-sasongsoppning-2026",
      "name": "MC Collection Säsongsöppning",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Tidö Slott, Västerås",
      "type": "Fest",
      "organizer": "MC Collection",
      "description": "Säsongsöppning av MC Collection museum vid Tidö Slott. Över 100 motorcyklar, utställningar och kafé.",
      "link": "https://mc-collection.com/kalender/",
      "region": "Västmanland",
      "source": "mc-collection.com",
      "descriptionFull": "MC Collection Säsongsöppning arrangeras av MC Collection.\n\nSäsongsöppning av MC Collection museum vid Tidö Slott. Över 100 motorcyklar, utställningar och kafé.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Tidö Slott, Västerås.",
      "links": [
        {
          "label": "mc-collection.com",
          "url": "https://mc-collection.com/kalender/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tid%C3%B6+Slott%2C+V%C3%A4ster%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-lordagsfika-pa-biltema-2026",
      "name": "DOA - Lördagsfika på Biltema",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Biltema, Haninge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Varje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.",
      "link": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-25/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Lördagsfika på Biltema arrangeras av H-DCS.\n\nVarje lördag med H-DCS. Kl.10:30 Fika - Biltema Biltema Haninge var mycket populärt förra vintersäsongen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Haninge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-lordagsfika-pa-biltema/2026-04-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+Haninge",
          "type": "map"
        }
      ]
    },
    {
      "id": "tranrikerundan-2026",
      "name": "Tranrikerundan",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Småland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Småland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/tranrikerundan/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Tranrikerundan arrangeras av H-DCS.\n\nArrangemang med H-DCS från Småland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/tranrikerundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-medlemsmote-25-26-april-2026",
      "name": "DOE Medlemsmöte 25-26 april",
      "date": "2026-04-25",
      "dateEnd": "2026-04-26",
      "location": "Småland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Medlemsmöte på Asa vandrarhem 25 - 26 april 2026 Välkomna till vårens medlemsmöte på Asa Vandrarhem som ligger ca 15 km öster om Lammhult.",
      "link": "https://hdcs.se/evenemang/doe-medlemsmote-25-26-april/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Medlemsmöte 25-26 april arrangeras av H-DCS.\n\nMedlemsmöte på Asa vandrarhem 25 - 26 april 2026 Välkomna till vårens medlemsmöte på Asa Vandrarhem som ligger ca 15 km öster om Lammhult.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-medlemsmote-25-26-april/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-pubkvall-stockholm-2026",
      "name": "DOA - Pubkväll Stockholm",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Pub Anchor, Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll med H-DCS DOA i Pub Anchor, Stockholm. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!",
      "link": "https://hdcs.se/evenemang/doa-pubkvall-stockholm/2026-04-25/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Pubkväll Stockholm arrangeras av H-DCS.\n\nPubkväll med H-DCS DOA i Pub Anchor, Stockholm. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Pub Anchor, Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-pubkvall-stockholm/2026-04-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Pub+Anchor%2C+Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "sportbaren-awee-adelsgatan-2026",
      "name": "Sportbaren Awee adelsgatan",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Adelsgatan 18, Visby",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Sportbararen Awee Adelsgatan, ej klart vad det blir, men någon aktivitet med efterföljande mat eller nåt.",
      "link": "https://hdcs.se/evenemang/sportbaren-awee-adelsgatan/",
      "region": "Gotland",
      "source": "hdcs.se",
      "descriptionFull": "Sportbaren Awee adelsgatan arrangeras av H-DCS.\n\nSportbararen Awee Adelsgatan, ej klart vad det blir, men någon aktivitet med efterföljande mat eller nåt.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Adelsgatan 18, Visby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/sportbaren-awee-adelsgatan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Adelsgatan+18%2C+Visby",
          "type": "map"
        }
      ]
    },
    {
      "id": "lob-morten-kofod-pub-kvall-i-markaryd-2026",
      "name": "LOB- Morten Kofod PUB kväll i Markaryd",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Markaryd",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Då har vi tänkt lägga en PUB kväll för er medlemmar i Markaryd området. Givetvis är alla välkomna. Det är vårt LO Morten som ordnar denna.",
      "link": "https://hdcs.se/evenemang/lob-morten-kofod-pub-kvall-i-markaryd/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "LOB- Morten Kofod PUB kväll i Markaryd arrangeras av H-DCS.\n\nDå har vi tänkt lägga en PUB kväll för er medlemmar i Markaryd området. Givetvis är alla välkomna. Det är vårt LO Morten som ordnar denna.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Markaryd.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lob-morten-kofod-pub-kvall-i-markaryd/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Markaryd",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-classic-veteranmarknad-2026",
      "name": "Classic Veteranmarknad",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Sparrsätra Lådö 16, Enköping",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Gemensam körning från Sparrsätra Lådö 16, 745 95 Enköping, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/classic-veteranmarknad/",
      "region": "Uppsala",
      "source": "bmwklubben.se",
      "descriptionFull": "Classic Veteranmarknad arrangeras av BMW MC-klubben.\n\nGemensam körning från Sparrsätra Lådö 16, 745 95 Enköping, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Sparrsätra Lådö 16, Enköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/classic-veteranmarknad/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sparrs%C3%A4tra+L%C3%A5d%C3%B6+16%2C+Enk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-klubbknix-pa-gtr-motorpark-2026",
      "name": "D7 KlubbKnix på GTR Motorpark",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Tommy Janssons väg 17, Eskilstuna",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D7 från Tommy Janssons väg 17, 635 11 Eskilstuna, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-klubbknix-pa-gtr-motorpark/",
      "region": "Västmanland",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 KlubbKnix på GTR Motorpark arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D7 från Tommy Janssons väg 17, 635 11 Eskilstuna, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Tommy Janssons väg 17, Eskilstuna.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-klubbknix-pa-gtr-motorpark/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tommy+Janssons+v%C3%A4g+17%2C+Eskilstuna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-kul-pa-hjul-2026-avrostning-med-smc-2026",
      "name": "D11 Kul På Hjul 2026 - Avrostning med SMC",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Småland",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Avrostningstur med D11. Säsongen drar igång med gemensam körning. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-kul-pa-hjul/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Kul På Hjul 2026 - Avrostning med SMC arrangeras av BMW MC-klubben.\n\nAvrostningstur med D11. Säsongen drar igång med gemensam körning. Samling enligt schema på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-kul-pa-hjul/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-dagtouring-apr-2026",
      "name": "ÖAMCK Dagtouring",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Lelles MC, Uppsala",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/dagtouring/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Dagtouring arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lelles MC, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/dagtouring/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lelles+MC%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "langkalsongrallyt-garphyttan-2026",
      "name": "Långkalsongrallyt 2026",
      "date": "2026-04-25",
      "dateEnd": "2026-04-25",
      "location": "Garphyttans Folkets Park, Garphyttan",
      "type": "Körning",
      "organizer": "Full Moppers MK",
      "description": "MC-rally genom vackra landskap runt Garphyttan. Ca 5 mil lång bana. Startavgift 200 kr inklusive mat, fika och lotteri.",
      "descriptionFull": "Långkalsongrallyt i Garphyttan den 25 april 2026.\n\nÅrligt MC-rally arrangerat av Full Moppers MK. Starten går från Folkets Park i Garphyttan och banan är ca 5 mil lång genom vackra landskap.\n\nSamling kl 10:00, start kl 11:00.\n\nStartavgift: 200 kr (kontant eller Swish). I priset ingår mat, fika och 2 st lotter med fina priser. Vid deltagande stödjer du olika hjälporganisationer.\n\nMer info: www.fullmoppers.se\n\nPlats: Garphyttans Folkets Park, Garphyttan.",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/1259563945922244/"
        },
        {
          "label": "fullmoppers.se",
          "url": "https://www.fullmoppers.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Garphyttans+Folkets+Park",
          "type": "map"
        }
      ],
      "backImage": "ads/langkalsongrallyt-back-2026-04-25.jpg",
      "link": "https://www.facebook.com/events/1259563945922244/",
      "region": "Örebro",
      "source": "facebook.com"
    },
    {
      "id": "bmw-d9-aprilgruset-2026",
      "name": "D9 Aprilgruset",
      "date": "2026-04-26",
      "dateEnd": "2026-04-26",
      "location": "Låskolvsgatan 1, Linköping",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Välkommen till den femte upplagan av april-/majgruset, en heldag på grus söderut från Linköping.",
      "link": "https://www.bmwklubben.se/aktiviteter/d9-aprilgruset/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "D9 Aprilgruset arrangeras av BMW MC-klubben.\n\nVälkommen till den femte upplagan av april-/majgruset, en heldag på grus söderut från Linköping.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Låskolvsgatan 1, Linköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d9-aprilgruset/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=L%C3%A5skolvsgatan+1%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d9-valkomst-tur-nya-medlemmar-2026",
      "name": "D9 Välkomst-tur nya medlemmar",
      "date": "2026-04-26",
      "dateEnd": "2026-04-26",
      "location": "Låskolvsgatan 1, Linköping",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Den här aktiviteten riktar sig främst till dig som är ny medlem eller aldrig varit med på en aktivitet.",
      "link": "https://www.bmwklubben.se/aktiviteter/d9-valkomst-tur-nya-medlemmar/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "D9 Välkomst-tur nya medlemmar arrangeras av BMW MC-klubben.\n\nDen här aktiviteten riktar sig främst till dig som är ny medlem eller aldrig varit med på en aktivitet.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Låskolvsgatan 1, Linköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d9-valkomst-tur-nya-medlemmar/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=L%C3%A5skolvsgatan+1%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-04-27",
      "dateEnd": "2026-04-27",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-04-27/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-04-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-04-28",
      "dateEnd": "2026-04-28",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-28/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-04-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-04-28",
      "dateEnd": "2026-04-28",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-28/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-04-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, jämna veckor",
      "date": "2026-04-28",
      "dateEnd": "2026-04-28",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-04-28/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-04-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-04-29",
      "dateEnd": "2026-04-29",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-04-29/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-04-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "lod-onsdagsfika-soldathemmet-skovde-2026",
      "name": "LoD Onsdagsfika Soldathemmet Skövde",
      "date": "2026-04-29",
      "dateEnd": "2026-04-29",
      "location": "Soldathemmet Skövde, Skövde",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-29/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LoD Onsdagsfika Soldathemmet Skövde arrangeras av H-DCS.\n\nTräffpunkt för LoD i Soldathemmet Skövde, Skövde. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Soldathemmet Skövde, Skövde.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lod-onsdagsfika-soldathemmet-skovde-2/2026-04-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Soldathemmet+Sk%C3%B6vde%2C+Sk%C3%B6vde",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-04-29",
      "dateEnd": "2026-04-29",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-04-29/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-04-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-04-30",
      "dateEnd": "2026-04-30",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-04-30/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-04-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-04-30",
      "dateEnd": "2026-04-30",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-04-30/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-04-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-04-30",
      "dateEnd": "2026-04-30",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-04-30/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-04-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-04-30",
      "dateEnd": "2026-04-30",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-04-30/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-04-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-majbrasa-2026",
      "name": "DOH-Majbrasa",
      "date": "2026-04-30",
      "dateEnd": "2026-04-30",
      "location": "Sawmill MC Tavelsjö, Tavelsjö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi firar Valborgsmässoafton med en en stor brasa på sågen i Tavelsjö. Om allt går enligt planerna tuttar vi på majbrasan klockan 19.00.",
      "link": "https://hdcs.se/evenemang/doh-majbrasa/2026-04-30/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Majbrasa arrangeras av H-DCS.\n\nVi firar Valborgsmässoafton med en en stor brasa på sågen i Tavelsjö. Om allt går enligt planerna tuttar vi på majbrasan klockan 19.00.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sawmill MC Tavelsjö, Tavelsjö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-majbrasa/2026-04-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sawmill+MC+Tavelsj%C3%B6%2C+Tavelsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "balders-hage-bil-mc-cafe-2026",
      "name": "Bil & MC Café - Säsongspremiär",
      "date": "2026-04-30",
      "dateEnd": "2026-04-30",
      "location": "Balders Hage Noje, Haradsback",
      "type": "Träff",
      "organizer": "Balders Hage Noje",
      "description": "Populart bil- och MC-cafe på Valborgsmassooafton! Buffe från 17:00. Sedan varannan vecka hela sommaren.",
      "descriptionFull": "Bil & MC Café på Balders Hage i Häradsbäck arrangeras av Balders Hage Nöje.\n\nBalders Hages kvällar med Bil- och Motorcykelcafé har blivit otroligt populärt sedan starten för ca 5 år sedan! Med gemytlig stämning samlas människor varannan vecka från april till september för att äta, prata, umgås och titta på fordon av olika slag.\n\nSäsongspremiär Valborgsmässoafton 30 april kl 17:00.\n\nBuffé från 17:00:\n- Timjan & vitlöksmarinerad fläskkarré, potatisgratäng, Béarnaise, sallad, kaffe/te - 135 kr (barn under 12 år - 65 kr)\n- Två stekta, mos, lingon, sallad - 95 kr\n- Tre pannkakor grädde & sylt - 65 kr\n- Ostkaka, grädde & sylt - 65 kr\n\nÖvriga datum 2026: 14/5, 28/5, 4/6, 25/6, 9/7, 23/7, 6/8, 20/8, 3/9, 17/9.\n\nPlats: Balders Hage Nöje, Linvägen 3, 343 92 Häradsbäck.",
      "backImage": "ads/balders-hage-bil-mc-cafe-back-2026-04-30.jpg",
      "links": [
        {
          "label": "baldershagenoje.se",
          "url": "https://baldershagenoje.se/biltraff/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Balders+Hage+N%C3%B6je+H%C3%A4radsb%C3%A4ck",
          "type": "map"
        }
      ],
      "link": "https://baldershagenoje.se/biltraff/",
      "region": "Kronoberg",
      "source": "FB",
      "frontImage": "ads/balders-hage-bil-mc-cafe-front-2026-04-30.jpg"
    },
    {
      "id": "dkb-mt-rally-mot-prostatacancer-2026",
      "name": "DKB- MT-Rally mot Prostatacancer",
      "date": "2026-05-01",
      "dateEnd": "2026-05-01",
      "location": "Blekinge",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DKB från Blekinge. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dkb-mt-rally-mot-prostatacancer/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DKB- MT-Rally mot Prostatacancer arrangeras av H-DCS.\n\nArrangemang med H-DCS DKB från Blekinge. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Blekinge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dkb-mt-rally-mot-prostatacancer/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Blekinge",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-forsta-maj-körning-2026",
      "name": "DO-C Första maj körning",
      "date": "2026-05-01",
      "dateEnd": "2026-05-01",
      "location": "Halland/VGötaland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Halland/VGötaland. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-forsta-maj-körning/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Första maj körning arrangeras av H-DCS.\n\nKörtur med H-DCS från Halland/VGötaland. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Halland/VGötaland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-forsta-maj-körning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halland%2FVG%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-mj-sa-rundt-1-mai-2026",
      "name": "Mjøsa Rundt - 1. Mai",
      "date": "2026-05-01",
      "dateEnd": "2026-05-01",
      "location": "Hamar, Innlandet, Norge",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Bli med og kjøre rundt Norges største innsjø. 1. mai 2026 vil den tradisjonelle Mjøsrunden gå för 54 gang.",
      "link": "https://www.bmwklubben.se/aktiviteter/mjosa-rundt-1-mai/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "Mjøsa Rundt - 1. Mai arrangeras av BMW MC-klubben.\n\nBli med og kjøre rundt Norges største innsjø. 1. mai 2026 vil den tradisjonelle Mjøsrunden gå för 54 gang.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Hamar, Innlandet, Norge.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/mjosa-rundt-1-mai/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hamar%2C+Innlandet%2C+Norge",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-inkörningen-en-tur-for-nya-och-nyfikna-2026",
      "name": "Inkörningen - en tur för nya och nyfikna",
      "date": "2026-05-01",
      "dateEnd": "2026-05-01",
      "location": "Norra Annebergsvägen, Kungsbacka",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Ny i klubben eller nyfiken på körning i grupp? Då är detta turen där vi rullar igång tillsammans, i lugnt tempo och med fokus på trygg körning.",
      "link": "https://www.bmwklubben.se/aktiviteter/tur-for-nya/",
      "region": "Halland",
      "source": "bmwklubben.se",
      "descriptionFull": "Inkörningen - en tur för nya och nyfikna arrangeras av BMW MC-klubben.\n\nNy i klubben eller nyfiken på körning i grupp? Då är detta turen där vi rullar igång tillsammans, i lugnt tempo och med fokus på trygg körning.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Norra Annebergsvägen, Kungsbacka.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/tur-for-nya/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Norra+Annebergsv%C3%A4gen%2C+Kungsbacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-oppet-hus-mc-massa-i-samarbete-med-omcc-2026",
      "name": "Öppet hus/MC mässa i samarbete med ÖMCC",
      "date": "2026-05-01",
      "dateEnd": "2026-05-01",
      "location": "Viktoriaesplanaden 3e, Örnsköldsvik",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Gemensam körning från Viktoriaesplanaden 3e, 891 39 Örnsköldsvik, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/oppet-hus-mc-massa-i-samarbete-med-omcc/",
      "region": "Västernorrland",
      "source": "bmwklubben.se",
      "descriptionFull": "Öppet hus/MC mässa i samarbete med ÖMCC arrangeras av BMW MC-klubben.\n\nGemensam körning från Viktoriaesplanaden 3e, 891 39 Örnsköldsvik, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Viktoriaesplanaden 3e, Örnsköldsvik.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/oppet-hus-mc-massa-i-samarbete-med-omcc/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Viktoriaesplanaden+3e%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "mxsm-landskrona-2026",
      "name": "MXSM Deltävling 1 - Landskrona",
      "date": "2026-05-01",
      "dateEnd": "2026-05-02",
      "location": "Landskrona MX, Landskrona",
      "type": "Racing",
      "organizer": "SVEMO / Landskrona MX",
      "description": "Svenska Mästerskapen i Motocross (MX1, MX2, MX-Women, USM 125 U17). Säsongens första MXSM-deltävling.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Skåne",
      "source": "svemo.se",
      "descriptionFull": "MXSM Deltävling 1 - Landskrona arrangeras av SVEMO / Landskrona MX.\n\nSvenska Mästerskapen i Motocross (MX1, MX2, MX-Women, USM 125 U17). Säsongens första MXSM-deltävling.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Landskrona MX, Landskrona.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Landskrona+MX%2C+Landskrona",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-05-02",
      "dateEnd": "2026-05-02",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-02/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-05-02",
      "dateEnd": "2026-05-02",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-02/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-05-02",
      "dateEnd": "2026-05-02",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-05-02/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-05-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-oskarshamnsgrus-norr-2026",
      "name": "D11 Oskarshamnsgrus Norr",
      "date": "2026-05-02",
      "dateEnd": "2026-05-02",
      "location": "Oskarshamn",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D11. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-oskarshamnsgrus-norr/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Oskarshamnsgrus Norr arrangeras av BMW MC-klubben.\n\nGrustour med D11. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Oskarshamn.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-oskarshamnsgrus-norr/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Oskarshamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d9-sormlandstur-2026",
      "name": "D9 Sörmlandstur",
      "date": "2026-05-02",
      "dateEnd": "2026-05-02",
      "location": "Stavsjö Trafikplats 5, Stavsjö",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D9 från Stavsjö Trafikplats 5, 618 95 Stavsjö, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d9-sormlandstur/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "D9 Sörmlandstur arrangeras av BMW MC-klubben.\n\nKörning med D9 från Stavsjö Trafikplats 5, 618 95 Stavsjö, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stavsjö Trafikplats 5, Stavsjö.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d9-sormlandstur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stavsj%C3%B6+Trafikplats+5%2C+Stavsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "sulas-mc-provkörning-2026",
      "name": "Sulas MC Provkörningshelg",
      "date": "2026-05-02",
      "dateEnd": "2026-05-03",
      "location": "Sulas MC, Säva 17, Uppsala",
      "type": "Fest",
      "organizer": "Sulas MC",
      "description": "Provkör demohojar från BSA, Honda, Indian, Kawasaki, Suzuki och QJ Motor. Lör 10-14, Sön 11-15.",
      "link": "https://sulas.se/provkörning/",
      "region": "Uppsala",
      "source": "sulas.se",
      "descriptionFull": "Sulas MC Provkörningshelg arrangeras av Sulas MC.\n\nProvkör demohojar från BSA, Honda, Indian, Kawasaki, Suzuki och QJ Motor. Lör 10-14, Sön 11-15.\n\nMC-mässa med utställare, provkörningar och tillbehör. Kolla arrangören för öppettider.\n\nPlats: Sulas MC, Säva 17, Uppsala.",
      "links": [
        {
          "label": "sulas.se",
          "url": "https://sulas.se/provkörning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sulas+MC%2C+S%C3%A4va+17%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "gokrallyt-ljungby-2026",
      "name": "Gökrallyt 2026",
      "date": "2026-05-02",
      "dateEnd": "2026-05-02",
      "location": "Vislandavägen, Ljungby",
      "type": "Körning",
      "organizer": "Hermans Hedningar LMCK",
      "description": "MC-rally med praktiska och teoretiska prov för både förare och passagerare. Start kl 10-12, mål vid MCK Röde Orms klubbstuga.",
      "descriptionFull": "Hermans Hedningar LMCK och MCK Röde Orm arrangerar Gökrallyt 2026.\n\nStart från HermansHedningar Lmck, Vislandavägen, Ljungby. Starten är öppen lördag mellan kl 10.00-12.00. Möjlighet att köpå fika finns i starten.\n\nMC-rally med praktiska och teoretiska prov för både förare och passagerare. Fina priser!\n\nMål vid MCK Röde Orms klubbstuga där aktiviteterna fortsätter med prisutdelning. Försäljning av mat och dryck.\n\nStartavgift: 100 kr. Startavgiften skänkes till välgörande ändamål som är Våra barn.\n\nKontakt:\nJohanna: 0761887226\nJan: 0703561060\n\nPlats: Vislandavägen, Ljungby.",
      "backImage": "ads/gokrallyt-back-2026-05-02.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1486198450181505/"
        },
        {
          "label": "hermanshedningar.se",
          "url": "https://www.hermanshedningar.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hermans+Hedningar+LMCK+Ljungby",
          "type": "map"
        }
      ],
      "link": "https://www.hermanshedningar.se/",
      "region": "Kronoberg",
      "source": "FB"
    },
    {
      "id": "motorcykelns-dag-2026",
      "name": "Motorcykelns dag 2026",
      "date": "2026-05-02",
      "dateEnd": "2026-05-02",
      "location": "Hela Sverige (lokala MC-handlare)",
      "type": "Show",
      "organizer": "McBranschen",
      "description": "Motorcykelns dag hos din lokala MC-handlare. Provkörningar, erbjudanden och aktiviteter på MC-butiker över hela Sverige.",
      "descriptionFull": "Motorcykelns dag den 2 maj 2026 arrangeras av McBranschen.\n\nÅrlig rikstäckande dag då MC-handlare över hela Sverige öppnar dörrarna för provkörningar, specialerbjudanden och aktiviteter. Besök din lokala MC-butik och starta säsongen!\n\nMotorcykelns dag är Sveriges största gemensamma MC-evenemang med hundratals deltagande butiker från Malmö till Luleå.\n\nPlats: Din lokala MC-handlare. Se mcbranschen.se för deltagande butiker.",
      "links": [
        {
          "label": "mcbranschen.se",
          "url": "https://mcbranschen.se/motorcykelns-dag/"
        }
      ],
      "link": "https://mcbranschen.se/motorcykelns-dag/",
      "backImage": "ads/motorcykelns-dag-back-2026-05-02.jpg",
      "region": "Sverige",
      "source": "mcbranschen.se"
    },
    {
      "id": "varmonstring-2026",
      "name": "Vårmönstring",
      "date": "2026-05-03",
      "dateEnd": "2026-05-03",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/varmonstring/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Vårmönstring arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/varmonstring/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-omberg-runt-2026-2026",
      "name": "D11 Omberg Runt 2026",
      "date": "2026-05-03",
      "dateEnd": "2026-05-03",
      "location": "Biltema, Nässjö",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Välkommen till Omberg Runt. Längs slingriga fina asfaltsvägar tar vi oss upp till Omberg.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-omberg-runt-2026/",
      "region": "Jönköping",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Omberg Runt 2026 arrangeras av BMW MC-klubben.\n\nVälkommen till Omberg Runt. Längs slingriga fina asfaltsvägar tar vi oss upp till Omberg.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Biltema, Nässjö.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-omberg-runt-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema%2C+N%C3%A4ssj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-tur-2026",
      "name": "D14-tur",
      "date": "2026-05-03",
      "dateEnd": "2026-05-03",
      "location": "Björkallén 32, Oskarström",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D14 från Björkallén 32, 313 33 Oskarström, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-test/",
      "region": "Gotland",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-tur arrangeras av BMW MC-klubben.\n\nKörning med D14 från Björkallén 32, 313 33 Oskarström, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Björkallén 32, Oskarström.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-test/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bj%C3%B6rkall%C3%A9n+32%2C+Oskarstr%C3%B6m",
          "type": "map"
        }
      ]
    },
    {
      "id": "ttc-avrostning-dala-jarna-2026",
      "name": "Avrostning Dala-Järna",
      "date": "2026-05-03",
      "dateEnd": "2026-05-03",
      "location": "Dala-Järna Flygplats, Flygfältsvägen 22, Dala-Järna",
      "type": "Träff",
      "organizer": "Västerdala MCK",
      "description": "Gratis avrostning på flygfältet i Dala-Järna. Alla välkomna oavsett kön eller MC-typ, även övningskörare. Samarbete med SMC Dalarna. Start kl 13:00.",
      "descriptionFull": "Den 3:e maj kl. 13:00 bjuder Västerdala MC-klubb in till avrostning på flygfältet i Dala-Järna.\n\nKursen är kostnadsfri. Medtag glatt humör.\n\nAvrostning vid Dala-Järna Flygraka för VMCK. Alla är välkomna på avrostning oavsett man/kvinna eller vad du kör för MC. Även om du övningskör är du välkommen, perfekt träning.\n\nAvrostningen börjar kl. 13:00.\n\nI samarbete med SMC Dalarna.",
      "links": [
        {
          "label": "thetwinclub.se",
          "url": "https://www.thetwinclub.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Flygfältsvägen+22+Dala-Järna",
          "type": "map"
        }
      ],
      "link": "https://www.thetwinclub.se/",
      "region": "Dalarna",
      "source": "thetwinclub.se"
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-05-04",
      "dateEnd": "2026-05-04",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-04/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-knixpremiar-2026",
      "name": "Knixpremiar",
      "date": "2026-05-04",
      "dateEnd": "2026-05-04",
      "location": "Rorken, Uppsala",
      "type": "Träff",
      "organizer": "SMC Uppsala",
      "description": "Arrangemang med SMC i Rorken, Uppsala. Mötespunkt för MC-klubben. Mer information på smc.nu.",
      "link": "https://oamck.se/events/knixpremiar-2/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "Knixpremiar arrangeras av SMC Uppsala.\n\nArrangemang med SMC i Rorken, Uppsala. Mötespunkt för MC-klubben. Mer information på smc.nu.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rorken, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/knixpremiar-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Rorken%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-05-05",
      "dateEnd": "2026-05-05",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-05/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-05-05",
      "dateEnd": "2026-05-05",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-05/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-05-05",
      "dateEnd": "2026-05-05",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-05-05/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-05-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-05-06",
      "dateEnd": "2026-05-06",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-06/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo, udda veckor",
      "date": "2026-05-06",
      "dateEnd": "2026-05-06",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-05-06/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo, udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-05-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-05-06",
      "dateEnd": "2026-05-06",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-06/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-onsdagstur-med-blandat-underlag-2026",
      "name": "D15 Onsdagstur med blandat underlag",
      "date": "2026-05-06",
      "dateEnd": "2026-05-06",
      "location": "Ringsjövägen 40, Hörby",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Onsdagstur med D15 på blandade vägar. Samling enligt schema. Se bmwklubben.se för tider och plats.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-onsdagstur-med-blandat-underlag-2/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Onsdagstur med blandat underlag arrangeras av BMW MC-klubben.\n\nOnsdagstur med D15 på blandade vägar. Samling enligt schema. Se bmwklubben.se för tider och plats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ringsjövägen 40, Hörby.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-onsdagstur-med-blandat-underlag-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ringsj%C3%B6v%C3%A4gen+40%2C+H%C3%B6rby",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-05-07",
      "dateEnd": "2026-05-07",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-07/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-05-07",
      "dateEnd": "2026-05-07",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-07/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-05-07",
      "dateEnd": "2026-05-07",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-07/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-05-07",
      "dateEnd": "2026-05-07",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-07/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-classic-traff-vid-flygvapenmuseet-2026",
      "name": "Classic-träff vid Flygvapenmuseet.",
      "date": "2026-05-07",
      "dateEnd": "2026-05-07",
      "location": "Carl Cederströms gata 2, Linköping",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Träffpunkt i Carl Cederströms gata 2, 586 63 Linköping, Sverige. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/classic-traff-for-alla/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "Classic-träff vid Flygvapenmuseet. arrangeras av BMW MC-klubben.\n\nTräffpunkt i Carl Cederströms gata 2, 586 63 Linköping, Sverige. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Carl Cederströms gata 2, Linköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/classic-traff-for-alla/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Carl+Cederstr%C3%B6ms+gata+2%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-kvallstraff-borlange-2026",
      "name": "Kvällsträff med begagnatförsäljning",
      "date": "2026-05-07",
      "dateEnd": "2026-05-07",
      "location": "MCK Tourings klubbstuga, Islingby, Borlänge",
      "type": "Träff",
      "organizer": "SMC Dalarna",
      "description": "Kvällsträff med SMC Dalarna i MCK Tourings klubbstuga. Tipspromenad, hamburgare och korv. Köp och sälj begagnad MC-utrustning. Grillen varm från 18:00.",
      "descriptionFull": "Välkommen på en kvällsträff med SMC Dalarna i MCK Tourings klubbstuga.\n\nVi träffas, umgås, går en tipspromenad med priser, hamburgare och korv finns till försäljning. Samtidigt finns möjligheten att sälja och köpa begagnad MC-utrustning.\n\nVi startar 17:30 och grillen är varm från 18:00.\n\nSMC Dalarna kommer att berätta vad som är på gång under säsongen.",
      "backImage": "ads/kvallstraff-mck-touring-back-2026-05-07.jpg",
      "links": [
        {
          "label": "thetwinclub.se",
          "url": "https://www.thetwinclub.se/event/kvallstraff-med-begagnat-forsaljning-hos-mck-touring/"
        },
        {
          "label": "svmc.se/dalarna",
          "url": "https://www.svmc.se/dalarna/aktiviteter/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=MCK+Tourings+klubbstuga+Islingby+Borlänge",
          "type": "map"
        }
      ],
      "link": "https://www.thetwinclub.se/event/kvallstraff-med-begagnat-forsaljning-hos-mck-touring/",
      "region": "Dalarna",
      "source": "smcboken.svmc.se"
    },
    {
      "id": "roadracing-sm-falkenberg-2026",
      "name": "Roadracing SM - Falkenberg",
      "date": "2026-05-08",
      "dateEnd": "2026-05-10",
      "location": "Falkenberg",
      "type": "Racing",
      "organizer": "MotoSE / Svemo",
      "description": "Svensk roadracing SM. MotoSE blir navet för svensk roadracing med ny struktur och ambition.",
      "link": "https://mcparken.se/calendar/179",
      "region": "Halland",
      "source": "mcparken.se",
      "descriptionFull": "Roadracing SM - Falkenberg arrangeras av MotoSE / Svemo.\n\nSvensk roadracing SM. MotoSE blir navet för svensk roadracing med ny struktur och ambition.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Falkenberg.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/179"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Falkenberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-goes-to-spangens-gastgiveri-2026-2026",
      "name": "D11 Goes to Spångens Gästgiveri 2026",
      "date": "2026-05-08",
      "dateEnd": "2026-05-10",
      "location": "Spångengatan 344, Ljungbyhed",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Välkommen på en helgtur till Spångens Gästgiveri (Skåne) - FULLBOKAD!D11 arrangerar en trevlig touringtur till klassiska Spångens Gästgiveri i Skåne. Missa inte detta! :-).",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-goes-to-spangens-gastgiveri/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Goes to Spångens Gästgiveri 2026 arrangeras av BMW MC-klubben.\n\nVälkommen på en helgtur till Spångens Gästgiveri (Skåne) - FULLBOKAD!D11 arrangerar en trevlig touringtur till klassiska Spångens Gästgiveri i Skåne. Missa inte detta! :-).\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Spångengatan 344, Ljungbyhed.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-goes-to-spangens-gastgiveri/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sp%C3%A5ngengatan+344%2C+Ljungbyhed",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-tiveden-spring-tour-2026-2026",
      "name": "D10 Tiveden Spring Tour 2026",
      "date": "2026-05-08",
      "dateEnd": "2026-05-10",
      "location": "Askersund",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D10 från Askersund, Örebro län, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-tiveden-spring-tour-2026/",
      "region": "Örebro",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Tiveden Spring Tour 2026 arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D10 från Askersund, Örebro län, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Askersund.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-tiveden-spring-tour-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Askersund",
          "type": "map"
        }
      ]
    },
    {
      "id": "mc-collection-bok-2026",
      "name": "MC Collection - Boklansering Ralf Arvidsson",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Tidö Slott, Västerås",
      "type": "Träff",
      "organizer": "MC Collection",
      "description": "Invigning av MOTOCROSS & RACING-utställning. Ralf Arvidsson presenterar sin nya bok Svenska Fyrtakts-Motorcyklar.",
      "link": "https://mc-collection.com/kalender/",
      "region": "Västmanland",
      "source": "mc-collection.com",
      "descriptionFull": "MC Collection - Boklansering Ralf Arvidsson arrangeras av MC Collection.\n\nInvigning av MOTOCROSS & RACING-utställning. Ralf Arvidsson presenterar sin nya bok Svenska Fyrtakts-Motorcyklar.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Tidö Slott, Västerås.",
      "links": [
        {
          "label": "mc-collection.com",
          "url": "https://mc-collection.com/kalender/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tid%C3%B6+Slott%2C+V%C3%A4ster%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-poker-run-2026",
      "name": "DOA - Poker Run",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Stockholm. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-poker-run/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Poker Run arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Stockholm. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-poker-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-gokart-hovmantorp-9-maj-2026-2026",
      "name": "DOE Gokart Hovmantorp 9 maj 2026",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Hovmantorp",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "DOE Gokart 9 maj Hovmantorp 2026 Var: Växjö Hyrkart -Hagaslätt Adreass: Hagaslättsvägen 360 51 Hovmantorp https://maps.app.goo.",
      "link": "https://hdcs.se/evenemang/doe-gokart-hovmantorp-9-maj/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Gokart Hovmantorp 9 maj 2026 arrangeras av H-DCS.\n\nDOE Gokart 9 maj Hovmantorp 2026 Var: Växjö Hyrkart -Hagaslätt Adreass: Hagaslättsvägen 360 51 Hovmantorp https://maps.app.goo.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Hovmantorp.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-gokart-hovmantorp-9-maj/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hovmantorp",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-mc-kortege-hedemora-2026",
      "name": "MC kortege Hedemora marknad",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Lernia (fd AMU center), Hedemora",
      "type": "Körning",
      "organizer": "SMC Dalarna",
      "description": "MC-kortege genom Hedemora under marknaden. Samling kl 09:30 vid Lernia, först en runda runt TT-banan, sedan genom stadens gator. Parkering vid Vasahallen.",
      "descriptionFull": "MC kortege genom Hedemora under Hedemora marknad.\n\nSamling kl 09:30 vid Lernia (fd AMU center), Hedemora.\n\nFörst kör vi en runda runt TT-banan. Sedan kl 11:00 från Lappens Grill kör vi kortege genom Hedemoras gator.\n\nParkering vid Vasahallen i minst en timme så folk kan titta på hojarna.\n\nArrangör: SMC Dalarna",
      "links": [
        {
          "label": "svmc.se/dalarna",
          "url": "https://www.svmc.se/dalarna/aktiviteter/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lernia+Hedemora",
          "type": "map"
        }
      ],
      "link": "https://www.svmc.se/dalarna/aktiviteter/",
      "region": "Dalarna",
      "source": "svmc.se"
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-09/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-09/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "ride-out-medlemsmote-2026",
      "name": "Ride-Out + Medlemsmöte",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Mora",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Ride-Out + Medlemsmöte 9 maj på Orsa Camping kl 13,00 Samlingsplats MC Dolnad Mora kl.",
      "link": "https://hdcs.se/evenemang/ride-out-medlemsmote/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "Ride-Out + Medlemsmöte arrangeras av H-DCS.\n\nRide-Out + Medlemsmöte 9 maj på Orsa Camping kl 13,00 Samlingsplats MC Dolnad Mora kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Mora.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/ride-out-medlemsmote/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Mora",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-forst-körningen-2026",
      "name": "D14-Först körningen",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Halmstad",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "BMW D14 bjuder in till årets premiärtur. En lugn och trevlig start på säsongen med café och social samvaro.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-forst-körningen/",
      "region": "Halland",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-Först körningen arrangeras av BMW MC-klubben.\n\nBMW D14 bjuder in till årets premiärtur. En lugn och trevlig start på säsongen med café och social samvaro.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Halmstad.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-forst-körningen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halmstad",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-gfa-grus-for-alla-2026",
      "name": "D7 GFA - Grus för Alla",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D7. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-gfa-grus-for-alla/",
      "region": "Stockholm",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 GFA - Grus för Alla arrangeras av BMW MC-klubben.\n\nGrustour med D7. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-gfa-grus-for-alla/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-hjalmaren-runt-fran-orebro-en-svensk-mc-klassiker-2026",
      "name": "Hjälmaren Runt från Örebro - En svensk mc klassiker",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Örebro",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "En del av En svensk mc klassiker. Idag runt Sveriges fjärde största sjö med start i örebro.",
      "link": "https://www.bmwklubben.se/aktiviteter/hjalmaren-runt-fran-orebro-en-svensk-mc-klassiker/",
      "region": "Örebro",
      "source": "bmwklubben.se",
      "descriptionFull": "Hjälmaren Runt från Örebro - En svensk mc klassiker arrangeras av BMW MC-klubben.\n\nEn del av En svensk mc klassiker. Idag runt Sveriges fjärde största sjö med start i örebro.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Örebro.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/hjalmaren-runt-fran-orebro-en-svensk-mc-klassiker/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96rebro",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-hjalmaren-runt-fran-eskilstuna-en-svensk-mc-klassi-2026",
      "name": "Hjälmaren Runt från Eskilstuna - En svensk MC klassiker",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Eskilstuna",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Eskilstuna. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/hjalmaren-runt-fran-eskilstuna-en-svensk-mc-klassiker/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "Hjälmaren Runt från Eskilstuna - En svensk MC klassiker arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Eskilstuna. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Eskilstuna.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/hjalmaren-runt-fran-eskilstuna-en-svensk-mc-klassiker/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Eskilstuna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-grustur-2026",
      "name": "D15 Grustur",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Oderljungavägen, Klippan",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D15. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-grustur/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Grustur arrangeras av BMW MC-klubben.\n\nGrustour med D15. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Oderljungavägen, Klippan.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-grustur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Oderljungav%C3%A4gen%2C+Klippan",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-mcq-med-eva-till-sagolika-vimmerby-2026",
      "name": "D11 mcQ - Med Eva till sagolika Vimmerby",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Oskarshamn",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Eva tar med oss på en tur i trakterna av Astrid Lindgrens Vimmerby. Slingriga vägar genom vackra små byar utlovas liksom en guidad stadsvandring till fots på Vimmerbys.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-mcq-evas-tur/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 mcQ - Med Eva till sagolika Vimmerby arrangeras av BMW MC-klubben.\n\nEva tar med oss på en tur i trakterna av Astrid Lindgrens Vimmerby. Slingriga vägar genom vackra små byar utlovas liksom en guidad stadsvandring till fots på Vimmerbys.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Oskarshamn.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-mcq-evas-tur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Oskarshamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-nationellt-årsmöte-2026",
      "name": "SMC Nationellt Årsmöte 2026",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Sundsvall",
      "type": "Träff",
      "organizer": "SMC",
      "description": "Sveriges MotorCyklisters nationella årsmöte. Lördagen den 9 maj kl 13.00 i Sundsvall. Exakt lokal meddelas i MC-Folket Nr 2, på hemsidan och sociala medier.",
      "link": "https://www.svmc.se",
      "region": "Västernorrland",
      "source": "MC-Folket Nr 1 2026",
      "descriptionFull": "SMC Nationellt Årsmöte 2026 arrangeras av SMC.\n\nSveriges MotorCyklisters nationella årsmöte. Lördagen den 9 maj kl 13.00 i Sundsvall. Exakt lokal meddelas i MC-Folket Nr 2, på hemsidan och sociala medier.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sundsvall.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "falkenbergs-majsvangen-2026",
      "name": "MajSvängen 2026",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Abilds Bygdegård, Abild",
      "type": "Körning",
      "organizer": "Falkenbergs MC",
      "description": "Välgörenhetsrunda med Falkenbergs MC. Samling kl 12:00, fika och körning. 70% av intäkterna går till Fontänhusen i Falkenberg och Varberg.",
      "descriptionFull": "MajSvängen 2026 arrangeras av Falkenbergs MC.\n\nVarmt välkomna att köra en fin runda på hoj tillsammans med Falkenbergs MC sällskap. Vi samlas vid Abilds Bygdegård kl 12:00 där börjar vi med en fika tillsammans och vi kommer att avsluta på samma plats.\n\nKostnad 100 kr/hjälm, lr högre belopp om man vill. Kontant lr swish. 70% av intäkten har vi valt att donera till Fontänhusen i Falkenberg och Varberg. Fontänhuset är en mötesplats för människor som lider utav psykisk ohälsa. För oss är detta en lokal och mkt viktig verksamhet som verkligen gör skillnad.\n\nDet blir ett stopp längs vägen där vi äter korv med bröd/dryck till valfritt pris.\n\nPå kvällen kl 20:00 öppnar våra vänner Anyway Riders upp portarna nere hos sig på Kilavägen 40. Då blir det Spring Party med 3 liveband, detta vill man inte missa. Kommer man långväga så finns campingmöjligheter, vill man veta mer kring det så släng iväg ett meddelande.\n\nOBS! Skulle vädret vara emot oss med mkt regn så kör vi inte utan flyttar då fram körningen till annat datum. Festen kvarstår dock hos Anyway Riders på kvällen.\n\nPlats: Abilds Bygdegård, Abild.",
      "backImage": "ads/falkenbergs-majsvangen-back-2026-05-09.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/2394980357594451"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Abilds+Bygdeg%C3%A5rd+Abild",
          "type": "map"
        }
      ],
      "link": "",
      "region": "Halland",
      "source": "FB"
    },
    {
      "id": "anyway-riders-spring-party-2026",
      "name": "Anyway Riders Spring Party",
      "date": "2026-05-09",
      "dateEnd": "2026-05-09",
      "location": "Kilavägen 40, Falkenberg",
      "type": "Fest",
      "organizer": "Anyway Riders",
      "description": "Anyway Riders bjuder in till Spring Party! Tre liveband: Anna and the Spirits, TAJT och HOODOO SILVER. Inträde 100 kr, gärna cash.",
      "descriptionFull": "Anyway Riders i Falkenberg öppnar portarna för Spring Party!\n\nAlla är välkomna den 9/5 ca kl 20. På scen: Anna and the Spirits, TAJT och HOODOO SILVER.\n\nInträde 100 kr, gärna cash. Välfylld bar och sköna människor.\n\nKommer man långväga finns campingmöjligheter. Hör av er till Anyway Riders för mer info.\n\nTips: Tidigare på dagen kör Falkenbergs MC sin MajSvängen med samling kl 12:00 vid Abilds Bygdegård. Perfekt kombination!\n\nPlats: Kilavägen 40, 311 94 Falkenberg.",
      "backImage": "ads/anyway-riders-spring-party-back-2026-05-09.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1643770660297347/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Anyway+Riders+Falkenberg",
          "type": "map"
        }
      ],
      "link": "",
      "region": "Halland",
      "source": "FB"
    },
    {
      "id": "doa-emilsturen-2026",
      "name": "DOA - Emilsturen",
      "date": "2026-05-10",
      "dateEnd": "2026-05-10",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-emilsturen/2026-05-10/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Emilsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-emilsturen/2026-05-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-fdo-s-runda-till-danmark-2026",
      "name": "DOB- FDO:s runda till Danmark",
      "date": "2026-05-10",
      "dateEnd": "2026-05-10",
      "location": "Danmark",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOB från Danmark. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-fdos-runda-till-danmark/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- FDO:s runda till Danmark arrangeras av H-DCS.\n\nArrangemang med H-DCS DOB från Danmark. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Danmark.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-fdos-runda-till-danmark/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "fikatraff-surahammar-2026",
      "name": "Fikaträff Surahammar",
      "date": "2026-05-10",
      "dateEnd": "2026-05-10",
      "location": "Surahammar",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Fikaträff Surahammar Vi träffas vid MC-museet Gyllene Hjulet i Surahammar söndag 10 maj kl.10.00 Museets adress är Kontorsvägen 2 73531 Surahammar.",
      "link": "https://hdcs.se/evenemang/fikatraff-surahammar/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "Fikaträff Surahammar arrangeras av H-DCS.\n\nFikaträff Surahammar Vi träffas vid MC-museet Gyllene Hjulet i Surahammar söndag 10 maj kl.10.00 Museets adress är Kontorsvägen 2 73531 Surahammar.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Surahammar.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/fikatraff-surahammar/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Surahammar",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-roadracing-falkenberg-2026",
      "name": "D8 Roadracing Falkenberg",
      "date": "2026-05-10",
      "dateEnd": "2026-05-10",
      "location": "Falkenberg",
      "type": "Racing",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D8 från Falkenberg. Gemensam körning på vägar. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-roadracing-falkenberg/",
      "region": "Halland",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Roadracing Falkenberg arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D8 från Falkenberg. Gemensam körning på vägar. Mer info på bmwklubben.se.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Falkenberg.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-roadracing-falkenberg/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Falkenberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-avrostning-kul-pa-hjul-gillingebanan-2026",
      "name": "D7 Avrostning/Kul på Hjul, Gillingebanan",
      "date": "2026-05-10",
      "dateEnd": "2026-05-10",
      "location": "Gillingebanan, Vallentuna",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "En dag fylld av gas, broms, balans, glädje och lite grus på/intill Gillingebanan!",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-avrostning-kul-pa-hjul-gillingebanan/",
      "region": "Stockholm",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 Avrostning/Kul på Hjul, Gillingebanan arrangeras av BMW MC-klubben.\n\nEn dag fylld av gas, broms, balans, glädje och lite grus på/intill Gillingebanan!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Gillingebanan, Vallentuna.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-avrostning-kul-pa-hjul-gillingebanan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gillingebanan%2C+Vallentuna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-nossans-broar-2026",
      "name": "D8 Nossans broar",
      "date": "2026-05-10",
      "dateEnd": "2026-05-10",
      "location": "Nossebro",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D8 från Nossebro. Vandringsrund och motorcykelkörning. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-nissans-broar/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Nossans broar arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D8 från Nossebro. Vandringsrund och motorcykelkörning. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nossebro.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-nissans-broar/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nossebro",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-05-11",
      "dateEnd": "2026-05-11",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-11/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-05-12",
      "dateEnd": "2026-05-12",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-12/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-05-12",
      "dateEnd": "2026-05-12",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-12/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, jämna veckor",
      "date": "2026-05-12",
      "dateEnd": "2026-05-12",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-05-12/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-05-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-05-13",
      "dateEnd": "2026-05-13",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-13/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-05-13",
      "dateEnd": "2026-05-13",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-13/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-torgnys-vardagstur-2026",
      "name": "D11 Torgnys Vardagstur",
      "date": "2026-05-13",
      "dateEnd": "2026-05-13",
      "location": "Småland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D11 från Småland. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-torgnys-vardagstur/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Torgnys Vardagstur arrangeras av BMW MC-klubben.\n\nKörning med D11 från Småland. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Småland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-torgnys-vardagstur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-grillkvall-aboda-klint-2026",
      "name": "Grillkväll på Aboda Klint",
      "date": "2026-05-13",
      "dateEnd": "2026-05-13",
      "location": "Aboda, Klint",
      "type": "Träff",
      "organizer": "SMC Kalmar",
      "description": "Arrangemang med SMC i Aboda Klint. Mötespunkt för MC-klubben. Mer information på smc.nu.",
      "link": "https://smc-kalmar.com/",
      "region": "Kalmar",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Grillkväll på Aboda Klint arrangeras av SMC Kalmar.\n\nArrangemang med SMC i Aboda Klint. Mötespunkt för MC-klubben. Mer information på smc.nu.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Aboda, Klint.",
      "links": [
        {
          "label": "smc-kalmar.com",
          "url": "https://smc-kalmar.com/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Aboda%2C+Klint",
          "type": "map"
        }
      ]
    },
    {
      "id": "kcs-trackdays-gellerasen-2026",
      "name": "KCS Track Days Gelleråsen 2026",
      "date": "2026-05-14",
      "dateEnd": "2026-05-14",
      "location": "Gelleråsen, Karlskoga",
      "type": "Racing",
      "organizer": "KCS",
      "description": "Bandagar på Gelleråsen. Pris 2000kr för KTM, Husqvarna och GasGas. Övriga märken välkomna.",
      "link": "https://mcparken.se/calendar/172",
      "region": "Örebro",
      "source": "mcparken.se",
      "descriptionFull": "KCS Track Days Gelleråsen 2026 arrangeras av KCS.\n\nBandagar på Gelleråsen. Pris 2000kr för KTM, Husqvarna och GasGas. Övriga märken välkomna.\n\nBankdag för motorcyklister. Kontrollera arrangören för anmälan och regler.\n\nPlats: Gelleråsen, Karlskoga.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/172"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Geller%C3%A5sen%2C+Karlskoga",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-05-14",
      "dateEnd": "2026-05-14",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "coast-to-coast-2026",
      "name": "Coast to Coast",
      "date": "2026-05-14",
      "dateEnd": "2026-05-14",
      "location": "Kallis Visby, Visby",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Coast to Coast, start vis Kallis i Visby hamn, vi kör en sväng och stannar och fikar på egen bekostnad.",
      "link": "https://hdcs.se/evenemang/coast-to-coast/",
      "region": "Gotland",
      "source": "hdcs.se",
      "descriptionFull": "Coast to Coast arrangeras av H-DCS.\n\nCoast to Coast, start vis Kallis i Visby hamn, vi kör en sväng och stannar och fikar på egen bekostnad.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kallis Visby, Visby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/coast-to-coast/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kallis+Visby%2C+Visby",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-blommans-amp-lybergs-saloon-runda-i-eringsboda-2026",
      "name": "DOE Blommans & Lybergs Saloon Runda i Eringsboda 14 - 15 maj 2026",
      "date": "2026-05-14",
      "dateEnd": "2026-05-15",
      "location": "Eringsboda",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "DOE Blommans & Lybergs Saloon Runda i Eringsboda 14 - 15 maj 2026 Det här får du inte missa!! Vi samlas torsdag den 14 maj kl 11.",
      "link": "https://hdcs.se/evenemang/doe-blomman-lybergs-runda-med-pub-grillkvall-i-eringsboda-14-15-maj/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Blommans & Lybergs Saloon Runda i Eringsboda 14 - 15 maj 2026 arrangeras av H-DCS.\n\nDOE Blommans & Lybergs Saloon Runda i Eringsboda 14 - 15 maj 2026 Det här får du inte missa!! Vi samlas torsdag den 14 maj kl 11.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Eringsboda.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-blomman-lybergs-runda-med-pub-grillkvall-i-eringsboda-14-15-maj/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Eringsboda",
          "type": "map"
        }
      ]
    },
    {
      "id": "mc-ontours-gravel-jamtland-2026",
      "name": "Jämtland-Härjedalen Gravel Adventure",
      "date": "2026-05-14",
      "dateEnd": "2026-05-17",
      "location": "Wikners i Persåsen, Oviken",
      "type": "Körning",
      "organizer": "MC Ontours",
      "description": "4-dagars MC-äventyr på grusvägar i Jämtland-Härjedalen. Alla nivåer välkomna. Bra mat och boende på Wikners i Persåsen. 3 900 kr inkl mat, logi från 1 950 kr.",
      "descriptionFull": "Jämtland-Härjedalen Gravel Adventure, 14-17 maj.\n\nFyra dagar för alla som älskar att köra MC på grus och som vill bo bra och äta gott. Utvecklande och anpassade utmaningar för alla nivåer, från nybörjare till erfarna förare.\n\nAlla typer av MC-modeller är välkomna, grövre däck rekommenderas.\n\nExtra dag: Chans att starta redan torsdag kl 09:00 och köra vid slalombacken med två instruktörer från CountryTrax Offroad Academy. Möjlighet att boka extra natt onsdag.\n\nEventavgift: 3 900 kr inkl mat (3 middagar, 3 lunchpaket, 3 frukostar). Varav 1 000 kr i anmälningsavgift.\n\nLogi tillkommer:\nDubbelrum/stuga, torsdag-söndag (3 nätter): 1 950 kr\nEnkelrum: 2 900 kr\n\nPlats: Wikners i Persåsen, Persåsen 336, Oviken.",
      "backImage": "ads/mc-ontours-gravel-back-2026-05-14.jpg",
      "links": [
        {
          "label": "MC Ontours (FB)",
          "url": "https://www.facebook.com/mcontours.se"
        },
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/775427375162661"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Persåsen+skid-+och+kälkklubb",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/775427375162661",
      "region": "Jämtland",
      "source": "FB"
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-05-14",
      "dateEnd": "2026-05-14",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-14/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-05-14",
      "dateEnd": "2026-05-14",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-05-14",
      "dateEnd": "2026-05-14",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-05-14/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-05-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-05-14",
      "dateEnd": "2026-05-14",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-14/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-regiontraff-vast-2026-2026",
      "name": "Regionträff Väst 2026",
      "date": "2026-05-14",
      "dateEnd": "2026-05-17",
      "location": "Lindängsvägen 11, Orsa",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Träffpunkt i Lindängsvägen 11, 794 92 Orsa, Sverige. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/regiontraff/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "Regionträff Väst 2026 arrangeras av BMW MC-klubben.\n\nTräffpunkt i Lindängsvägen 11, 794 92 Orsa, Sverige. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Lindängsvägen 11, Orsa.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/regiontraff/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lind%C3%A4ngsv%C3%A4gen+11%2C+Orsa",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-minitraff-herrfallet-2026",
      "name": "GWCS Minitraff Herrfallet",
      "date": "2026-05-14",
      "dateEnd": "2026-05-17",
      "location": "Herrfallets Camping, Arboga",
      "type": "Träff",
      "organizer": "Sormlandswingarna / GWCS",
      "description": "MC-arrangemang i Herrfallets Camping, Arboga. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://media.gwcs.se/2026/01/Herr-2.pdf",
      "region": "Västmanland",
      "source": "gwcs.se",
      "descriptionFull": "GWCS Minitraff Herrfallet arrangeras av Sormlandswingarna / GWCS.\n\nMC-arrangemang i Herrfallets Camping, Arboga. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Herrfallets Camping, Arboga.",
      "links": [
        {
          "label": "media.gwcs.se",
          "url": "https://media.gwcs.se/2026/01/Herr-2.pdf"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Herrfallets+Camping%2C+Arboga",
          "type": "map"
        }
      ]
    },
    {
      "id": "körning-h-d-butiken-i-linkoping-med-overnattning-2026",
      "name": "Körning H-D butiken i Linköping med övernattning",
      "date": "2026-05-15",
      "dateEnd": "2026-05-16",
      "location": "Linköping",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Linköping. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/körning-h-d-butiken-i-linkoping-med-overnattning/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "Körning H-D butiken i Linköping med övernattning arrangeras av H-DCS.\n\nKörtur med H-DCS från Linköping. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Linköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/körning-h-d-butiken-i-linkoping-med-overnattning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "mctouring-oppet-hus-gagnef-2026",
      "name": "Rönnegård Bädd & Frukost",
      "date": "2026-05-16",
      "dateEnd": "2026-05-17",
      "location": "Gagnef",
      "type": "Träff",
      "organizer": "mctouring.se",
      "description": "Snacka MC-touring över kaffe, kolla touringboden och leverantörer. Grillkväll 175 kr med resebilder, i år med fokus på Japan. Övernattning möjlig.",
      "descriptionFull": "Lördagen den 16 maj håller mctouring.se Öppet hus på Tallbacken i Gagnef. Kom förbi och snacka MC-touring över en kopp kaffe med tilltugg.\nVi håller även MC-touringboden öppen med både gammalt och nytt! Leverantörer på plats med produkter för MC-touring på både asfalt och grus.\nPå kvällen kör vi en grillkväll (175 kr/person) med bilder och film från olika resor, i år med fokus på Japan.\nÖvernattning i delad fyrbäddsstuga 375 kr/person. Eget tält med servicehus och frukost 225 kr/person.\nLördag 10.00 - 23.00 | Söndag 08.00 - 10.00\nBoende bokas via e-post: hans@mctouring.se\nVälkommen till en härlig dag/helg med stort fokus på MC-touring!\nHälsningar era värdar // Gun och Hans Rönnegård",
      "backImage": "ads/mctouring-oppet-hus-2026.jpg",
      "links": [
        {
          "label": "mctouring.se",
          "url": "https://mctouring.se"
        },
        {
          "label": "rönnegård.se",
          "url": "http://www.ronnegard.se/mc"
        }
      ],
      "link": "http://www.ronnegard.se/mc",
      "region": "Dalarna",
      "source": "FB"
    },
    {
      "id": "dob-aviatorernas-flygarrally-2026",
      "name": "DOB- Aviatörernas Flygarrally",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Skåne",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOB från Skåne. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-aviatorernas-flygarrally/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Aviatörernas Flygarrally arrangeras av H-DCS.\n\nArrangemang med H-DCS DOB från Skåne. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-aviatorernas-flygarrally/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-poker-run-lordag-16-maj-2026-pa-flying-eagle-s-2026",
      "name": "DOE Poker Run Lördag 16 maj 2026 på Flying Eagle´s Stora Vårfest i Växjö",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Växjö",
      "type": "Fest",
      "organizer": "H-DCS",
      "description": "Poker Run Lördag 16 maj 2026 på Flying Eagle´s Stora Vårfest i Växjö Samla ihop era MC-vänner och kom hit!",
      "link": "https://hdcs.se/evenemang/doe-poker-run-vaxjo-16-maj/",
      "region": "Kronoberg",
      "source": "hdcs.se",
      "descriptionFull": "DOE Poker Run Lördag 16 maj 2026 på Flying Eagle´s Stora Vårfest i Växjö arrangeras av H-DCS.\n\nPoker Run Lördag 16 maj 2026 på Flying Eagle´s Stora Vårfest i Växjö Samla ihop era MC-vänner och kom hit!\n\nFest och social sammankomst för MC-folket. Se arrangören för detaljer.\n\nPlats: Växjö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-poker-run-vaxjo-16-maj/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4xj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-rundtur-runt-enkoping-2026",
      "name": "DOA - Rundtur runt Enköping",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Mels garage, Enköping",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Mika kommer att ta oss med på en rundtur på fina MC vägar runt Enköping inkl. några fina stopp, en tur på ca.20 mil.",
      "link": "https://hdcs.se/evenemang/doa-rundtur-runt-enkoping/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Rundtur runt Enköping arrangeras av H-DCS.\n\nMika kommer att ta oss med på en rundtur på fina MC vägar runt Enköping inkl. några fina stopp, en tur på ca.20 mil.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Mels garage, Enköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-rundtur-runt-enkoping/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Mels+garage%2C+Enk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-16/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-16/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-05-16/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-05-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-asnenturen-2026",
      "name": "D15 Åsnenturen",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Skåne",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D15 från Skåne. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-asnenturen/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Åsnenturen arrangeras av BMW MC-klubben.\n\nKörning med D15 från Skåne. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-asnenturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-mcq-gor-en-vartur-till-omberg-2026",
      "name": "mcQ gör en vårtur till Omberg",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Ryds allé Linköping",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Gemensam körning från Ryds allé, 584 33 Linköping, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/mcq-gor-en-vartur-till-omberg/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "mcQ gör en vårtur till Omberg arrangeras av BMW MC-klubben.\n\nGemensam körning från Ryds allé, 584 33 Linköping, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ryds allé Linköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/mcq-gor-en-vartur-till-omberg/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ryds+all%C3%A9+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-hemlig-resa-maj-2026",
      "name": "Hemlig resa",
      "date": "2026-05-16",
      "dateEnd": "2026-05-17",
      "location": "Hemligt",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Hemligt. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/hemlig-resa/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "Hemlig resa arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Hemligt. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Hemligt.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/hemlig-resa/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hemligt",
          "type": "map"
        }
      ]
    },
    {
      "id": "mc-kortegen-kristianstad-2026",
      "name": "MC Kortegen Kristianstad - Åhusparken",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Ishallen, Kristianstad",
      "type": "Körning",
      "organizer": "Kristianstad MC-grupp",
      "description": "Öppen kortege för insamling till Barn- och ungdomssjukhuset i Lund. Samling kl 11, avfärd kl 13. Deltagaravgift 100 kr/hjälm. Utlottning och mat i Åhusparken.",
      "descriptionFull": "MC Kortegen Kristianstad - Åhusparken är tillbaka!\n\nÖppen kortege för insamling till Barn- och ungdomssjukhuset i Lund och Barnkliniken CSK.\n\nSamling 11:00-12:45 vid Kristianstad Ishallen med avfärd kl 13:00.\n\nDeltagaravgift 100 kr/hjälm som går till BUS (Barn- och ungdomssjukhuset) i Lund. Betalas vid ankomsten med kontanter eller Swish.\n\nKiK kommer hålla sitt cafe öppet för fika. Toalett tillgänglig.\n\nAlla deltagare är med i en utlottning med fina priser. Extra lotter går att köpa för 50 kr styck.\n\nI Åhusparken finns mat till bra priser.\n\nI samarbete med föreningen Nya Åhusparken.\n\nVälkomna! Tillsammans gör vi skillnad.",
      "backImage": "ads/mc-kortegen-kristianstad-back-2026-05-16.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1401669478335139"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ishallen+Kristianstad",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1401669478335139",
      "region": "Skåne",
      "source": "FB"
    },
    {
      "id": "dannike-motorstop-2026",
      "name": "Dannike Motorstopp 2026",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Dannike Hills Harley Davidson, Dannike",
      "type": "Fest",
      "organizer": "Dannike Hills Harley Davidson",
      "description": "Starta motorsäsongen i Dannike! MC- och bilutställning, pris till finaste MC och bil, tombola, livemusik, hamburgare och fika.",
      "descriptionFull": "Dannike Hills Harley Davidson bjuder in till Dannike Motorstopp 2026!\n\nStarta motorsäsongen tillsammans med oss i Dannike. En dag fylld med motorer, musik, god mat och härlig stämning. Ta med hojen, bilen, kompisarna eller bara kom för att njuta av en riktigt skön motordag.\n\nLördag 16 maj, 10:00-16:00.\n\nUnder dagen:\n- MC- och bilutställning\n- Pris till finaste MC och finaste bil\n- Tombola och stort lotteri (dragning kl 14:00)\n- Chokladhjul\n- Grillade hamburgare, kaffe och fika\n- Live musik\n- Utställare\n\nPå kvällen fortsätter festen i klubblokalen med liveband!\n\nFri entré, alla är välkomna, med eller utan fordon!\n\nPlats: Dannike Hills Harley Davidson, Länghemsvägen, Dannike.",
      "backImage": "ads/dannike-motorstop-back-2026-05-16.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/2077864559663074/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Dannike+Hills+Harley+Davidson+DHHD",
          "type": "map"
        }
      ],
      "link": "",
      "region": "Västra Götaland",
      "source": "FB"
    },
    {
      "id": "mxsm-linkoping-2026",
      "name": "MXSM Deltävling 2 - Linköping",
      "date": "2026-05-16",
      "dateEnd": "2026-05-17",
      "location": "Linköpings MS, Linköping",
      "type": "Racing",
      "organizer": "SVEMO / Linköpings MS",
      "description": "Deltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Östergötland",
      "source": "svemo.se",
      "descriptionFull": "MXSM Deltävling 2 - Linköping arrangeras av SVEMO / Linköpings MS.\n\nDeltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Linköpings MS, Linköping.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Link%C3%B6pings+MS%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "hoktraffen-med-rally-ronneby-2026",
      "name": "Hökträffen med rally 2026",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Hökarna MC, Ronneby",
      "type": "Träff",
      "organizer": "Hökarna MC",
      "description": "Hökarna MC firar 55-årsjubileum! Fredagsbarhäng, lördagsrally med start kl 11, live med Take Two och sängplatser inomhus.",
      "link": "https://hokarna.se/",
      "region": "Blekinge",
      "source": "FB",
      "descriptionFull": "Hökarna MC firar 55 års jubileum med Hökträffen 2026!\n\nNi är givetvis välkomna tidigare. På fredagen är det barhäng med begränsad service.\n\nLördag kör vi rally med start mellan kl 11.00-13.00. Väl i mål finns det mat och dryck till bra priser. Johan Stenström underhåller i väntan på prisutdelning.\n\nPå kvällen blir det fest i ladan med Take Two på scenen.\n\nDet finns sängplatser inomhus. Först till kvarn gäller.\n\nKontakt: www.hokarna.se\nDennis: 0708-924575\n\nPlats: Gråsparvsvägen 15, 372 38 Ronneby.",
      "backImage": "ads/hoktraffen-med-rally-back-2026-05-16.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1177061637902975/"
        },
        {
          "label": "hokarna.se",
          "url": "https://hokarna.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=H%C3%B6karna+MC+Ronneby",
          "type": "map"
        }
      ]
    },
    {
      "id": "poker-run-orebro-2026",
      "name": "Poker Run 2026",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Osmundgatan 3, 703 83 Örebro",
      "type": "Körning",
      "organizer": "SMC Örebro",
      "description": "Poker Run genom Örebro. Start från Garaget/Genuine Bike Parts, målgång vid ATM MC. Arrangeras av SMC Örebro och Network MC.",
      "backImage": "ads/poker-run-orebro-back-2026-05-16.jpg",
      "descriptionFull": "Poker Run i Örebro den 16 maj 2026.\n\nÅrets Poker Run i Örebro arrangeras av SMC Örebro och Network MC. Starten går från Garaget/Genuine Bike Parts på Osmundgatan 3 kl 09:00-10:00. Målet är hos ATM MC, senast kl 15:00.\n\nStartavgift: 50 kr (betala gärna med Swish).\n\nMedverkande klubbar: MC Karlskoga, Genuine Bike Parts, ATM MC, Network MC, Nerikes Bikers MC, Templars Against, SMC Örebro, Garaget.\n\nPlats: Start Osmundgatan 3, Örebro.",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/2095757894602710/"
        },
        {
          "label": "svmc.se/orebro",
          "url": "https://www.svmc.se/orebro"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Osmundgatan+3+%C3%96rebro",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/2095757894602710/",
      "region": "Örebro",
      "source": "facebook.com"
    },
    {
      "id": "mhs-varmarknad-nifsta-gard-2026",
      "name": "MHS Vårmarknad",
      "date": "2026-05-16",
      "dateEnd": "2026-05-16",
      "location": "Nifsta Gård 1, 762 95 Rimbo",
      "type": "Träff",
      "organizer": "Nifsta Classic Cars",
      "description": "Klassisk veteranmarknad på Nifsta Gård i Rimbo. Veteranfordon, reservdelar, nostalgi och fika. Fri entré, MC parkerar gratis.",
      "backImage": "ads/mhs-varmarknad-nifsta-back-2026-05-16.jpg",
      "descriptionFull": "MHS Vårmarknad på Nifsta Gård den 16 maj 2026.\n\nTraditionell marknad för veteranfordon, renovering och hobbyn. Köp, sälj, snacka och nörda ner dig i allt som rullar. Bilar, motorcyklar, mopeder och tillbehör.\n\nFri entré. Parkering 100 kr/bil. MC, traktor och EPA parkerar gratis.\n\nÖppet kl 08:00-16:00. Mat, dryck och fika finns på plats.\n\n1 600 intresserade på Facebook.\n\nPlats: Nifsta Gård 1, 762 95 Rimbo.",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/1776707783025197/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nifsta+G%C3%A5rd+Rimbo",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1776707783025197/",
      "region": "Stockholm",
      "source": "facebook.com"
    },
    {
      "id": "do-c-tjorn-orust-rundan-2026",
      "name": "DO-C Tjörn/Orust rundan",
      "date": "2026-05-17",
      "dateEnd": "2026-05-17",
      "location": "Vikingagrillen, Lilla Kongahällavägen 11, Kungälv",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Vikingagrillen, Lilla Kongahällavägen 11, 442 90 Kungälv, Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-tjorn-orust-rundan/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Tjörn/Orust rundan arrangeras av H-DCS.\n\nArrangemang med H-DCS från Vikingagrillen, Lilla Kongahällavägen 11, 442 90 Kungälv, Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vikingagrillen, Lilla Kongahällavägen 11, Kungälv.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-tjorn-orust-rundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Vikingagrillen%2C+Lilla+Kongah%C3%A4llav%C3%A4gen+11%2C+Kung%C3%A4lv",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-mixtur-med-sandra-och-mikael-2026",
      "name": "D11 Mixtur med Sandra och Mikael",
      "date": "2026-05-17",
      "dateEnd": "2026-05-17",
      "location": "Alsteråvägen 1, Ålem",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Välkommen att följa med på en mixad asfalts- och grustur genom mellersta Kalmar län.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-mixtur-med-sandra-och-mikael/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Mixtur med Sandra och Mikael arrangeras av BMW MC-klubben.\n\nVälkommen att följa med på en mixad asfalts- och grustur genom mellersta Kalmar län.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Alsteråvägen 1, Ålem.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-mixtur-med-sandra-och-mikael/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Alster%C3%A5v%C3%A4gen+1%2C+%C3%85lem",
          "type": "map"
        }
      ]
    },
    {
      "id": "karlshamn-kustcruisers-treff-2026",
      "name": "Kustcruisers Sommartreff",
      "date": "2026-05-17",
      "dateEnd": "2026-05-17",
      "location": "Karlshamn",
      "type": "Träff",
      "organizer": "Blekinge Kustcruisers MC",
      "description": "Sommartreff med Blekinge Kustcruisers MC i Karlshamn. Fika och gemenskap.",
      "link": "",
      "region": "Blekinge",
      "source": "web",
      "_canary": true
    },
    {
      "id": "kemora-ring-speedskills-2026",
      "name": "Kemora Ring med SpeedSkills",
      "date": "2026-05-18",
      "dateEnd": "2026-05-21",
      "location": "Kemora Ring, Finland",
      "type": "Racing",
      "organizer": "SpeedSkills",
      "description": "Kemora vecka 21 - en tradition i över 30 år. Bankörning och speedträning i Finland.",
      "link": "https://mcparken.se/calendar/173",
      "region": "Finland",
      "source": "mcparken.se",
      "descriptionFull": "Kemora Ring med SpeedSkills arrangeras av SpeedSkills.\n\nKemora vecka 21 - en tradition i över 30 år. Bankörning och speedträning i Finland.\n\nBankdag för motorcyklister. Kontrollera arrangören för anmälan och regler.\n\nPlats: Kemora Ring, Finland.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/173"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kemora+Ring%2C+Finland",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-05-18",
      "dateEnd": "2026-05-18",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-18/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-05-19",
      "dateEnd": "2026-05-19",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-19/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-05-19",
      "dateEnd": "2026-05-19",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-19/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-05-19",
      "dateEnd": "2026-05-19",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-05-19/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-05-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-fikatraff-pa-jagmastargarden-med-classic-ganget-2026",
      "name": "Fikaträff på Jägmästargården med Classic-gänget",
      "date": "2026-05-19",
      "dateEnd": "2026-05-19",
      "location": "Lunds Västergård 1, Söderköping",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Kom och njut av gammelbumsar och en riktig go’fika på klassiska Jägmästargården!",
      "link": "https://www.bmwklubben.se/aktiviteter/fikatraff-pa-jagmastargarden-med-classic-ganget/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "Fikaträff på Jägmästargården med Classic-gänget arrangeras av BMW MC-klubben.\n\nKom och njut av gammelbumsar och en riktig go’fika på klassiska Jägmästargården!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Lunds Västergård 1, Söderköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/fikatraff-pa-jagmastargarden-med-classic-ganget/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lunds+V%C3%A4sterg%C3%A5rd+1%2C+S%C3%B6derk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "super-rally-2026-2026",
      "name": "Super Rally 2026",
      "date": "2026-05-20",
      "dateEnd": "2026-05-24",
      "location": "Fredericia, Danmark",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Harley Davidson Club of Denmark Truelstrupvejen 4 4130 Viby Sjælland Denmark info@hdc.dk +45 2560 7606 www.hdc.",
      "link": "https://hdcs.se/evenemang/super-rally-2026/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Super Rally 2026 arrangeras av H-DCS.\n\nHarley Davidson Club of Denmark Truelstrupvejen 4 4130 Viby Sjælland Denmark info@hdc.dk +45 2560 7606 www.hdc.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Fredericia, Danmark.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/super-rally-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Fredericia%2C+Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "super-rally-2026-fredericia-danmark-2026",
      "name": "Super Rally 2026 Fredericia Danmark",
      "date": "2026-05-20",
      "dateEnd": "2026-05-24",
      "location": "Danmark",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Danmark. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/super-rally-fredericia-danmark/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "Super Rally 2026 Fredericia Danmark arrangeras av H-DCS.\n\nArrangemang med H-DCS från Danmark. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Danmark.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/super-rally-fredericia-danmark/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-05-20",
      "dateEnd": "2026-05-20",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-20/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo, udda veckor",
      "date": "2026-05-20",
      "dateEnd": "2026-05-20",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-05-20/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo, udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-05-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-05-20",
      "dateEnd": "2026-05-20",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-20/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "grillkvall-longsjon-ankarsrum-2026",
      "name": "Grillkväll Långsjön",
      "date": "2026-05-20",
      "dateEnd": "2026-05-20",
      "location": "Rastplats väg 40, Långsjön, Ankarsrum",
      "type": "Träff",
      "organizer": "Törnros MC",
      "description": "MC-arrangemang i Rastplats väg 40, Långsjön, Ankarsrum. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://smcboken.svmc.se/traffar/mcträffar",
      "region": "Kalmar",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Grillkväll Långsjön arrangeras av Törnros MC.\n\nMC-arrangemang i Rastplats väg 40, Långsjön, Ankarsrum. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rastplats väg 40, Långsjön, Ankarsrum.",
      "links": [
        {
          "label": "smcboken.svmc.se",
          "url": "https://smcboken.svmc.se/traffar/mcträffar"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Rastplats+v%C3%A4g+40%2C+L%C3%A5ngsj%C3%B6n%2C+Ankarsrum",
          "type": "map"
        }
      ]
    },
    {
      "id": "tjejdag-kinnekulle-2026",
      "name": "Tjejdag Kinnekulle Trackdays",
      "date": "2026-05-21",
      "dateEnd": "2026-05-21",
      "location": "Kinnekulle Ring, Götene",
      "type": "Racing",
      "organizer": "Kinnekulle Trackdays",
      "description": "Bankörning enbart för tjejer på Kinnekulle Ring. Upplev fartens tjusning i trygg miljö.",
      "link": "https://mcparken.se/calendar/181",
      "region": "Västra Götaland",
      "source": "mcparken.se",
      "descriptionFull": "Tjejdag Kinnekulle Trackdays arrangeras av Kinnekulle Trackdays.\n\nBankörning enbart för tjejer på Kinnekulle Ring. Upplev fartens tjusning i trygg miljö.\n\nBankdag för motorcyklister. Kontrollera arrangören för anmälan och regler.\n\nPlats: Kinnekulle Ring, Götene.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/181"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kinnekulle+Ring%2C+G%C3%B6tene",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-05-21",
      "dateEnd": "2026-05-21",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-21/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-05-21",
      "dateEnd": "2026-05-21",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-21/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-05-21",
      "dateEnd": "2026-05-21",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-21/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-05-21",
      "dateEnd": "2026-05-21",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-21/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-finlandstur-22-24-maj-2026",
      "name": "DOA - Finlandstur 22-24 maj",
      "date": "2026-05-22",
      "dateEnd": "2026-05-24",
      "location": "Vikingterminalen, Stadsgårdskajen, Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi, de som vill är inbjudna till Finland, samma helg som Super Rallyt men för de som vill ha en kortare resa. Boka själv med Viking line.",
      "link": "https://hdcs.se/evenemang/doa-finlandstur-22-24-maj/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Finlandstur 22-24 maj arrangeras av H-DCS.\n\nVi, de som vill är inbjudna till Finland, samma helg som Super Rallyt men för de som vill ha en kortare resa. Boka själv med Viking line.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vikingterminalen, Stadsgårdskajen, Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-finlandstur-22-24-maj/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Vikingterminalen%2C+Stadsg%C3%A5rdskajen%2C+Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "ride-to-eat-hojresa-till-finland-2026",
      "name": "Ride to eat - Hojresa till Finland",
      "date": "2026-05-22",
      "dateEnd": "2026-05-24",
      "location": "Finland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Finland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/ride-to-eat-hojresa-till-finland/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "Ride to eat - Hojresa till Finland arrangeras av H-DCS.\n\nArrangemang med H-DCS från Finland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Finland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/ride-to-eat-hojresa-till-finland/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Finland",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-minitraff-svenljunga-2026",
      "name": "GWCS Minitraff Svenljunga Nydal",
      "date": "2026-05-22",
      "dateEnd": "2026-05-24",
      "location": "Nydal, Svenljunga",
      "type": "Träff",
      "organizer": "Knallewingarna / GWCS",
      "description": "MC-arrangemang i Nydal, Svenljunga. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://media.gwcs.se/2025/10/Nydal.jpeg",
      "region": "Västra Götaland",
      "source": "gwcs.se",
      "frontImage": "ads/gwcs-svenljunga-nydal-2026-05-22.jpg",
      "descriptionFull": "GWCS Minitraff Svenljunga Nydal arrangeras av Knallewingarna / GWCS.\n\nMC-arrangemang i Nydal, Svenljunga. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nydal, Svenljunga.",
      "links": [
        {
          "label": "media.gwcs.se",
          "url": "https://media.gwcs.se/2025/10/Nydal.jpeg"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nydal%2C+Svenljunga",
          "type": "map"
        }
      ]
    },
    {
      "id": "ride-for-ukraine-2026",
      "name": "Ride for Ukraine 2026",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Ukrainska ambassaden, Lidingö",
      "type": "Körning",
      "organizer": "Templar Knights MC",
      "description": "Välgörenhetskörning till stöd för Ukraina via UNHCR. Samling 09:30, start 10:00. Från Lidingö till Sollentuna. Swisha valfri summa, allt går oavkortat till UNHCR.",
      "link": "https://min.sverigeforunhcr.se/fundraisers/ride-for-ukraine2",
      "region": "Stockholm",
      "source": "MC-Folket / FB",
      "frontImage": "ads/ride-for-ukraine-2026-05-23.jpg",
      "descriptionFull": "Ride for Ukraine 2026 arrangeras av Templar Knights MC.\n\nVälgörenhetskörning till stöd för Ukraina via UNHCR. Samling 09:30, start 10:00. Från Lidingö till Sollentuna. Swisha valfri summa, allt går oavkortat till UNHCR.\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Ukrainska ambassaden, Lidingö.",
      "links": [
        {
          "label": "min.sverigeforunhcr.se",
          "url": "https://min.sverigeforunhcr.se/fundraisers/ride-for-ukraine2"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ukrainska+ambassaden%2C+Liding%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "oppet-hus-rapido-smc-2026",
      "name": "Öppet Hus Rapido MCK och SMC Stockholm",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Skansbacken 4, Gullmarsplan, Stockholm",
      "type": "Träff",
      "organizer": "Rapido MCK / SMC Stockholm",
      "description": "Lör 11-15. Provkör nya hojar, köp och sälj MC-prylar, mat på plats. SMC Stockholm svarar på frågor om medlemskap.",
      "link": "https://www.svmc.se/stockholm",
      "region": "Stockholm",
      "source": "svmc.se",
      "descriptionFull": "Öppet Hus Rapido MCK och SMC Stockholm arrangeras av Rapido MCK / SMC Stockholm.\n\nLör 11-15. Provkör nya hojar, köp och sälj MC-prylar, mat på plats. SMC Stockholm svarar på frågor om medlemskap.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skansbacken 4, Gullmarsplan, Stockholm.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se/stockholm"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skansbacken+4%2C+Gullmarsplan%2C+Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-motorbiten-falun-2026",
      "name": "Motorbiten i Falun",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Motorbiten i Falun",
      "type": "Träff",
      "organizer": "SMC Dalarna",
      "description": "Provkör nya modeller, kolla utrustning och fynda MC-prylar. Gratis kaffe, grillad korv och dryck. Förra året kom över 1000 besökare. Burgarbussen på plats.",
      "descriptionFull": "Motorbiten i Falun, lördagen den 23 maj kl 11:00-15:00 (kan bli till 16:00).\n\nProvkör nya modeller, kolla utrustning och fynda MC-prylar hos Motorbiten.\n\nGratis kaffe, grillad korv och dryck på plats. Förra året kom över 1000 besökare.\n\nBurgarbussen kommer också och serverar hamburgare.\n\nVälkommen!",
      "links": [
        {
          "label": "svmc.se/dalarna",
          "url": "https://www.svmc.se/dalarna/aktiviteter/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Motorbiten+Falun",
          "type": "map"
        }
      ],
      "link": "https://www.svmc.se/dalarna/aktiviteter/",
      "region": "Dalarna",
      "source": "svmc.se"
    },
    {
      "id": "strangnas-bikeshow-2026",
      "name": "Strängnäs Bike Show",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Strängnäs",
      "type": "Show",
      "organizer": "Blotsvens MC",
      "description": "Populär bike show sedan 90-talet. MC-förare från hela Mälardalen samlas för en dag av gemenskap.",
      "link": "https://www.alltommc.se/kategori/evenemang/strangnas-bike-show/",
      "region": "Södermanland",
      "source": "alltommc.se",
      "descriptionFull": "Strängnäs Bike Show arrangeras av Blotsvens MC.\n\nPopulär bike show sedan 90-talet. MC-förare från hela Mälardalen samlas för en dag av gemenskap.\n\nMC-show med utsmyckade hojar och byggen. Kolla arrangören för tider.\n\nPlats: Strängnäs.",
      "links": [
        {
          "label": "alltommc.se",
          "url": "https://www.alltommc.se/kategori/evenemang/strangnas-bike-show/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Str%C3%A4ngn%C3%A4s",
          "type": "map"
        }
      ]
    },
    {
      "id": "hdcs-nopla-meeting-2026",
      "name": "H-DCS Noplå Meeting",
      "date": "2026-05-23",
      "dateEnd": "2026-05-25",
      "location": "Noplå",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "H-DCS klassiska Noplå Meeting. En av klubbens största årliga träffar för Harley-ägare.",
      "link": "https://hdcs.se/evenemang/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "H-DCS Noplå Meeting arrangeras av H-DCS.\n\nH-DCS klassiska Noplå Meeting. En av klubbens största årliga träffar för Harley-ägare.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Noplå.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nopl%C3%A5",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-23/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-23/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "tur-till-frosakers-brygga-2026",
      "name": "Tur till Frösåkers brygga.",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Frösåkers Brygga",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "En tur till Frösåkers brygga med start vid Färjkajen Västerås. Det går att förbeställa mackor för den som vill. 070 573 43 43.",
      "link": "https://hdcs.se/evenemang/tur-till-frosakers-brygga/",
      "region": "Gotland",
      "source": "hdcs.se",
      "descriptionFull": "Tur till Frösåkers brygga. arrangeras av H-DCS.\n\nEn tur till Frösåkers brygga med start vid Färjkajen Västerås. Det går att förbeställa mackor för den som vill. 070 573 43 43.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Frösåkers Brygga.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/tur-till-frosakers-brygga/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Fr%C3%B6s%C3%A5kers+Brygga",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-tiveden-grus-2026",
      "name": "D8 Tiveden grus",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Tiveden",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D8. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-tiveden-grus/",
      "region": "Örebro",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Tiveden grus arrangeras av BMW MC-klubben.\n\nGrustour med D8. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Tiveden.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-tiveden-grus/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tiveden",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-malsjo-classic-cars-art-gallery-2026",
      "name": "Malsjö Classic Cars & Art Gallery",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Östra Åsenvägen, Åmål",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Vi kjører en runde i grensetraktene mellom Värmland og Nordvästra Götaland. Vi besøker Malsjö Classic utenfor Grums og titter på de mellom 50 og 100 eksotiske bilene i den.",
      "link": "https://www.bmwklubben.se/aktiviteter/malsjo-classic-cars-art-gallery/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "Malsjö Classic Cars & Art Gallery arrangeras av BMW MC-klubben.\n\nVi kjører en runde i grensetraktene mellom Värmland og Nordvästra Götaland. Vi besøker Malsjö Classic utenfor Grums og titter på de mellom 50 og 100 eksotiske bilene i den.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Östra Åsenvägen, Åmål.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/malsjo-classic-cars-art-gallery/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96stra+%C3%85senv%C3%A4gen%2C+%C3%85m%C3%A5l",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-miniature-kingdom-kungsor-2026",
      "name": "D7 Miniature Kingdom, Kungsör",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Gamla Strängnäsvägen 509, Nykvarn",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D7 från Gamla Strängnäsvägen 509, 155 91 Nykvarn, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-miniature-kingdom-kungsor/",
      "region": "Västmanland",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 Miniature Kingdom, Kungsör arrangeras av BMW MC-klubben.\n\nKörning med D7 från Gamla Strängnäsvägen 509, 155 91 Nykvarn, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Gamla Strängnäsvägen 509, Nykvarn.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-miniature-kingdom-kungsor/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gamla+Str%C3%A4ngn%C3%A4sv%C3%A4gen+509%2C+Nykvarn",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-tur-i-osterled-2026",
      "name": "D10 Tur i Österled",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Borås",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D10 från Borås, Västra Götalands län, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-tur-i-osterled/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Tur i Österled arrangeras av BMW MC-klubben.\n\nKörning med D10 från Borås, Västra Götalands län, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Borås.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-tur-i-osterled/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-dagtouring-maj-2026",
      "name": "ÖAMCK Dagtouring",
      "date": "2026-05-23",
      "dateEnd": "2026-05-23",
      "location": "Lelles MC, Uppsala",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/dagtouring-2/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Dagtouring arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lelles MC, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/dagtouring-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lelles+MC%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-mcq-pernillas-tur-2026",
      "name": "D11 mcQ Pernillas tur",
      "date": "2026-05-24",
      "dateEnd": "2026-05-24",
      "location": "Öland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D11 från Öland. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-mcq-pernillas-tur/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 mcQ Pernillas tur arrangeras av BMW MC-klubben.\n\nKörning med D11 från Öland. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Öland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-mcq-pernillas-tur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-morgon-i-kopenhamn-2026",
      "name": "D15 Morgon i Köpenhamn",
      "date": "2026-05-24",
      "dateEnd": "2026-05-24",
      "location": "Utsiktsvägen 10, Limhamn",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Utsiktsvägen 10, 216 30 Limhamn, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-morgon-i-kopenhamn/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Morgon i Köpenhamn arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Utsiktsvägen 10, 216 30 Limhamn, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Utsiktsvägen 10, Limhamn.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-morgon-i-kopenhamn/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Utsiktsv%C3%A4gen+10%2C+Limhamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-05-25",
      "dateEnd": "2026-05-25",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-25/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-05-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-05-26",
      "dateEnd": "2026-05-26",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-26/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-05-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-05-26",
      "dateEnd": "2026-05-26",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-26/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-05-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, jämna veckor",
      "date": "2026-05-26",
      "dateEnd": "2026-05-26",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-05-26/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-05-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-05-27",
      "dateEnd": "2026-05-27",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-27/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-05-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-05-27",
      "dateEnd": "2026-05-27",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-27/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-05-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d7-tur-for-nya-medlemmar-2026",
      "name": "D7 Tur för nya medlemmar",
      "date": "2026-05-27",
      "dateEnd": "2026-05-27",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D7 från Stockholm. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d7-tur-for-nya-medlemmar/",
      "region": "Stockholm",
      "source": "bmwklubben.se",
      "descriptionFull": "D7 Tur för nya medlemmar arrangeras av BMW MC-klubben.\n\nKörning med D7 från Stockholm. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d7-tur-for-nya-medlemmar/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-05-28",
      "dateEnd": "2026-05-28",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-28/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-05-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-05-28",
      "dateEnd": "2026-05-28",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-28/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-05-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-05-28",
      "dateEnd": "2026-05-28",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-28/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-05-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-05-28",
      "dateEnd": "2026-05-28",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-05-28/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-05-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-05-28",
      "dateEnd": "2026-05-28",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-28/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-05-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "roadracing-sm-knutstorp-2026",
      "name": "Roadracing SM - Ring Knutstorp",
      "date": "2026-05-29",
      "dateEnd": "2026-05-31",
      "location": "Ring Knutstorp, Kågeröd",
      "type": "Racing",
      "organizer": "MotoSE / Svemo",
      "description": "MC-arrangemang i Ring Knutstorp, Kågeröd, Skåne län. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://mcparken.se/calendar/175",
      "region": "Skåne",
      "source": "mcparken.se",
      "descriptionFull": "Roadracing SM - Ring Knutstorp arrangeras av MotoSE / Svemo.\n\nMC-arrangemang i Ring Knutstorp, Kågeröd, Skåne län. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Ring Knutstorp, Kågeröd.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/175"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ring+Knutstorp%2C+K%C3%A5ger%C3%B6d",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-vatternrundan-2026",
      "name": "DOA - Vätternrundan",
      "date": "2026-05-29",
      "dateEnd": "2026-05-30",
      "location": "Vättern",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Vättern. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-vatternrundan/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Vätternrundan arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Vättern. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vättern.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-vatternrundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ttern",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-noplatraffen-i-kristianopel-29-31-maj-2026",
      "name": "DOE Noplaträffen i Kristianopel 29-31 maj",
      "date": "2026-05-29",
      "dateEnd": "2026-05-31",
      "location": "Kristianopel",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Välkomna till årets första träff på Kristianopel Camping/Resort. Campingen erbjuder följande rätter om vi förbokar senast 22 maj.",
      "link": "https://hdcs.se/evenemang/doe-noplatraffen-i-kristianopel-29-31-maj/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Noplaträffen i Kristianopel 29-31 maj arrangeras av H-DCS.\n\nVälkomna till årets första träff på Kristianopel Camping/Resort. Campingen erbjuder följande rätter om vi förbokar senast 22 maj.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kristianopel.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-noplatraffen-i-kristianopel-29-31-maj/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kristianopel",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-vikingtraffen-gallo-2026",
      "name": "Vikingtraffen Gallo",
      "date": "2026-05-29",
      "dateEnd": "2026-05-31",
      "location": "Gallo, Jamtland",
      "type": "Träff",
      "organizer": "Jan Karlstrand / GWCS",
      "description": "MC-arrangemang i Gallo, Jamtland. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://media.gwcs.se/2026/02/gallo26.jpeg",
      "region": "Jämtland",
      "source": "gwcs.se",
      "frontImage": "ads/gwcs-vikingtraffen-gallo-2026-05-29.jpg",
      "descriptionFull": "Vikingtraffen Gallo arrangeras av Jan Karlstrand / GWCS.\n\nMC-arrangemang i Gallo, Jamtland. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Gallo, Jamtland.",
      "links": [
        {
          "label": "media.gwcs.se",
          "url": "https://media.gwcs.se/2026/02/gallo26.jpeg"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gallo%2C+Jamtland",
          "type": "map"
        }
      ]
    },
    {
      "id": "sidvagnsstraff-med-arsmoete-2026",
      "name": "Sidvagnsträff med årsmöte",
      "date": "2026-05-29",
      "dateEnd": "2026-05-31",
      "location": "Loiterer MC klubbstuga, Falkenberg",
      "type": "Träff",
      "organizer": "Svenska Sidvagnsklubben SVEA",
      "description": "Träffpunkt i Loiterer MC klubbstuga, Falkenberg. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://www.sidvagn.se/",
      "region": "Halland",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Sidvagnsträff med årsmöte arrangeras av Svenska Sidvagnsklubben SVEA.\n\nTräffpunkt i Loiterer MC klubbstuga, Falkenberg. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Loiterer MC klubbstuga, Falkenberg.",
      "links": [
        {
          "label": "sidvagn.se",
          "url": "https://www.sidvagn.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Loiterer+MC+klubbstuga%2C+Falkenberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "hermans-sjosalg-2026",
      "name": "Hermans Sjöslag 2026",
      "date": "2026-05-29",
      "dateEnd": "2026-05-31",
      "location": "Sundet vid Sjön Vidöstern",
      "type": "Träff",
      "organizer": "SMC",
      "description": "Arrangemang med SMC i Sundet vid Sjön Vidöstern. Mötespunkt för MC-klubben. Mer information på smc.nu.",
      "link": "https://www.hermanshedningar.se/",
      "region": "Jönköping",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Hermans Sjöslag 2026 arrangeras av SMC.\n\nArrangemang med SMC i Sundet vid Sjön Vidöstern. Mötespunkt för MC-klubben. Mer information på smc.nu.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sundet vid Sjön Vidöstern.",
      "links": [
        {
          "label": "hermanshedningar.se",
          "url": "https://www.hermanshedningar.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sundet+vid+Sj%C3%B6n+Vid%C3%B6stern",
          "type": "map"
        }
      ]
    },
    {
      "id": "vattern-runt-med-region-mitt-distrikt-a-d-f-j-2026",
      "name": "Vättern runt med region mitt distrikt A , D , F , J",
      "date": "2026-05-30",
      "dateEnd": "2026-05-31",
      "location": "Vättern",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vättern runt med region mitt distrikt A , D , F.",
      "link": "https://hdcs.se/evenemang/vattern-runt-med-region-mitt-distrikt-a-d-f-j/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Vättern runt med region mitt distrikt A , D , F , J arrangeras av H-DCS.\n\nVättern runt med region mitt distrikt A , D , F.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vättern.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/vattern-runt-med-region-mitt-distrikt-a-d-f-j/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ttern",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-tur-till-medlemsbutiken-i-ljungby-2026",
      "name": "DOB- Tur till medlemsbutiken i Ljungby",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Ljungby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOB från Ljungby. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-kickoff-medlems-butiken-ljungby/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Tur till medlemsbutiken i Ljungby arrangeras av H-DCS.\n\nKörtur med H-DCS DOB från Ljungby. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ljungby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-kickoff-medlems-butiken-ljungby/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ljungby",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-kickoff-medlems-butiken-ljungby-2026",
      "name": "DO-C Kickoff Medlems butiken Ljungby",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Medlemsbutiken, Ljungby",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Medlemsbutiken, Ljungby. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-körning-till-medlems-butiken-ljungby/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Kickoff Medlems butiken Ljungby arrangeras av H-DCS.\n\nArrangemang med H-DCS från Medlemsbutiken, Ljungby. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Medlemsbutiken, Ljungby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-körning-till-medlems-butiken-ljungby/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Medlemsbutiken%2C+Ljungby",
          "type": "map"
        }
      ]
    },
    {
      "id": "korvrundan-2026",
      "name": "Körvrundan",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Sverige",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/korvrundan/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "Körvrundan arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/korvrundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-30/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-05-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-30/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-05-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "ds-i-orange-day-sundsvall-2026",
      "name": "DS-I Orange Day Sundsvall",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Shell Punkten Bosvedjan, Sundsvall",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi kör en tur för Orange-Day , med insamling för att motverka våld mot kvinnor och barn. Start 12:00 från Shell Bydalen.",
      "link": "https://hdcs.se/evenemang/ds-i-orange-day-sundsvall/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "DS-I Orange Day Sundsvall arrangeras av H-DCS.\n\nVi kör en tur för Orange-Day , med insamling för att motverka våld mot kvinnor och barn. Start 12:00 från Shell Bydalen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Shell Punkten Bosvedjan, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/ds-i-orange-day-sundsvall/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Shell+Punkten+Bosvedjan%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-05-30/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-05-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "lerduveskytte-2026",
      "name": "Lerduveskytte",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Hejdeby norrbys 131, Visby",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Hejdeby norrbys 131, Visby. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/lerduveskytte/",
      "region": "Gotland",
      "source": "hdcs.se",
      "descriptionFull": "Lerduveskytte arrangeras av H-DCS.\n\nArrangemang med H-DCS från Hejdeby norrbys 131, Visby. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Hejdeby norrbys 131, Visby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lerduveskytte/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hejdeby+norrbys+131%2C+Visby",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-mcq-morsdagsrunda-2026-2026",
      "name": "D11 mcQ Morsdagsrunda 2026",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Småland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D11 från Småland. Gemensam körning för mödrar. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-mcq-morsdagsrunda-2026/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 mcQ Morsdagsrunda 2026 arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D11 från Småland. Gemensam körning för mödrar. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-mcq-morsdagsrunda-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-asarna-2026",
      "name": "D15 Åsarna",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Rusthållsgatan, Helsingborg",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Rusthållsgatan, 253 61 Helsingborg, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-asarna/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Åsarna arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Rusthållsgatan, 253 61 Helsingborg, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rusthållsgatan, Helsingborg.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-asarna/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Rusth%C3%A5llsgatan%2C+Helsingborg",
          "type": "map"
        }
      ]
    },
    {
      "id": "bollebygds-veteranmarknad-2026",
      "name": "Bollebygds Veteranmarknad",
      "date": "2026-05-30",
      "dateEnd": "2026-05-30",
      "location": "Hägnen 8, Bollebygd",
      "type": "Show",
      "organizer": "Bollebygds Veteranmarknad",
      "description": "Veteranmarknad för bilar, motorcyklar, mopeder och nostalgi i Bollebygd. Ny plats på Hägnen 8.",
      "descriptionFull": "Bollebygds Veteranmarknad den 30 maj 2026.\n\nPopulär veteranmarknad med bilar, motorcyklar, mopeder och nostalgi. Säljare och köpare samlas för att hitta reservdelar, tillbehör och kuriosa.\n\nNy plats: Hägnen 8, Bollebygd.\n\nKontakt: 070-826 54 09 eller 070-657 88 67.\n\nPlats: Hägnen 8, Bollebygd.",
      "links": [
        {
          "label": "bollebygdsveteranmarknad.com",
          "url": "https://bollebygdsveteranmarknad.com/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=H%C3%A4gnen+8+Bollebygd",
          "type": "map"
        }
      ],
      "link": "https://bollebygdsveteranmarknad.com/",
      "backImage": "ads/bollebygds-veteranmarknad-back-2026-05-30.webp",
      "region": "Västra Götaland",
      "source": "bollebygdsveteranmarknad.com"
    },
    {
      "id": "filip-backlund-trackdays-mantorp-2026",
      "name": "Filip Backlund Trackdays Mantorp Park",
      "date": "2026-05-31",
      "dateEnd": "2026-06-01",
      "location": "Mantorp, Park",
      "type": "Racing",
      "organizer": "Filip Backlund Trackdays",
      "description": "Trackdays 2.0 - eventet för dig som vill lyfta din körupplevelse till nya höjder.",
      "link": "https://mcparken.se/calendar/180",
      "region": "Östergötland",
      "source": "mcparken.se",
      "descriptionFull": "Filip Backlund Trackdays Mantorp Park arrangeras av Filip Backlund Trackdays.\n\nTrackdays 2.0 - eventet för dig som vill lyfta din körupplevelse till nya höjder.\n\nBankdag för motorcyklister. Kontrollera arrangören för anmälan och regler.\n\nPlats: Mantorp, Park.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/180"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Mantorp%2C+Park",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-mats-och-bodils-tur-2026",
      "name": "D15 Mats och Bodils tur",
      "date": "2026-05-31",
      "dateEnd": "2026-05-31",
      "location": "Skåne",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D15 från Skåne. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-mats-och-bodils-tur/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Mats och Bodils tur arrangeras av BMW MC-klubben.\n\nKörning med D15 från Skåne. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-mats-och-bodils-tur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-soder8-an-2026",
      "name": "Söder8:an",
      "date": "2026-05-31",
      "dateEnd": "2026-05-31",
      "location": "Fjärås Industriväg 2, Fjärås",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Fjärås Industriväg 2, 439 74 Fjärås, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/soder8an/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "Söder8:an arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Fjärås Industriväg 2, 439 74 Fjärås, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Fjärås Industriväg 2, Fjärås.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/soder8an/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Fj%C3%A4r%C3%A5s+Industriv%C3%A4g+2%2C+Fj%C3%A4r%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-06-01",
      "dateEnd": "2026-06-01",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-01/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-06-02",
      "dateEnd": "2026-06-02",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-02/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-06-02",
      "dateEnd": "2026-06-02",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-02/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-06-02",
      "dateEnd": "2026-06-02",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-06-02/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-06-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-dagsledigtur-med-rune-2026",
      "name": "D11 Dagsledigtur med Rune",
      "date": "2026-06-02",
      "dateEnd": "2026-06-02",
      "location": "Tingsryd, Tingsryd",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D11 från Tingsryd, 362 32 Tingsryd, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-dagsledigtur-med-rune/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Dagsledigtur med Rune arrangeras av BMW MC-klubben.\n\nKörning med D11 från Tingsryd, 362 32 Tingsryd, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Tingsryd, Tingsryd.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-dagsledigtur-med-rune/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tingsryd%2C+Tingsryd",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-06-03",
      "dateEnd": "2026-06-03",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-03/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo, udda veckor",
      "date": "2026-06-03",
      "dateEnd": "2026-06-03",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-06-03/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo, udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-06-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-06-03",
      "dateEnd": "2026-06-03",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-03/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-onsdagstur-med-blandat-underlag-2026",
      "name": "D15 Onsdagstur med blandat underlag",
      "date": "2026-06-03",
      "dateEnd": "2026-06-03",
      "location": "Ringsjövägen 40, Hörby",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Onsdagstur med D15 på blandade vägar. Samling enligt schema. Se bmwklubben.se för tider och plats.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-onsdagstur-med-blandat-underlag-3/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Onsdagstur med blandat underlag arrangeras av BMW MC-klubben.\n\nOnsdagstur med D15 på blandade vägar. Samling enligt schema. Se bmwklubben.se för tider och plats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ringsjövägen 40, Hörby.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-onsdagstur-med-blandat-underlag-3/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ringsj%C3%B6v%C3%A4gen+40%2C+H%C3%B6rby",
          "type": "map"
        }
      ]
    },
    {
      "id": "american-days-2026",
      "name": "American Days",
      "date": "2026-06-04",
      "dateEnd": "2026-06-07",
      "location": "Lundegård Camping, Öland",
      "type": "Fest",
      "organizer": "VICS",
      "description": "Skandinaviens största motor-, musik- och matfestival för amerikanska mc-märken. Victory, Indian, Harley-Davidson och fler. 4-7 juni 2026 på Lundegård Camping, Öland.",
      "link": "https://www.americandays.se/",
      "region": "Kalmar",
      "source": "vics.se",
      "descriptionFull": "American Days arrangeras av VICS.\n\nSkandinaviens största motor-, musik- och matfestival för amerikanska mc-märken. Victory, Indian, Harley-Davidson och fler. 4-7 juni 2026 på Lundegård Camping, Öland.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Lundegård Camping, Öland.",
      "links": [
        {
          "label": "americandays.se",
          "url": "https://www.americandays.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lundeg%C3%A5rd+Camping%2C+%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "american-days-oland-2026",
      "name": "American Days Öland",
      "date": "2026-06-04",
      "dateEnd": "2026-06-07",
      "location": "Lundegård Camping, Öland",
      "type": "Fest",
      "organizer": "H-DCS",
      "description": "H-DCS egen träff under American Days-helgen på Lundegård Camping, Öland. Mötesplats för Harley-klubbens medlemmar. 4-7 juni. Se hdcs.se för samlingspunkt och tider.",
      "link": "https://hdcs.se/evenemang/american-days-oland/",
      "region": "Kalmar",
      "source": "hdcs.se",
      "descriptionFull": "American Days Öland arrangeras av H-DCS.\n\nH-DCS egen träff under American Days-helgen på Lundegård Camping, Öland. Mötesplats för Harley-klubbens medlemmar. 4-7 juni. Se hdcs.se för samlingspunkt och tider.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Lundegård Camping, Öland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/american-days-oland/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lundeg%C3%A5rd+Camping%2C+%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-gemensam-körning-till-american-days-2026",
      "name": "DO-C Gemensam körning till American days",
      "date": "2026-06-04",
      "dateEnd": "2026-06-04",
      "location": "Halland/VGötaland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Halland/VGötaland. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-gemensam-körning-till-american-days/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Gemensam körning till American days arrangeras av H-DCS.\n\nKörtur med H-DCS från Halland/VGötaland. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Halland/VGötaland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-gemensam-körning-till-american-days/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halland%2FVG%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-06-04",
      "dateEnd": "2026-06-04",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-04/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-06-04",
      "dateEnd": "2026-06-04",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-04/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-06-04",
      "dateEnd": "2026-06-04",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-04/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-06-04",
      "dateEnd": "2026-06-04",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-04/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "trollhattetraffen-2026",
      "name": "Trollhätteträffen",
      "date": "2026-06-05",
      "dateEnd": "2026-06-07",
      "location": "Ursands Camping, Trollhättan",
      "type": "Träff",
      "organizer": "SMC Västra Götaland",
      "description": "Stor MC-träff på nationaldagshelgen. Camping, livemusik och gemensamma utfärder.",
      "link": "https://www.svmc.se/",
      "region": "Västra Götaland",
      "source": "svmc.se",
      "descriptionFull": "Trollhätteträffen arrangeras av SMC Västra Götaland.\n\nStor MC-träff på nationaldagshelgen. Camping, livemusik och gemensamma utfärder.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Ursands Camping, Trollhättan.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ursands+Camping%2C+Trollh%C3%A4ttan",
          "type": "map"
        }
      ]
    },
    {
      "id": "siljansstraffene-mora-2026",
      "name": "Siljansträffen",
      "date": "2026-06-05",
      "dateEnd": "2026-06-07",
      "location": "Red Wings Mora MC klubbstuga, Lomsmyren, Mora",
      "type": "Träff",
      "organizer": "Red Wings Mora MC",
      "description": "Klassisk MC-träff i gammal stil. Litet och gemytligt, lättillgängligt vid sjön Siljan.",
      "link": "https://www.redwingsmoramc.se/",
      "region": "Dalarna",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Siljansträffen arrangeras av Red Wings Mora MC.\n\nKlassisk MC-träff i gammal stil. Litet och gemytligt, lättillgängligt vid sjön Siljan.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Red Wings Mora MC klubbstuga, Lomsmyren, Mora.",
      "links": [
        {
          "label": "redwingsmoramc.se",
          "url": "https://www.redwingsmoramc.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Red+Wings+Mora+MC+klubbstuga%2C+Lomsmyren%2C+Mora",
          "type": "map"
        }
      ]
    },
    {
      "id": "custom-bikeshow-norrtalje-2026",
      "name": "Custom Bike Show Norrtälje",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Norrtälje",
      "type": "Show",
      "organizer": "Custom Bike Show",
      "description": "Europas äldsta custom bike show. Choppers, bobbers, café racers och konstverk på två hjul.",
      "link": "https://www.custombikeshow.se/",
      "region": "Uppsala",
      "source": "custombikeshow.se",
      "backImage": "ads/custom-bike-show-norrtalje-back-2026-06-06.jpg",
      "descriptionFull": "Custom Bike Show Norrtälje arrangeras av Custom Bike Show.\n\nEuropas äldsta custom bike show. Choppers, bobbers, café racers och konstverk på två hjul.\n\nMC-show med utsmyckade hojar och byggen. Kolla arrangören för tider.\n\nPlats: Norrtälje.",
      "links": [
        {
          "label": "custombikeshow.se",
          "url": "https://www.custombikeshow.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "nationaldagstur-2026",
      "name": "Nationaldagstur",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Sverige",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Sverige. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/nationaldagstur/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Nationaldagstur arrangeras av H-DCS.\n\nKörtur med H-DCS från Sverige. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/nationaldagstur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-nationaldagstur-2026",
      "name": "DOA - Nationaldagstur",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-nationaldagstur/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Nationaldagstur arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-nationaldagstur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-custom-bike-show-norrtalje-2026",
      "name": "DOA - Custom Bike Show, Norrtälje",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Norrtälje",
      "type": "Show",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Norrtälje. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-custom-bike-show-norrtalje-2/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Custom Bike Show, Norrtälje arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Norrtälje. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nMC-show med utsmyckade hojar och byggen. Kolla arrangören för tider.\n\nPlats: Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-custom-bike-show-norrtalje-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-gemensam-körning-till-baprun-2026",
      "name": "DO-C Gemensam körning till Baprun",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Halland/VGötaland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Halland/VGötaland. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-gemensam-körning-till-baprun/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Gemensam körning till Baprun arrangeras av H-DCS.\n\nKörtur med H-DCS från Halland/VGötaland. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Halland/VGötaland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-gemensam-körning-till-baprun/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halland%2FVG%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "tur-till-torsangs-motormuseum-2026",
      "name": "Tur till Torsångs Motormuseum",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Torsångs Motormuseum, Borlänge",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Start från Färjkajen i Västerås kl. 10 Vi kör över Norberg, Avesta, Hedemora.",
      "link": "https://hdcs.se/evenemang/tur-till-torsangs-motormuseum/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "Tur till Torsångs Motormuseum arrangeras av H-DCS.\n\nStart från Färjkajen i Västerås kl. 10 Vi kör över Norberg, Avesta, Hedemora.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Torsångs Motormuseum, Borlänge.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/tur-till-torsangs-motormuseum/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tors%C3%A5ngs+Motormuseum%2C+Borl%C3%A4nge",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-06/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-06/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-aventyrshelg-grus-2026",
      "name": "D15 Åventyrshelg Grus",
      "date": "2026-06-06",
      "dateEnd": "2026-06-07",
      "location": "Skåne",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D15. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-aventyrshelg-grus/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Åventyrshelg Grus arrangeras av BMW MC-klubben.\n\nGrustour med D15. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-aventyrshelg-grus/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d9-nationaldagstur-2026",
      "name": "D9 Nationaldagstur",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Östergötland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D9 från Östergötland. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d9-nationaldagstur/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "D9 Nationaldagstur arrangeras av BMW MC-klubben.\n\nKörning med D9 från Östergötland. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Östergötland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d9-nationaldagstur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96sterg%C3%B6tland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-nationaldagsgruset-2026",
      "name": "D10 Nationaldagsgruset",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Västra, Götaland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D10. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-nationaldagsgruset/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Nationaldagsgruset arrangeras av BMW MC-klubben.\n\nGrustour med D10. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Västra, Götaland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-nationaldagsgruset/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4stra%2C+G%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-bengts-grustur-2026",
      "name": "D8 Bengts grustur",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Västra, Götaland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D8. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-bengts-grustur/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Bengts grustur arrangeras av BMW MC-klubben.\n\nGrustour med D8. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Västra, Götaland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-bengts-grustur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4stra%2C+G%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-historisk-heldagstur-oland-2026-2026",
      "name": "D11 Historisk heldagstur Öland 2026",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Öland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D11 från Öland. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-historisk-heldagstur-oland-2026/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Historisk heldagstur Öland 2026 arrangeras av BMW MC-klubben.\n\nKörning med D11 från Öland. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Öland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-historisk-heldagstur-oland-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-nationaldagsturen-2026",
      "name": "D14-Nationaldagsturen",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Växjövägen 15, Halmstad",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D14 från Växjövägen 15, 302 44 Halmstad, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-nationaldagsturen/",
      "region": "Halland",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-Nationaldagsturen arrangeras av BMW MC-klubben.\n\nKörning med D14 från Växjövägen 15, 302 44 Halmstad, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Växjövägen 15, Halmstad.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-nationaldagsturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4xj%C3%B6v%C3%A4gen+15%2C+Halmstad",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-nationaldagstur-2026",
      "name": "GWCS Nationaldagstur Sigridslunds Cafe",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Sigridslunds Cafe, Ardala",
      "type": "Körning",
      "organizer": "Sormlandswingarna / GWCS",
      "description": "Gemensam körning från Sigridslunds Cafe, Ardala. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://gwcs.se/events/nationaldagstur-till-sigridslunds-cafe/?occurrence=2026-06-06",
      "region": "Södermanland",
      "source": "gwcs.se",
      "descriptionFull": "GWCS Nationaldagstur Sigridslunds Cafe arrangeras av Sormlandswingarna / GWCS.\n\nGemensam körning från Sigridslunds Cafe, Ardala. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Sigridslunds Cafe, Ardala.",
      "links": [
        {
          "label": "gwcs.se",
          "url": "https://gwcs.se/events/nationaldagstur-till-sigridslunds-cafe/?occurrence=2026-06-06"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sigridslunds+Cafe%2C+Ardala",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-nationaldagsloppet-2026",
      "name": "Nationaldagsloppet (ordonnanser)",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "MC-ordonnans",
      "description": "Gemensam körning från Stockholm. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://oamck.se/events/nationaldagsloppet-ordonnanser/",
      "region": "Stockholm",
      "source": "oamck.se",
      "descriptionFull": "Nationaldagsloppet (ordonnanser) arrangeras av MC-ordonnans.\n\nGemensam körning från Stockholm. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/nationaldagsloppet-ordonnanser/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "mustaschkortegen-broby-2026",
      "name": "Mustaschkortegen",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "Thygesson Bussar, Broby",
      "type": "Träff",
      "organizer": "Lille Mats MCK",
      "description": "MC-kortege till förmån för prostacancerföreningen. Pris 150 kr per hjälm. Alla medel går till Pro Vitae.",
      "link": "https://smcboken.svmc.se/traffar/mcträffar",
      "region": "Skåne",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Mustaschkortegen arrangeras av Lille Mats MCK.\n\nMC-kortege till förmån för prostacancerföreningen. Pris 150 kr per hjälm. Alla medel går till Pro Vitae.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Thygesson Bussar, Broby.",
      "links": [
        {
          "label": "smcboken.svmc.se",
          "url": "https://smcboken.svmc.se/traffar/mcträffar"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Thygesson+Bussar%2C+Broby",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-malaren-runt-fran-nykvarn-en-svensk-mc-klassiker-2026",
      "name": "Mälaren Runt från Nykvarn - En svensk mc klassiker",
      "date": "2026-06-07",
      "dateEnd": "2026-06-07",
      "location": "Kraftleden 2, Bålsta",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "En del av En svensk mc klassiker. Idag runt Sveriges tredje största sjö med start från Nykvarn.",
      "link": "https://www.bmwklubben.se/aktiviteter/malaren-runt-fran-nykvarn-en-svensk-mc-klassiker/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "Mälaren Runt från Nykvarn - En svensk mc klassiker arrangeras av BMW MC-klubben.\n\nEn del av En svensk mc klassiker. Idag runt Sveriges tredje största sjö med start från Nykvarn.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kraftleden 2, Bålsta.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/malaren-runt-fran-nykvarn-en-svensk-mc-klassiker/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kraftleden+2%2C+B%C3%A5lsta",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-malaren-runt-fran-balsta-en-svensk-mc-klassiker-2026",
      "name": "Mälaren Runt från Bålsta - En svensk mc klassiker",
      "date": "2026-06-07",
      "dateEnd": "2026-06-07",
      "location": "Kraftleden 2, Bålsta",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "En del av En svensk mc klassiker. Idag runt Sveriges tredje största sjö med start från Bålsta.",
      "link": "https://www.bmwklubben.se/aktiviteter/malaren-runt-fran-balsta-en-svensk-mc-klassiker/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "Mälaren Runt från Bålsta - En svensk mc klassiker arrangeras av BMW MC-klubben.\n\nEn del av En svensk mc klassiker. Idag runt Sveriges tredje största sjö med start från Bålsta.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kraftleden 2, Bålsta.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/malaren-runt-fran-balsta-en-svensk-mc-klassiker/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kraftleden+2%2C+B%C3%A5lsta",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-dagtouring-jun-2026",
      "name": "ÖAMCK Dagtouring (sondag)",
      "date": "2026-06-07",
      "dateEnd": "2026-06-07",
      "location": "Lelles MC, Uppsala",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/dagtouring-obs-sondag/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Dagtouring (sondag) arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lelles MC, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/dagtouring-obs-sondag/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lelles+MC%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-06-08",
      "dateEnd": "2026-06-08",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-08/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-goes-to-pite-havsbad-arstraff-2026-2026",
      "name": "D11 Goes to Pite Havsbad (Årsträff 2026)",
      "date": "2026-06-08",
      "dateEnd": "2026-06-10",
      "location": "Hotellvägen 50, Piteå",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Träffpunkt med D11 i Hotellvägen 50, 941 43 Piteå, Sverige. Gemensam fika och MC-prat. Alla medlemmar välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-goes-to-pite-havsbad-arstraff-2026/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Goes to Pite Havsbad (Årsträff 2026) arrangeras av BMW MC-klubben.\n\nTräffpunkt med D11 i Hotellvägen 50, 941 43 Piteå, Sverige. Gemensam fika och MC-prat. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Hotellvägen 50, Piteå.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-goes-to-pite-havsbad-arstraff-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hotellv%C3%A4gen+50%2C+Pite%C3%A5",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-06-09",
      "dateEnd": "2026-06-09",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-09/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-06-09",
      "dateEnd": "2026-06-09",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-09/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, jämna veckor",
      "date": "2026-06-09",
      "dateEnd": "2026-06-09",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-06-09/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-06-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-06-10",
      "dateEnd": "2026-06-10",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-10/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-06-10",
      "dateEnd": "2026-06-10",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-10/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "htc-och-smc-kvallsstraff-nassjo-2026",
      "name": "HTC och SMC kvällsträff Nässjö",
      "date": "2026-06-10",
      "dateEnd": "2026-06-10",
      "location": "Gisshults badplats, Nässjö",
      "type": "Träff",
      "organizer": "Höglandets Touring Club",
      "description": "Träffpunkt i Gisshults badplats, Nässjö. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://www.htconline.se/",
      "region": "Jönköping",
      "source": "smcboken.svmc.se",
      "descriptionFull": "HTC och SMC kvällsträff Nässjö arrangeras av Höglandets Touring Club.\n\nTräffpunkt i Gisshults badplats, Nässjö. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Gisshults badplats, Nässjö.",
      "links": [
        {
          "label": "htconline.se",
          "url": "https://www.htconline.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gisshults+badplats%2C+N%C3%A4ssj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "76-mil-grus-2026",
      "name": "76 mil grus",
      "date": "2026-06-11",
      "dateEnd": "2026-06-14",
      "location": "Ockelbo, Gävleborg",
      "type": "Körning",
      "organizer": "Privat arrangör",
      "description": "Gemensam körning från Ockelbo, Gävleborg. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://mcparken.se/calendar/166",
      "region": "Gävleborg",
      "source": "mcparken.se",
      "descriptionFull": "76 mil grus arrangeras av Privat arrangör.\n\nGemensam körning från Ockelbo, Gävleborg. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ockelbo, Gävleborg.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/166"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ockelbo%2C+G%C3%A4vleborg",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-06-11",
      "dateEnd": "2026-06-11",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-11/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-06-11",
      "dateEnd": "2026-06-11",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-11/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-06-11",
      "dateEnd": "2026-06-11",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-11/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-06-11",
      "dateEnd": "2026-06-11",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-06-11/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-06-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-06-11",
      "dateEnd": "2026-06-11",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-11/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-arstraff-2026-pitea-2026",
      "name": "Årsträff 2026 Piteå",
      "date": "2026-06-11",
      "dateEnd": "2026-06-14",
      "location": "Havsbadsvägen, Piteå",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Torsdag 11 juni t.o.m. söndag 14 juni går årsträffen 2026 av stapeln på Pite Havsbad. Varmt välkommen! ...",
      "link": "https://www.bmwklubben.se/aktiviteter/arstraff-2026-pitea/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "Årsträff 2026 Piteå arrangeras av BMW MC-klubben.\n\nTorsdag 11 juni t.o.m. söndag 14 juni går årsträffen 2026 av stapeln på Pite Havsbad. Varmt välkommen! ...\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Havsbadsvägen, Piteå.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/arstraff-2026-pitea/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Havsbadsv%C3%A4gen%2C+Pite%C3%A5",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-sagverksfesten-xii-2026",
      "name": "DOH-Sågverksfesten XII",
      "date": "2026-06-12",
      "dateEnd": "2026-06-14",
      "location": "Sawmill MC Tavelsjö, Tavelsjö",
      "type": "Fest",
      "organizer": "H-DCS",
      "description": "Träffen blir i år som vanligt helgen före midsommar. Detta blir den tolfte Sågverksfesten.",
      "link": "https://hdcs.se/evenemang/doh-sagverksfesten-xii/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Sågverksfesten XII arrangeras av H-DCS.\n\nTräffen blir i år som vanligt helgen före midsommar. Detta blir den tolfte Sågverksfesten.\n\nFest och social sammankomst för MC-folket. Se arrangören för detaljer.\n\nPlats: Sawmill MC Tavelsjö, Tavelsjö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-sagverksfesten-xii/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sawmill+MC+Tavelsj%C3%B6%2C+Tavelsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-traff-wiggersvik-camping-2026",
      "name": "DO-C Träff Wiggersvik camping",
      "date": "2026-06-12",
      "dateEnd": "2026-06-14",
      "location": "Wiggersvik camping",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Wiggersvik camping. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-traff-wiggersvik-camping/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Träff Wiggersvik camping arrangeras av H-DCS.\n\nArrangemang med H-DCS från Wiggersvik camping. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Wiggersvik camping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-traff-wiggersvik-camping/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Wiggersvik+camping",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-grangetraffen-2026",
      "name": "DOG Grängeträffen",
      "date": "2026-06-12",
      "dateEnd": "2026-06-14",
      "location": "Grängesberg",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Grängeträffen i Grangärde 12-14 juni Mer info .Annemona Andersson 070 - 282 27 12 Christer Andersson 070.",
      "link": "https://hdcs.se/evenemang/dog-grangetraffen/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG Grängeträffen arrangeras av H-DCS.\n\nGrängeträffen i Grangärde 12-14 juni Mer info .Annemona Andersson 070 - 282 27 12 Christer Andersson 070.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Grängesberg.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-grangetraffen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gr%C3%A4ngesberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-hultsfred-2026-grus-och-asfalt-2026",
      "name": "D10 Hultsfred 2026, Grus OCH asfalt",
      "date": "2026-06-12",
      "dateEnd": "2026-06-14",
      "location": "Folkparksvägen 10, Hultsfred",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Välkomna att boka din plats till Hultsfred 2025. Det finns 34 platser och en oändlig reservlista.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-hultsfred-2026-grus-och-asfalt/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 Hultsfred 2026, Grus OCH asfalt arrangeras av BMW MC-klubben.\n\nVälkomna att boka din plats till Hultsfred 2025. Det finns 34 platser och en oändlig reservlista.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Folkparksvägen 10, Hultsfred.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-hultsfred-2026-grus-och-asfalt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Folkparksv%C3%A4gen+10%2C+Hultsfred",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-vatternrundan-2026",
      "name": "Vatternrundan (Ordonnanser)",
      "date": "2026-06-12",
      "dateEnd": "2026-06-14",
      "location": "Motala",
      "type": "Körning",
      "organizer": "MC-ordonnans",
      "description": "Gemensam körning från Motala. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://oamck.se/events/vatternrundan-ordonnanser-2/",
      "region": "Östergötland",
      "source": "oamck.se",
      "descriptionFull": "Vatternrundan (Ordonnanser) arrangeras av MC-ordonnans.\n\nGemensam körning från Motala. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Motala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/vatternrundan-ordonnanser-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Motala",
          "type": "map"
        }
      ]
    },
    {
      "id": "sagverksfesten-xii-tavelssjo-2026",
      "name": "Sågverksfesten XII",
      "date": "2026-06-12",
      "dateEnd": "2026-06-14",
      "location": "Tavelsjö",
      "type": "Träff",
      "organizer": "SMC",
      "description": "Arrangemang med SMC i Tavelsjö. Mötespunkt för MC-klubben. Mer information på smc.nu.",
      "link": "https://smcboken.svmc.se/traffar/mcträffar",
      "region": "Västerbotten",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Sågverksfesten XII arrangeras av SMC.\n\nArrangemang med SMC i Tavelsjö. Mötespunkt för MC-klubben. Mer information på smc.nu.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Tavelsjö.",
      "links": [
        {
          "label": "smcboken.svmc.se",
          "url": "https://smcboken.svmc.se/traffar/mcträffar"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tavelsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "kustturen-2026",
      "name": "Kustturen",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Västkusten",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Västkusten. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/kustturen/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Kustturen arrangeras av H-DCS.\n\nKörtur med H-DCS från Västkusten. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Västkusten.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/kustturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4stkusten",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-girl-fun-run-2026",
      "name": "DOA - Girl Fun Run",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Stockholm. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-girl-fun-run/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Girl Fun Run arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Stockholm. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-girl-fun-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-staffanstorp-motordag-2026",
      "name": "DOB- Staffanstorp motordag",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Staffanstorp",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOB från Staffanstorp. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-staffanstorp-motordag/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Staffanstorp motordag arrangeras av H-DCS.\n\nArrangemang med H-DCS DOB från Staffanstorp. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Staffanstorp.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-staffanstorp-motordag/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Staffanstorp",
          "type": "map"
        }
      ]
    },
    {
      "id": "charity-run-distrikt-k-2026",
      "name": "Charity Run - Distrikt K",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/charity-run-distrikt-k/",
      "region": "Blekinge",
      "source": "hdcs.se",
      "descriptionFull": "Charity Run - Distrikt K arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/charity-run-distrikt-k/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-13/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-13/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-06-13/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-06-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-skane-runt-med-spattan-2026",
      "name": "D15 Skåne runt med Spättan",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Kronetorpsvägen 8, Arlöv",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Kronetorpsvägen 8, 232 37 Arlöv, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-skane-runt-med-spettan/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Skåne runt med Spättan arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Kronetorpsvägen 8, 232 37 Arlöv, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kronetorpsvägen 8, Arlöv.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-skane-runt-med-spettan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kronetorpsv%C3%A4gen+8%2C+Arl%C3%B6v",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-bmwklubben-norge-d8-telemarken-runt-2026",
      "name": "BMWKLUBBEN Norge/D8 Telemarken runt",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Telemarken, Norge",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D8 från Telemarken, Norge. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/bmwklubben-norge-d8-telemarken-runt/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "BMWKLUBBEN Norge/D8 Telemarken runt arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D8 från Telemarken, Norge. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Telemarken, Norge.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/bmwklubben-norge-d8-telemarken-runt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Telemarken%2C+Norge",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-nashultarundan-2026",
      "name": "D11 Näshultarundan",
      "date": "2026-06-13",
      "dateEnd": "2026-06-13",
      "location": "Norremarksvägen, Växjö",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D11 från Norremarksvägen, 352 45 Växjö, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/nashultarundan/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Näshultarundan arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D11 från Norremarksvägen, 352 45 Växjö, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Norremarksvägen, Växjö.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/nashultarundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Norremarksv%C3%A4gen%2C+V%C3%A4xj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "hakan-p-slottsrundan-2026",
      "name": "Håkan P Slottsrundan",
      "date": "2026-06-14",
      "dateEnd": "2026-06-14",
      "location": "Sverige",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/hakan-pslottsrundan/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "Håkan P Slottsrundan arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/hakan-pslottsrundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-emilsturen-2026",
      "name": "DOA - Emilsturen",
      "date": "2026-06-14",
      "dateEnd": "2026-06-14",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-emilsturen/2026-06-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Emilsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-emilsturen/2026-06-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "mxsm-tibro-2026",
      "name": "MXSM Deltävling 3 - Tibro",
      "date": "2026-06-14",
      "dateEnd": "2026-06-14",
      "location": "Tibro MK, Tibro",
      "type": "Racing",
      "organizer": "SVEMO / Tibro MK",
      "description": "Deltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Västra Götaland",
      "source": "svemo.se",
      "descriptionFull": "MXSM Deltävling 3 - Tibro arrangeras av SVEMO / Tibro MK.\n\nDeltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Tibro MK, Tibro.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tibro+MK%2C+Tibro",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-06-15",
      "dateEnd": "2026-06-15",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-15/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-06-16",
      "dateEnd": "2026-06-16",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-16/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-06-16",
      "dateEnd": "2026-06-16",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-16/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-06-16",
      "dateEnd": "2026-06-16",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-06-16/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-06-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-06-17",
      "dateEnd": "2026-06-17",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-17/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo, udda veckor",
      "date": "2026-06-17",
      "dateEnd": "2026-06-17",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-06-17/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo, udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor/2026-06-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-06-17",
      "dateEnd": "2026-06-17",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-17/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-06-18",
      "dateEnd": "2026-06-18",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-18/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-06-18",
      "dateEnd": "2026-06-18",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-18/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-06-18",
      "dateEnd": "2026-06-18",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-18/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-06-18",
      "dateEnd": "2026-06-18",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-18/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "indian-riders-fest-2026",
      "name": "Indian Riders Fest 2026",
      "date": "2026-06-18",
      "dateEnd": "2026-06-21",
      "location": "Lipno nad Vltavou, Tjeckien",
      "type": "Fest",
      "organizer": "Indian Riders Fest",
      "description": "Europas största Indian Motorcycle-festival firar markts 125-årsjubileum. Fyra dagar med custom show, demo rides och live musik vid Lipnosjon.",
      "descriptionFull": "18-21 juni 2026, Lipno nad Vltavou, Tjeckien\n\nEn speciell utgåva som firar Indian Motorcycles 125-årsjubileum. Eventet äger rum vid den vackra Lipnosjon i Tjeckien.\n\nFestivalen samlar tusentals MC-entusiaster från hela världen. 2025 lockade eventet nästan 4 000 besökare från 44 länder.\n\nHöjdpunkter:\n- Budweis Indian Motorcycle Custom Show (BCS), 5:e upplagan\n- Demo rides på Indian Motorcycles\n- Live konserter och underhållning\n- Wall of Death\n- Utstallare och tillbehör\n- Parade Ride med Ladies First-prioritet\n\nIRF26 Riders Pack (125 EUR) inkluderar limited edition t-shirt, patch, parkering och tillgång till demo rides. Bara 1 250 pack finns tillgångliga.\n\nPlats: Lipno nad Vltavou, Tjeckien.",
      "backImage": "ads/indian-riders-fest-back-2026-06-18.jpg",
      "frontImage": "ads/indian-riders-fest-front-2026-06-18.jpg",
      "links": [
        {
          "label": "indianridersfest.eu",
          "url": "https://www.indianridersfest.eu/"
        },
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/IndianRidersFestCZ/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lipno+nad+Vltavou%2C+Czech+Republic",
          "type": "map"
        }
      ],
      "link": "https://www.indianridersfest.eu/",
      "region": "Tjeckien",
      "source": "FB"
    },
    {
      "id": "hdcs-midsommartraff-2026",
      "name": "H-DCS Midsommarträff",
      "date": "2026-06-19",
      "dateEnd": "2026-06-21",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "H-DCS midsommarträff. Fira midsommar tillsammans med Harley-vänner från hela landet.",
      "link": "https://hdcs.se/evenemang/",
      "region": "Sverige",
      "source": "hdcs.se",
      "descriptionFull": "H-DCS Midsommarträff arrangeras av H-DCS.\n\nH-DCS midsommarträff. Fira midsommar tillsammans med Harley-vänner från hela landet.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "h-dcdk-sjalland-midsommar-traff-2026",
      "name": "H-DCDK Själland Midsommar träff",
      "date": "2026-06-19",
      "dateEnd": "2026-06-21",
      "location": "Själland, Danmark",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Själland, Danmark. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/h-dcdk-sjalland-midsommar-traff-2/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "H-DCDK Själland Midsommar träff arrangeras av H-DCS.\n\nArrangemang med H-DCS från Själland, Danmark. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Själland, Danmark.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/h-dcdk-sjalland-midsommar-traff-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sj%C3%A4lland%2C+Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-midsommartraff-pa-hasselbacken-i-alstermo-19-2-2026",
      "name": "DOE Midsommarträff på Hasselbacken i Alstermo 19-21 juni",
      "date": "2026-06-19",
      "dateEnd": "2026-06-21",
      "location": "Alstermo",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Midsommarträff 19 - 21 juni på Hasselbacken i Alstermo. Välkommen att fira midsommar med oss i HDCS-E på den fantastiskt fina festplatsen.",
      "link": "https://hdcs.se/evenemang/doe-midsommartraff-pa-hasselbacken-i-alstermo-19-21-juni/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Midsommarträff på Hasselbacken i Alstermo 19-21 juni arrangeras av H-DCS.\n\nMidsommarträff 19 - 21 juni på Hasselbacken i Alstermo. Välkommen att fira midsommar med oss i HDCS-E på den fantastiskt fina festplatsen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Alstermo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-midsommartraff-pa-hasselbacken-i-alstermo-19-21-juni/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Alstermo",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-06-20",
      "dateEnd": "2026-06-20",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-20/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-06-20",
      "dateEnd": "2026-06-20",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-20/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "skovde-mc-sommartorget-2026",
      "name": "MC-Sommartorget Skövde",
      "date": "2026-06-20",
      "dateEnd": "2026-06-20",
      "location": "Kulturhuset, Skövde",
      "type": "Träff",
      "organizer": "Skaraborg MC Vanner",
      "description": "En avslappnad sommartraff för alla MC-entusiaster i Skaraborg. Parkering och fika.",
      "link": "https://druk.se/",
      "region": "Västra Götaland",
      "source": "manual",
      "_canary": true,
      "descriptionFull": "MC-Sommartorget Skövde arrangeras av Skaraborg MC Vanner.\n\nEn avslappnad sommartraff för alla MC-entusiaster i Skaraborg. Parkering och fika.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kulturhuset, Skövde.",
      "links": [
        {
          "label": "druk.se",
          "url": "https://druk.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kulturhuset%2C+Skovde",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-06-22",
      "dateEnd": "2026-06-22",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-22/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-06-23",
      "dateEnd": "2026-06-23",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-23/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-06-23",
      "dateEnd": "2026-06-23",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-23/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, jämna veckor",
      "date": "2026-06-23",
      "dateEnd": "2026-06-23",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-06-23/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor/2026-06-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-06-24",
      "dateEnd": "2026-06-24",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-24/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby/2026-06-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-hoj-cafe-sidensjo-2026",
      "name": "LO-I Hoj-Cafe Sidensjö",
      "date": "2026-06-24",
      "dateEnd": "2026-06-24",
      "location": "Sidensjö Bygdegård, Sidensjö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Ännu en gång är vi välkomna till Bygdegården i Sidensjö.",
      "link": "https://hdcs.se/evenemang/lo-i-hoj-cafe-sidensjo/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Hoj-Cafe Sidensjö arrangeras av H-DCS.\n\nÄnnu en gång är vi välkomna till Bygdegården i Sidensjö.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sidensjö Bygdegård, Sidensjö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-hoj-cafe-sidensjo/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sidensj%C3%B6+Bygdeg%C3%A5rd%2C+Sidensj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-06-24",
      "dateEnd": "2026-06-24",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-24/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning/2026-06-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-06-25",
      "dateEnd": "2026-06-25",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-25/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero/2026-06-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-06-25",
      "dateEnd": "2026-06-25",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-25/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-3/2026-06-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-skogskyrkogarden-2026",
      "name": "DOA - Kvällskörning Skogskyrkogården",
      "date": "2026-06-25",
      "dateEnd": "2026-06-25",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-25/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-skogskyrkogarden-3/2026-06-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-06-25",
      "dateEnd": "2026-06-25",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-06-25/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-06-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-06-25",
      "dateEnd": "2026-06-25",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-25/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-06-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-nationell-traff-degeberga-2026",
      "name": "Nationell traff Degeberga GWCS",
      "date": "2026-06-25",
      "dateEnd": "2026-06-28",
      "location": "Degeberga Stugby, Degeberga",
      "type": "Träff",
      "organizer": "Skanewingarna / GWCS",
      "description": "GWCS Nationella traff och foreningsstamma i Degeberga, Skane. Traffavgift 450:- för medlemmar.",
      "link": "https://media.gwcs.se/2026/01/Inbjudan-Nationella-2026-foreningsstamma1.pdf",
      "region": "Skåne",
      "source": "gwcs.se",
      "descriptionFull": "Nationell traff Degeberga GWCS arrangeras av Skanewingarna / GWCS.\n\nGWCS Nationella traff och foreningsstamma i Degeberga, Skane. Traffavgift 450:- för medlemmar.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Degeberga Stugby, Degeberga.",
      "links": [
        {
          "label": "media.gwcs.se",
          "url": "https://media.gwcs.se/2026/01/Inbjudan-Nationella-2026-foreningsstamma1.pdf"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Degeberga+Stugby%2C+Degeberga",
          "type": "map"
        }
      ]
    },
    {
      "id": "midnattssolsstraffenen-2026",
      "name": "Midnattssolsträffen",
      "date": "2026-06-25",
      "dateEnd": "2026-06-28",
      "location": "MMCK klubbområde",
      "type": "Träff",
      "organizer": "MMCK Malmfältens MCK",
      "description": "50:e Midnattssolsträffen med extra program. En av Sveriges nordligaste MC-träffar.",
      "link": "http://mmck.se/",
      "region": "Norrbotten",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Midnattssolsträffen arrangeras av MMCK Malmfältens MCK.\n\n50:e Midnattssolsträffen med extra program. En av Sveriges nordligaste MC-träffar.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: MMCK klubbområde.",
      "links": [
        {
          "label": "mmck.se",
          "url": "http://mmck.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=MMCK+klubbomr%C3%A5de",
          "type": "map"
        }
      ]
    },
    {
      "id": "roadracing-sm-sviestad-2026",
      "name": "Roadracing SM - Sviestad",
      "date": "2026-06-26",
      "dateEnd": "2026-06-28",
      "location": "Sviestad, Linköping",
      "type": "Racing",
      "organizer": "MotoSE / Svemo",
      "description": "MC-arrangemang i Sviestad, Linköping. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://mcparken.se/calendar/178",
      "region": "Östergötland",
      "source": "mcparken.se",
      "descriptionFull": "Roadracing SM - Sviestad arrangeras av MotoSE / Svemo.\n\nMC-arrangemang i Sviestad, Linköping. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Sviestad, Linköping.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/178"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sviestad%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-mcq-lager-hos-marre-i-mahult-2026",
      "name": "D14. McQ-läger hos Marre i Mahult",
      "date": "2026-06-26",
      "dateEnd": "2026-06-28",
      "location": "Ryabergsvägen 6, Simlångsdalen",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "För fjärde året bjuds det in till lägerhelg utanför Halmstad. Helgen 26-28 juni ses vi i metropolen Mahult i Simlångsdalen för att köra på fina halländska och småländska vägar!",
      "link": "https://www.bmwklubben.se/aktiviteter/mcq-lager-hos-marre-i-mahult/",
      "region": "Gotland",
      "source": "bmwklubben.se",
      "descriptionFull": "D14. McQ-läger hos Marre i Mahult arrangeras av BMW MC-klubben.\n\nFör fjärde året bjuds det in till lägerhelg utanför Halmstad. Helgen 26-28 juni ses vi i metropolen Mahult i Simlångsdalen för att köra på fina halländska och småländska vägar!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Ryabergsvägen 6, Simlångsdalen.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/mcq-lager-hos-marre-i-mahult/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ryabergsv%C3%A4gen+6%2C+Siml%C3%A5ngsdalen",
          "type": "map"
        }
      ]
    },
    {
      "id": "charity-run-2026",
      "name": "Charity Run",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/charity-run/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Charity Run arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/charity-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-adelso-runt-2026",
      "name": "DOA - Adelsö Runt",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Adelsö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Adelsö. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-adelso-runt/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Adelsö Runt arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Adelsö. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Adelsö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-adelso-runt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Adels%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-charity-run-2026",
      "name": "DOB- Charity Run",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Skåne",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOB från Skåne. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-charity-run/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Charity Run arrangeras av H-DCS.\n\nArrangemang med H-DCS DOB från Skåne. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-charity-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-h-d-golfen-2026",
      "name": "DO-C H-D Golfen",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Halland/VGötaland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-h-d-golfen/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C H-D Golfen arrangeras av H-DCS.\n\nArrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Halland/VGötaland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-h-d-golfen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halland%2FVG%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-charity-run-2026",
      "name": "DO-C Charity Run",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Halland/VGötaland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-charity-run/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Charity Run arrangeras av H-DCS.\n\nArrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Halland/VGötaland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-charity-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halland%2FVG%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-charity-run-27-juni-2026",
      "name": "DOE Charity run 27 juni",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Småland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Småland. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-charity-run-27-juni/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Charity run 27 juni arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Småland. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-charity-run-27-juni/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-charity-run-2026-2026",
      "name": "DOH-Charity Run 2026",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Monikas Café, Norsjö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "I år prövar vi samma plats som ligger i Norsjövallen för vår gemensamma samlingspunkt i länet.",
      "link": "https://hdcs.se/evenemang/doh-charity-run-2026/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Charity Run 2026 arrangeras av H-DCS.\n\nI år prövar vi samma plats som ligger i Norsjövallen för vår gemensamma samlingspunkt i länet.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Monikas Café, Norsjö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-charity-run-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Monikas+Caf%C3%A9%2C+Norsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-27/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-06-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-27/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-06-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-i-charity-run-2026",
      "name": "DO-I Charity Run",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Västernorrland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Årets körning körs till gemensamt mål och startar på 6 olika platser. -Sundsvall. Avfärd 11:30 från Shell Bydalen.",
      "link": "https://hdcs.se/evenemang/do-i-charity-run-2/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "DO-I Charity Run arrangeras av H-DCS.\n\nÅrets körning körs till gemensamt mål och startar på 6 olika platser. -Sundsvall. Avfärd 11:30 från Shell Bydalen.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Västernorrland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-i-charity-run-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4sternorrland",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-06-27/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-06-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-tur-fran-bromolla-2026",
      "name": "D15 Tur från Bromölla",
      "date": "2026-06-27",
      "dateEnd": "2026-06-27",
      "location": "Bromölla",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D15 från Bromölla. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-tur-fran-bromolla-2/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Tur från Bromölla arrangeras av BMW MC-klubben.\n\nKörning med D15 från Bromölla. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Bromölla.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-tur-fran-bromolla-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Brom%C3%B6lla",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-b2b-back-to-basic-2026",
      "name": "D15 B2b (back to basic)",
      "date": "2026-06-27",
      "dateEnd": "2026-06-28",
      "location": "Frostavallsvägen 2, Höör",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Frostavallsvägen 2, 243 32 Höör, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-b2b-back-to-basic/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 B2b (back to basic) arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Frostavallsvägen 2, 243 32 Höör, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Frostavallsvägen 2, Höör.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-b2b-back-to-basic/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Frostavallsv%C3%A4gen+2%2C+H%C3%B6%C3%B6r",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-06-29",
      "dateEnd": "2026-06-29",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "OBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-29/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nOBS ny samlingsplats, bortre parkeringen på McDonalds Nacka Välkomna på måndagskörning, OBS ny samlingsplats.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-3/2026-06-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-06-30",
      "dateEnd": "2026-06-30",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-30/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-06-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-boras-tisdags-körningar-2026",
      "name": "LO-C Borås tisdags körningar.",
      "date": "2026-06-30",
      "dateEnd": "2026-06-30",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.",
      "link": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-30/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Borås tisdags körningar. arrangeras av H-DCS.\n\nVälkomna på tisdags körning LO Paula guidar oss på en tur. Samling kl. 17:20 Avfärd kl. 17.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-boras-tisdags-körningar/2026-06-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-06-30",
      "dateEnd": "2026-06-30",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-06-30/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor/2026-06-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "smt-gotland-2026",
      "name": "Gotland MC-tur",
      "date": "2026-06-30",
      "dateEnd": "2026-07-03",
      "location": "Nynäshamn",
      "type": "Körning",
      "organizer": "Swedish MC Touring",
      "description": "Guidad 4-dagarstur till Gotland via Nynäshamn. Fina MC-vägar, kultur, raukar och sevärdheter. Hotell, middag och färja ingår. 9 850 kr.",
      "descriptionFull": "Den här 4-dagarsturen ger dig 4 härliga dagar på Gotland. Sveriges största ö bjuder på fina MC-vägar och massor av kultur.\n\nHär får vi njuta av härliga vägar, vackra vyer och intressanta sevärdheter. Självklart bor vi bra och äter gott.\n\nDagsetapperna ligger på 7-25 mil. Du får bland annat uppleva: Gotlands regemente, ett försvarsmuseum, unika kyrkor, raukar från Hoburgen till Fårö, ett båtmotormuseum och den speciella naturen på Gotland.\n\nStartar: Tisdag 30 juni, kl 9.00 vid norra infarten till Nynäshamn, längs väg 73.\nSlutar: Fredag 3 juli, ca kl 20.30 vid färjeterminalen i Nynäshamn.\n\nI priset ingår: Hotell 3 nätter (dubbelrum), halvpension med två-rättersmiddag och frukost, färja Nynäshamn-Visby ToR, inträden och guidning.\n\nResans pris: 9 850 SEK (passagerare 9 350 SEK). Enkelrumstillägg 1 600 SEK.",
      "links": [
        {
          "label": "swedishmctouring.se",
          "url": "https://www.swedishmctouring.se/gotland-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nynäshamn+station",
          "type": "map"
        }
      ],
      "link": "https://www.swedishmctouring.se/gotland-2026/",
      "region": "Gotland",
      "source": "swedishmctouring.se"
    },
    {
      "id": "doa-sommarkvallstur-nacka-2026",
      "name": "DOA - Sommarkvällstur Nacka",
      "date": "2026-07-01",
      "dateEnd": "2026-07-01",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "LO Göran tar oss med på en sommarkvällstur Samling kl. 17:30, Samlingsplats, bortre parkeringen på McDonalds Nacka Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-sommarkvallstur-nacka/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Sommarkvällstur Nacka arrangeras av H-DCS.\n\nLO Göran tar oss med på en sommarkvällstur Samling kl. 17:30, Samlingsplats, bortre parkeringen på McDonalds Nacka Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-sommarkvallstur-nacka/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "kvallsstraff-smtt-steno-2026",
      "name": "Kvällsträff SMTT Stenö",
      "date": "2026-07-01",
      "dateEnd": "2026-07-01",
      "location": "Larsbo, Stenö, Sandarne",
      "type": "Träff",
      "organizer": "SMTT Söderhamns MC Touring Team",
      "description": "Träffpunkt i Larsbo, Stenö, Sandarne. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://smtt.se/",
      "region": "Gävleborg",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Kvällsträff SMTT Stenö arrangeras av SMTT Söderhamns MC Touring Team.\n\nTräffpunkt i Larsbo, Stenö, Sandarne. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Larsbo, Stenö, Sandarne.",
      "links": [
        {
          "label": "smtt.se",
          "url": "https://smtt.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Larsbo%2C+Sten%C3%B6%2C+Sandarne",
          "type": "map"
        }
      ]
    },
    {
      "id": "vasteras-summermeet-2026",
      "name": "Västerås Summer Meet",
      "date": "2026-07-02",
      "dateEnd": "2026-07-04",
      "location": "Västerås",
      "type": "Träff",
      "organizer": "Västerås Summer Meet",
      "description": "Stort bil- och mc-evenemang mitt i sommaren. Cruising, utställningar och livemusik.",
      "link": "https://vasterassummermeet.se/",
      "region": "Västmanland",
      "source": "vasterassummermeet.se",
      "descriptionFull": "Västerås Summer Meet arrangeras av Västerås Summer Meet.\n\nStort bil- och mc-evenemang mitt i sommaren. Cruising, utställningar och livemusik.\n\nMC-show med utsmyckade hojar och byggen. Kolla arrangören för tider.\n\nPlats: Västerås.",
      "links": [
        {
          "label": "vasterassummermeet.se",
          "url": "https://vasterassummermeet.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ster%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "alandstraffen-2026",
      "name": "Ålandsträffen",
      "date": "2026-07-02",
      "dateEnd": "2026-07-05",
      "location": "Gröna Uddens Camping, Åland",
      "type": "Träff",
      "organizer": "SMC",
      "description": "Arrangemang med SMC i Gröna Uddens Camping, Åland. Mötespunkt för MC-klubben. Mer information på smc.nu.",
      "link": "https://www.svmc.se/",
      "region": "Finland",
      "source": "svmc.se",
      "descriptionFull": "Ålandsträffen arrangeras av SMC.\n\nArrangemang med SMC i Gröna Uddens Camping, Åland. Mötespunkt för MC-klubben. Mer information på smc.nu.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Gröna Uddens Camping, Åland.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gr%C3%B6na+Uddens+Camping%2C+%C3%85land",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-07-02",
      "dateEnd": "2026-07-02",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-02/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "gotland-ring-bikeweek-2026",
      "name": "Gotland Ring Bike Week",
      "date": "2026-07-03",
      "dateEnd": "2026-07-05",
      "location": "Gotland Ring, Gotland",
      "type": "Racing",
      "organizer": "Gotland Ring",
      "description": "Tre dagar på Gotlands racingbana. Bankörning, provkörningar och mc-festival på ön.",
      "link": "https://www.alltommc.se/gotland-ring-bike-week-2026-early-bird-erbjudande/",
      "region": "Gotland",
      "source": "alltommc.se",
      "descriptionFull": "Gotland Ring Bike Week arrangeras av Gotland Ring.\n\nTre dagar på Gotlands racingbana. Bankörning, provkörningar och mc-festival på ön.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Gotland Ring, Gotland.",
      "links": [
        {
          "label": "alltommc.se",
          "url": "https://www.alltommc.se/gotland-ring-bike-week-2026-early-bird-erbjudande/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gotland+Ring%2C+Gotland",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-enuddetraffen-oland-3-5-juli-2026",
      "name": "DOE Enuddeträffen Öland 3-5 juli",
      "date": "2026-07-03",
      "dateEnd": "2026-07-05",
      "location": "Öland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Enuddenträffen Öland Kalmar Harley Club 3-5 juli 2026 Från kl 12.00 fredag till söndag kl 12.00 Träffavgift: 350 kr inkl frukost.",
      "link": "https://hdcs.se/evenemang/doe-enuddetraffen-oland-3-5-juli/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Enuddeträffen Öland 3-5 juli arrangeras av H-DCS.\n\nEnuddenträffen Öland Kalmar Harley Club 3-5 juli 2026 Från kl 12.00 fredag till söndag kl 12.00 Träffavgift: 350 kr inkl frukost.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Öland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-enuddetraffen-oland-3-5-juli/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "torringelund-bike-meet-2026",
      "name": "Törringelund Bike Meet 2026",
      "date": "2026-07-03",
      "dateEnd": "2026-07-04",
      "location": "Hyltarpsvägen 771, Svedala",
      "type": "Fest",
      "organizer": "Törringelund",
      "description": "Ny MC-festival i Skåne! Två dagar med MC-utställning, liveband, foodtrucks, marknad, camping och fri entré för barn under 15.",
      "descriptionFull": "Törringelund bjuder in till en helt ny festival med fullt fokus på motorcyklar, livemusik, camping och bikerkultur.\n\n3-4 juli, Törringelund, Skåne.\n\nTvå dagar fyllda med:\n- MC-utställning\n- Flera liveband och olika musikstilar\n- Foodtrucks och barer\n- Marknad och knallare\n- Camping på området\n\nSommarkvällar, motorljud och gemenskap. Alla är välkomna redan på torsdagen, campingen är öppen och puben är laddad.\n\nBarn upp till 15 år: fri entré.\n\nBiljetter via nortic.se.\n\nKontakt: event@torringelund.se\n\nPlats: Hyltarpsvägen 771, 233 92 Svedala.",
      "backImage": "ads/torringelund-bike-meet-back-2026-07-03.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1148594720484480/"
        },
        {
          "label": "Biljetter",
          "url": "https://www.nortic.se/ticket/event/78262"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=T%C3%B6rringelund+Svedala",
          "type": "map"
        }
      ],
      "link": "https://www.nortic.se/ticket/event/78262",
      "region": "Skåne",
      "source": "FB"
    },
    {
      "id": "45an-hoting-2026",
      "name": "45:an",
      "date": "2026-07-03",
      "dateEnd": "2026-07-05",
      "location": "Bredviksnäset, Höting",
      "type": "Träff",
      "organizer": "DMCK Fridas",
      "description": "45:an, en träff med underbara människor och otrolig atmosfär. Levande musik, matservering.",
      "link": "https://dmck-fridas.se/",
      "region": "Jämtland",
      "source": "smcboken.svmc.se",
      "descriptionFull": "45:an arrangeras av DMCK Fridas.\n\n45:an, en träff med underbara människor och otrolig atmosfär. Levande musik, matservering.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Bredviksnäset, Höting.",
      "links": [
        {
          "label": "dmck-fridas.se",
          "url": "https://dmck-fridas.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bredviksn%C3%A4set%2C+H%C3%B6ting",
          "type": "map"
        }
      ]
    },
    {
      "id": "vanern-runt-2026",
      "name": "Vänern runt",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Vänern",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Vänern. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/vanern-runt/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Vänern runt arrangeras av H-DCS.\n\nArrangemang med H-DCS från Vänern. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vänern.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/vanern-runt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4nern",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-charity-run-2026",
      "name": "DOA - Charity Run",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Stockholm. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-charity-run-2/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Charity Run arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Stockholm. Gemensamt MC-event för gemenskap och underhållning. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-charity-run-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-cherity-run-2026",
      "name": "DOG  Cherity Run",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Mora",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Västerdalarna: Stefan kör från Frendo i Malng Micke kör från MC Dolnad Mora Hudiksvall Lasse och Anders Sandviken.",
      "link": "https://hdcs.se/evenemang/dog-cherity-run/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  Cherity Run arrangeras av H-DCS.\n\nVästerdalarna: Stefan kör från Frendo i Malng Micke kör från MC Dolnad Mora Hudiksvall Lasse och Anders Sandviken.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Mora.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-cherity-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Mora",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-04/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-04/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-vattern-runt-fran-askersund-en-svensk-mc-klassiker-2026",
      "name": "Vättern Runt från Askersund - En svensk mc-klassiker",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Klockaregatan, Askersund",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Klockaregatan, 696 32 Askersund, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/vattern-runt-fran-askersund-en-svensk-mc-klassiker/",
      "region": "Örebro",
      "source": "bmwklubben.se",
      "descriptionFull": "Vättern Runt från Askersund - En svensk mc-klassiker arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Klockaregatan, 696 32 Askersund, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Klockaregatan, Askersund.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/vattern-runt-fran-askersund-en-svensk-mc-klassiker/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Klockaregatan%2C+Askersund",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-vattern-runt-fran-jonkoping-en-svensk-mc-klassiker-2026",
      "name": "Vättern Runt från Jönköping - En svensk mc-klassiker",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Birkagatan 8, Jönköping",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Birkagatan 8, 554 65 Jönköping, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/vattern-runt-fran-jonkoping-en-svensk-mc-klassiker/",
      "region": "Jönköping",
      "source": "bmwklubben.se",
      "descriptionFull": "Vättern Runt från Jönköping - En svensk mc-klassiker arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Birkagatan 8, 554 65 Jönköping, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Birkagatan 8, Jönköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/vattern-runt-fran-jonkoping-en-svensk-mc-klassiker/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Birkagatan+8%2C+J%C3%B6nk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-udda-platser-i-skane-smakrunda-pa-osterlen-2026",
      "name": "D15 Udda platser i Skåne: Smakrunda på Österlen",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Sellerigatan 7, Lund",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Sellerigatan 7, 224 78 Lund, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-udda-matplatser/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Udda platser i Skåne: Smakrunda på Österlen arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Sellerigatan 7, 224 78 Lund, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sellerigatan 7, Lund.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-udda-matplatser/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sellerigatan+7%2C+Lund",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-dagtouring-jul-2026",
      "name": "ÖAMCK Dagtouring",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Lelles MC, Uppsala",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/dagtouring-3/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Dagtouring arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lelles MC, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/dagtouring-3/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lelles+MC%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "lag-sm-mx-helsingborg-2026",
      "name": "Lag-SM & SM Motocross Sprint - Helsingborg",
      "date": "2026-07-04",
      "dateEnd": "2026-07-05",
      "location": "Helsingborgs MCK, Helsingborg",
      "type": "Racing",
      "organizer": "SVEMO / Helsingborgs MCK",
      "description": "Lag-SM i Motocross (4 jul) och SM Motocross Sprint Open & Women (5 jul). MX-Women deltävling ingår.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Skåne",
      "source": "svemo.se",
      "descriptionFull": "Lag-SM & SM Motocross Sprint - Helsingborg arrangeras av SVEMO / Helsingborgs MCK.\n\nLag-SM i Motocross (4 jul) och SM Motocross Sprint Open & Women (5 jul). MX-Women deltävling ingår.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Helsingborgs MCK, Helsingborg.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Helsingborgs+MCK%2C+Helsingborg",
          "type": "map"
        }
      ]
    },
    {
      "id": "mini-jumbo-i-klippan-2026",
      "name": "Mini Jumbo i Klippan",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Sågen Möjligheternas hus, Klippan",
      "type": "Träff",
      "organizer": "Svenska Sidvagnsklubben SVEA",
      "description": "Miniträff där funktionsnedsatta personer får åka i sidvagnar. Tält och husbilsparkering.",
      "link": "https://www.sidvagn.se/",
      "region": "Skåne",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Mini Jumbo i Klippan arrangeras av Svenska Sidvagnsklubben SVEA.\n\nMiniträff där funktionsnedsatta personer får åka i sidvagnar. Tält och husbilsparkering.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sågen Möjligheternas hus, Klippan.",
      "links": [
        {
          "label": "sidvagn.se",
          "url": "https://www.sidvagn.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=S%C3%A5gen+M%C3%B6jligheternas+hus%2C+Klippan",
          "type": "map"
        }
      ]
    },
    {
      "id": "iron-horse-mc-show-nykoping-2026",
      "name": "Iron Horse MC Show Nyköping",
      "date": "2026-07-04",
      "dateEnd": "2026-07-04",
      "location": "Nyköpings Hamn, Nyköping",
      "type": "Show",
      "organizer": "Iron Horse MC Show",
      "description": "Årlig MC-show i Nyköpings Hamn. Custombyggen, klassiska hojar och gemenskap i sommarens Nyköping.",
      "descriptionFull": "Iron Horse MC Show i Nyköping den 4 juli 2026.\n\nÅrlig MC-show som hålls lördagen efter midsommar i Nyköpings Hamn. Custombyggen, klassiska motorcyklar och gemenskap.\n\nÖppet från kl 11:00.\n\nPlats: Nyköpings Hamn, Nyköping.",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/ironhorsemcshow/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nyk%C3%B6pings+Hamn",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/ironhorsemcshow/",
      "region": "Södermanland",
      "source": "glinder.se"
    },
    {
      "id": "do-c-lars-eriks-glassrunda-2026",
      "name": "DO-C Lars-Eriks glassrunda",
      "date": "2026-07-05",
      "dateEnd": "2026-07-05",
      "location": "Halland/VGötaland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-lars-eriks-glassrunda/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Lars-Eriks glassrunda arrangeras av H-DCS.\n\nArrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Halland/VGötaland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-lars-eriks-glassrunda/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halland%2FVG%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-nordic-run-2026",
      "name": "DOB- Nordic Run",
      "date": "2026-07-07",
      "dateEnd": "2026-07-07",
      "location": "Skåne",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Nordic Run 2026 2026-07-07 Samling och start vid Mc Donald Lockarp. Ligger c:a 8 km efter betalstationen avfart Lockarp. Starten går kl.",
      "link": "https://hdcs.se/evenemang/dob-nordic-run/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Nordic Run arrangeras av H-DCS.\n\nNordic Run 2026 2026-07-07 Samling och start vid Mc Donald Lockarp. Ligger c:a 8 km efter betalstationen avfart Lockarp. Starten går kl.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-nordic-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-nordic-run-genom-distrikt-e-7-8-juli-2026",
      "name": "DOE Nordic run genom Distrikt-E 7-8 juli",
      "date": "2026-07-07",
      "dateEnd": "2026-07-08",
      "location": "Småland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-nordic-run-genom-distrikt-e-7-8-juli/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Nordic run genom Distrikt-E 7-8 juli arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-nordic-run-genom-distrikt-e-7-8-juli/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-07-07",
      "dateEnd": "2026-07-07",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-07/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-sommarkvallstur-taby-2026",
      "name": "DOA - Sommarkvällstur Täby",
      "date": "2026-07-08",
      "dateEnd": "2026-07-08",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "LO Martin tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-sommarkvallstur-taby/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Sommarkvällstur Täby arrangeras av H-DCS.\n\nLO Martin tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-sommarkvallstur-taby/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "stiligaste-hojen-akers-styckebruk-2026",
      "name": "Stiligaste Hojen",
      "date": "2026-07-08",
      "dateEnd": "2026-07-08",
      "location": "Solbergavagen 28, Akers Styckebruk",
      "type": "Träff",
      "organizer": "Persakers Speed Shop AB",
      "description": "Kvällens tema är Stiligaste Hojen! Motorcykel med valfritt antal hjul tävlar om kvällens pris. Fräna Fiket serverar korv och fika.",
      "descriptionFull": "Persåkers Speed Shop arrangerar Stiligaste Hojen den 8 juli.\n\nKvällens tema är Stiligaste Hojen. Motorcykel med valfritt antal hjul tävlar om kvällens pris!\n\nFräna Fiket serverar korv och fika och är öppet som vanligt 16:00-19:00. Prisutdelning ca 18:15.\n\nTid: 16:00-19:00\nLängd: ca 3 timmar\n\nVälkommen!\n\nPlats: Solbergavägen 28, 64752 Åkers Styckebruk.",
      "backImage": "ads/stiligaste-hojen-back-2026-07-08.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/3895905843875566/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Solbergav%C3%A4gen+28%2C+64752+%C3%85kers+Styckebruk",
          "type": "map"
        }
      ],
      "link": "",
      "region": "Södermanland",
      "source": "FB"
    },
    {
      "id": "do-c-gemensam-körning-till-bornholm-2026",
      "name": "DO-C Gemensam körning till Bornholm",
      "date": "2026-07-09",
      "dateEnd": "2026-07-09",
      "location": "Bornholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Bornholm. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-gemensam-körning-till-bornholm/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Gemensam körning till Bornholm arrangeras av H-DCS.\n\nKörtur med H-DCS från Bornholm. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Bornholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-gemensam-körning-till-bornholm/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bornholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-07-09",
      "dateEnd": "2026-07-09",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-07-09/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-07-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-07-09",
      "dateEnd": "2026-07-09",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-09/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-smalandshelg-med-grand-prix-speedway-i-malilla-2026",
      "name": "D15 Smålandshelg med Grand prix speedway i Målilla",
      "date": "2026-07-09",
      "dateEnd": "2026-07-12",
      "location": "Frostavallsvägen 2, Höör",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Frostavallsvägen 2, 243 32 Höör, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-smalandshelg-med-grand-prix-speedway-i-malilla/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Smålandshelg med Grand prix speedway i Målilla arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Frostavallsvägen 2, 243 32 Höör, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Frostavallsvägen 2, Höör.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-smalandshelg-med-grand-prix-speedway-i-malilla/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Frostavallsv%C3%A4gen+2%2C+H%C3%B6%C3%B6r",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-smygeturen-2026",
      "name": "D15 Smygeturen",
      "date": "2026-07-09",
      "dateEnd": "2026-07-09",
      "location": "Bökeberg, Svedala",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Kvällstur till Sveriges sydligaste plats. Samling vid Bökeberg Södra parkering, väg 108 mellan Klågerup och Svedala.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-smygeturen/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Smygeturen arrangeras av BMW MC-klubben.\n\nKvällstur till Sveriges sydligaste plats. Samling vid Bökeberg Södra parkering, väg 108 mellan Klågerup och Svedala.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Bökeberg, Svedala.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-smygeturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=B%C3%B6keberg%2C+Svedala",
          "type": "map"
        }
      ]
    },
    {
      "id": "bornholms-traffen-2026",
      "name": "Bornholmertræffet 2026",
      "date": "2026-07-10",
      "dateEnd": "2026-07-12",
      "location": "Skrædderbakkevejen 2, Rønne, Bornholm",
      "type": "Fest",
      "organizer": "H-D Club Denmark Distrikt 6",
      "description": "Harley-träff på Bornholm! Camping från fredag, livemusik, aftensmad, morgenmad och rökt sild vid fällessturen.",
      "link": "https://hdcs.se/evenemang/bornholms-traffen/",
      "region": "Danmark",
      "source": "FB",
      "descriptionFull": "Harley-Davidson Club of Denmark Distrikt 6, Bornholm bjuder in till Bornholmertræffet 2026!\n\n10-12 juli, Rønne, Bornholm, Danmark.\n\nHusk medlemskort! Icke-medlemmar betalar entré.\n\nI entréen ingår:\n- Aftensmad fredag och lördag\n- Morgenmad lördag och söndag\n- Camping från fredag\n- Livemusik\n- Öl/vand och rökt sild vid fällessturen\n- Gode toilet och badeforhold\n\nDer er live musik i det store telt fra torsdag.\n\nPlats: Skrædderbakkevejen 2, 3700 Rønne, Danmark.",
      "backImage": "ads/bornholmertraeffet-back-2026-07-10.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/895791316440713/"
        },
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/bornholms-traffen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skr%C3%A6dderbakkevejen+2+R%C3%B8nne+Bornholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-fredagstur-med-nordic-run-2026",
      "name": "DOA - Fredagstur med Nordic Run",
      "date": "2026-07-10",
      "dateEnd": "2026-07-10",
      "location": "Mels garage, Enköping",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kommer ledsaga Nordic run genom distrikt A via; Enköping - Sala silvergruva - Vikingabyn Storholmen - Restaurang Havspiren.",
      "link": "https://hdcs.se/evenemang/doa-fredagstur-med-nordic-run/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Fredagstur med Nordic Run arrangeras av H-DCS.\n\nVi kommer ledsaga Nordic run genom distrikt A via; Enköping - Sala silvergruva - Vikingabyn Storholmen - Restaurang Havspiren.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Mels garage, Enköping.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-fredagstur-med-nordic-run/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Mels+garage%2C+Enk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-07-11",
      "dateEnd": "2026-07-11",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-11/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-07-11",
      "dateEnd": "2026-07-11",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-11/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-07-11",
      "dateEnd": "2026-07-11",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-07-11/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-07-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-grillfest-2026",
      "name": "ÖAMCK Grillfest",
      "date": "2026-07-11",
      "dateEnd": "2026-07-11",
      "location": "Stangby, Uppsala",
      "type": "Träff",
      "organizer": "ÖAMCK",
      "description": "Arrangemang med ÖAMCK i Stangby, Uppsala. Gemensamt event för MC-entusiaster. Mer info på oamck.se.",
      "link": "https://oamck.se/events/grillfest-2/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Grillfest arrangeras av ÖAMCK.\n\nArrangemang med ÖAMCK i Stangby, Uppsala. Gemensamt event för MC-entusiaster. Mer info på oamck.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stangby, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/grillfest-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stangby%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "karlstad-varmlandskransen-2026",
      "name": "Varmlandskransen MC-treff",
      "date": "2026-07-11",
      "dateEnd": "2026-07-11",
      "location": "Mariebergsskogen, Karlstad",
      "type": "Träff",
      "organizer": "Klaralvens MC Forening",
      "description": "MC-arrangemang i Mariebergsskogen, Karlstad. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://druk.se/",
      "region": "Värmland",
      "source": "manual",
      "_canary": true,
      "descriptionFull": "Varmlandskransen MC-treff arrangeras av Klaralvens MC Forening.\n\nMC-arrangemang i Mariebergsskogen, Karlstad. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Mariebergsskogen, Karlstad.",
      "links": [
        {
          "label": "druk.se",
          "url": "https://druk.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Mariebergsskogen%2C+Karlstad",
          "type": "map"
        }
      ]
    },
    {
      "id": "mxsm-varberg-2026",
      "name": "MXSM Deltävling 4 - Varberg",
      "date": "2026-07-12",
      "dateEnd": "2026-07-12",
      "location": "Varbergs MK, Varberg",
      "type": "Racing",
      "organizer": "SVEMO / Varbergs MK",
      "description": "Deltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Halland",
      "source": "svemo.se",
      "descriptionFull": "MXSM Deltävling 4 - Varberg arrangeras av SVEMO / Varbergs MK.\n\nDeltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Varbergs MK, Varberg.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Varbergs+MK%2C+Varberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-07-14",
      "dateEnd": "2026-07-14",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-sommarkvallstur-ekero-2026",
      "name": "DOA - Sommarkvällstur Ekerö",
      "date": "2026-07-15",
      "dateEnd": "2026-07-15",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "DO/LO Lennart tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-sommarkvallstur-ekero/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Sommarkvällstur Ekerö arrangeras av H-DCS.\n\nDO/LO Lennart tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-sommarkvallstur-ekero/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-07-16",
      "dateEnd": "2026-07-16",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-16/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "hdcs-rally-2026",
      "name": "H-DCS Rally",
      "date": "2026-07-17",
      "dateEnd": "2026-07-19",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "H-DCS årliga rally. Harley-ägare från hela Sverige samlas för sommarens stora händelse.",
      "link": "https://hdcs.se/evenemang/",
      "region": "Sverige",
      "source": "hdcs.se",
      "descriptionFull": "H-DCS Rally arrangeras av H-DCS.\n\nH-DCS årliga rally. Harley-ägare från hela Sverige samlas för sommarens stora händelse.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "hemlig-tur-8230-2026",
      "name": "Hemlig tur...?",
      "date": "2026-07-17",
      "dateEnd": "2026-07-18",
      "location": "Hemligt",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Hemligt. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/hemlig-tur/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Hemlig tur...? arrangeras av H-DCS.\n\nKörtur med H-DCS från Hemligt. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Hemligt.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/hemlig-tur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hemligt",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-hoglandsturen-med-jonas-lager-17-juli-2026",
      "name": "DOE Höglandsturen med Jonas Lager 17 juli",
      "date": "2026-07-17",
      "dateEnd": "2026-07-17",
      "location": "Småland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOE från Småland. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-hoglandsturen-med-jonas-lager-17-juli/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Höglandsturen med Jonas Lager 17 juli arrangeras av H-DCS.\n\nKörtur med H-DCS DOE från Småland. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-hoglandsturen-med-jonas-lager-17-juli/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-bergsjotraffen-2026",
      "name": "DOG Bergsjöträffen",
      "date": "2026-07-17",
      "dateEnd": "2026-07-19",
      "location": "Bergsjö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Bergsjö. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-bergsjotraffen/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG Bergsjöträffen arrangeras av H-DCS.\n\nTräffpunkt för DOG i Bergsjö. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Bergsjö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-bergsjotraffen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bergsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "wheelswings-falkenberg-2026",
      "name": "Wheels & Wings 2026",
      "date": "2026-07-17",
      "dateEnd": "2026-07-18",
      "location": "Falkenbergs motorbana, Falkenberg",
      "type": "Fest",
      "organizer": "Wheels & Wings",
      "description": "En av Sveriges största bil- och mcträffar med utställning, swapmeet och flyguppvisning på Falkenbergs motorbana. Tva dagar fulla av motor och gemenskap.",
      "descriptionFull": "Wheels & Wings är en av Sveriges största bil- och mcträffar och arrangeras på Falkenbergs motorbana i Halland.\n\nEventet pågår i två dagar med utställning, swapmeet och flyguppvisning. Här samlas entusiaster från hela landet för att visa upp sina fordon och njuta av gemenskapen.\n\nFredag 17 juli: kl 8-17\nLördag 18 juli: kl 8-18\n\nEntré: 200 kr/dag/person.\n\nAll information finns på wheelsandwings.se.\n\nPlats: Falkenbergs Motorbana, Hallarp, SE-311 95 Falkenberg.",
      "backImage": "ads/wheelswings-back-2026-07-17.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1886493125306984/"
        },
        {
          "label": "wheelsandwings.se",
          "url": "https://www.wheelsandwings.se"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Falkenbergs+motorbana+Falkenberg",
          "type": "map"
        }
      ],
      "link": "https://www.wheelsandwings.se",
      "region": "Halland",
      "source": "FB"
    },
    {
      "id": "gotlands-mc-klubbs-campingtraff-2026",
      "name": "Gotlands MC klubbs campingträff",
      "date": "2026-07-17",
      "dateEnd": "2026-07-19",
      "location": "GMCK Klubbstuga, Västerhejde, Gotland",
      "type": "Träff",
      "organizer": "Gotlands MC klubb",
      "description": "Träffpunkt i GMCK Klubbstuga, Västerhejde, Gotland. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://www.gmck.se/",
      "region": "Gotland",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Gotlands MC klubbs campingträff arrangeras av Gotlands MC klubb.\n\nTräffpunkt i GMCK Klubbstuga, Västerhejde, Gotland. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: GMCK Klubbstuga, Västerhejde, Gotland.",
      "links": [
        {
          "label": "gmck.se",
          "url": "https://www.gmck.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=GMCK+Klubbstuga%2C+V%C3%A4sterhejde%2C+Gotland",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-07-18",
      "dateEnd": "2026-07-18",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-18/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-07-18",
      "dateEnd": "2026-07-18",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-18/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-tandstick-motormuseum-2026",
      "name": "D14-Tändstick & Motormuseum",
      "date": "2026-07-18",
      "dateEnd": "2026-07-19",
      "location": "Jönköping",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D14 från Jönköping. Besök samt MC-körning. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-tandsticksfabriken/",
      "region": "Jönköping",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-Tändstick & Motormuseum arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D14 från Jönköping. Besök samt MC-körning. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Jönköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-tandsticksfabriken/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=J%C3%B6nk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-emilsturen-2026",
      "name": "DOA - Emilsturen",
      "date": "2026-07-19",
      "dateEnd": "2026-07-19",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-emilsturen/2026-07-19/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Emilsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-emilsturen/2026-07-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-07-21",
      "dateEnd": "2026-07-21",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-21/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-fikatraff-med-classic-pa-nykvarns-slusscaf-alla-ar-2026",
      "name": "Fikaträff med Classic på Nykvarns Slusscafé (alla är välkomna)",
      "date": "2026-07-21",
      "dateEnd": "2026-07-21",
      "location": "Nykvarns Slusscafé, Linköping",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Fikaträff vid Nykvarns Slusscafé i Linköping. Kolla in fina BMW-hojar, ta en fika och umgås med klubbkompisar.",
      "link": "https://www.bmwklubben.se/aktiviteter/fikatur-med-classic-till-nykvarns-slusscafe-alla-ar-valkomna/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "Fikaträff med Classic på Nykvarns Slusscafé (alla är välkomna) arrangeras av BMW MC-klubben.\n\nFikaträff vid Nykvarns Slusscafé i Linköping. Kolla in fina BMW-hojar, ta en fika och umgås med klubbkompisar.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nykvarns Slusscafé, Linköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/fikatur-med-classic-till-nykvarns-slusscafe-alla-ar-valkomna/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nykvarns+Slusscaf%C3%A9%2C+Link%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-sommarkvallstur-skogskyrkogarden-2026",
      "name": "DOA - Sommarkvällstur Skogskyrkogården",
      "date": "2026-07-22",
      "dateEnd": "2026-07-22",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "LO Petra tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/57263/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Sommarkvällstur Skogskyrkogården arrangeras av H-DCS.\n\nLO Petra tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/57263/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "hojrock-2026",
      "name": "HojRock",
      "date": "2026-07-23",
      "dateEnd": "2026-07-26",
      "location": "Tånga Hed, Vårgårda",
      "type": "Fest",
      "organizer": "HojRock",
      "description": "En av Nordens största MC-träffar. Livemusik, camping och mc-gemenskap på Tånga Hed.",
      "link": "https://www.hojrock.se/",
      "region": "Västra Götaland",
      "source": "hojrock.se",
      "descriptionFull": "HojRock arrangeras av HojRock.\n\nEn av Nordens största MC-träffar. Livemusik, camping och mc-gemenskap på Tånga Hed.\n\nFest och social sammankomst för MC-folket. Se arrangören för detaljer.\n\nPlats: Tånga Hed, Vårgårda.",
      "links": [
        {
          "label": "hojrock.se",
          "url": "https://www.hojrock.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=T%C3%A5nga+Hed%2C+V%C3%A5rg%C3%A5rda",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-07-23",
      "dateEnd": "2026-07-23",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-07-23/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-07-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-07-23",
      "dateEnd": "2026-07-23",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-23/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "roadracing-sm-gellerasen-2026",
      "name": "Roadracing SM - Gelleråsen",
      "date": "2026-07-24",
      "dateEnd": "2026-07-26",
      "location": "Gelleråsen, Karlskoga",
      "type": "Racing",
      "organizer": "MotoSE / Svemo",
      "description": "MC-arrangemang i Gelleråsen, Karlskoga. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://mcparken.se/calendar/176",
      "region": "Örebro",
      "source": "mcparken.se",
      "descriptionFull": "Roadracing SM - Gelleråsen arrangeras av MotoSE / Svemo.\n\nMC-arrangemang i Gelleråsen, Karlskoga. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Gelleråsen, Karlskoga.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/176"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Geller%C3%A5sen%2C+Karlskoga",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-skanetraffen-2-0-2026",
      "name": "DOB- Skåneträffen 2.0",
      "date": "2026-07-24",
      "dateEnd": "2026-07-26",
      "location": "Skåne",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOB i Skåne. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dob-skanetraffen-2-0-2/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Skåneträffen 2.0 arrangeras av H-DCS.\n\nTräffpunkt för DOB i Skåne. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-skanetraffen-2-0-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "knallletraffenen-boras-2026",
      "name": "Knalleträffen",
      "date": "2026-07-24",
      "dateEnd": "2026-07-26",
      "location": "Eagle Riders MC, Borås",
      "type": "Träff",
      "organizer": "Eagle Riders MC Borås",
      "description": "Träffpunkt i Eagle Riders MC, Borås. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "http://eagleriders.se/",
      "region": "Västra Götaland",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Knalleträffen arrangeras av Eagle Riders MC Borås.\n\nTräffpunkt i Eagle Riders MC, Borås. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Eagle Riders MC, Borås.",
      "links": [
        {
          "label": "eagleriders.se",
          "url": "http://eagleriders.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Eagle+Riders+MC%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "norge-tur-fran-trollhattan-2026",
      "name": "Norge-tur från Trollhättan",
      "date": "2026-07-25",
      "dateEnd": "2026-07-25",
      "location": "Trollhättan",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Trollhättan. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/norge-tur-fran-trollhattan/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Norge-tur från Trollhättan arrangeras av H-DCS.\n\nKörtur med H-DCS från Trollhättan. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Trollhättan.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/norge-tur-fran-trollhattan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Trollh%C3%A4ttan",
          "type": "map"
        }
      ]
    },
    {
      "id": "dagstur-med-distrikt-i-2026",
      "name": "Dagstur med distrikt I",
      "date": "2026-07-25",
      "dateEnd": "2026-07-25",
      "location": "Västernorrland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Dagstur till Trollhagens fiskerestaurang med distrikt I 25 juli Tider och samlingsplatser kommer senare Mer info: Annemona Andersson 070.",
      "link": "https://hdcs.se/evenemang/dagstur-med-distrikt-i/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "Dagstur med distrikt I arrangeras av H-DCS.\n\nDagstur till Trollhagens fiskerestaurang med distrikt I 25 juli Tider och samlingsplatser kommer senare Mer info: Annemona Andersson 070.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Västernorrland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dagstur-med-distrikt-i/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4sternorrland",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-07-25",
      "dateEnd": "2026-07-25",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-25/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-07-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-07-25",
      "dateEnd": "2026-07-25",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-25/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-07-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-07-25",
      "dateEnd": "2026-07-25",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-07-25/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-07-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-bynanders-2026",
      "name": "D14-Bynanders",
      "date": "2026-07-25",
      "dateEnd": "2026-07-25",
      "location": "Preem, Falkenberg",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Tur på slingriga vägar från Falkenberg till Bynanders Motormuseum i Svenljunga. Lunch efter museibesöket.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-bynanders/",
      "region": "Halland",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-Bynanders arrangeras av BMW MC-klubben.\n\nTur på slingriga vägar från Falkenberg till Bynanders Motormuseum i Svenljunga. Lunch efter museibesöket.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Preem, Falkenberg.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-bynanders/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Preem%2C+Falkenberg",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-07-28",
      "dateEnd": "2026-07-28",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-28/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-07-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-sommarkvallstur-nockeby-2026",
      "name": "DOA - Sommarkvällstur Nockeby",
      "date": "2026-07-29",
      "dateEnd": "2026-07-29",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "LO Michael tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-sommarkvallstur-nockeby/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Sommarkvällstur Nockeby arrangeras av H-DCS.\n\nLO Michael tar oss med på en sommarkvällstur Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-sommarkvallstur-nockeby/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "treffen-norway-gwef-2026",
      "name": "Treffen Norway (GWEF)",
      "date": "2026-07-30",
      "dateEnd": "2026-08-02",
      "location": "Namsos, Norge",
      "type": "Träff",
      "organizer": "Gold Wing Club Norway / GWEF",
      "description": "MC-arrangemang i Namsos, Norge. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://gwef.eu/index.php/2026",
      "region": "Norge",
      "source": "gwef.eu",
      "descriptionFull": "Treffen Norway (GWEF) arrangeras av Gold Wing Club Norway / GWEF.\n\nMC-arrangemang i Namsos, Norge. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Namsos, Norge.",
      "links": [
        {
          "label": "gwef.eu",
          "url": "https://gwef.eu/index.php/2026"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Namsos%2C+Norge",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-07-30",
      "dateEnd": "2026-07-30",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-30/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-07-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tur-till-bisonturen-2026",
      "name": "DOA - Tur till Bisonturen",
      "date": "2026-07-31",
      "dateEnd": "2026-08-01",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-tur-till-bisonturen/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tur till Bisonturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tur-till-bisonturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "seskaro-2026",
      "name": "Seskarö",
      "date": "2026-07-31",
      "dateEnd": "2026-08-02",
      "location": "Seskarö Camping, Seskarö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från seskarö camping, Seskarö. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/seskaro/",
      "region": "Blekinge",
      "source": "hdcs.se",
      "descriptionFull": "Seskarö arrangeras av H-DCS.\n\nArrangemang med H-DCS från seskarö camping, Seskarö. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Seskarö Camping, Seskarö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/seskaro/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Seskar%C3%B6+Camping%2C+Seskar%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bisonturen-2026",
      "name": "Bisonturen",
      "date": "2026-08-01",
      "dateEnd": "2026-08-01",
      "location": "Heby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Heby. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/bisonturen/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Bisonturen arrangeras av H-DCS.\n\nKörtur med H-DCS från Heby. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Heby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/bisonturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Heby",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-08-01",
      "dateEnd": "2026-08-01",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-01/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-08-01",
      "dateEnd": "2026-08-01",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-01/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-skaneturen-2026",
      "name": "D14-Skåneturen",
      "date": "2026-08-01",
      "dateEnd": "2026-08-01",
      "location": "Laholm",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D14 från 312 34, Laholm, Hallands län, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-skaneturen/",
      "region": "Halland",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-Skåneturen arrangeras av BMW MC-klubben.\n\nKörning med D14 från 312 34, Laholm, Hallands län, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Laholm.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-skaneturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Laholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "blackngrey-summer-party-halmstad-2026",
      "name": "Black'n Grey Summer Party",
      "date": "2026-08-01",
      "dateEnd": "2026-08-01",
      "location": "Sadelvägen 720, Halmstad",
      "type": "Fest",
      "organizer": "Insane MC Halmstad",
      "description": "Insane MC Halmstad drar igang sin stora sommarfest igen. Fest, gemenskap och hojar i Tronninge utanfor Halmstad.",
      "descriptionFull": "Insane MC Halmstad bjuder in till Black'n Grey Summer Party den 1 augusti 2026.\n\nDags att dra igang den stora festen igen! Kvallen startar kl 18:00.\n\nPlats: Sadelvägen 720, Tronninge, Halmstad.",
      "backImage": "ads/blackngrey-summer-party-back-2026-08-01.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/2091545891645970/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sadelv%C3%A4gen+720+Halmstad",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/2091545891645970/",
      "region": "Halland",
      "source": "FB"
    },
    {
      "id": "nordic-wilderness-tour-2026",
      "name": "Nordic Wilderness Tour 2026",
      "date": "2026-08-02",
      "dateEnd": "2026-08-06",
      "location": "Norge - Sverige",
      "type": "Körning",
      "organizer": "GWEF Nordic",
      "description": "Gemensam körning från Norge - Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://gwef.eu/index.php/2026",
      "region": [
        "Norge",
        "Sverige"
      ],
      "source": "gwef.eu",
      "descriptionFull": "Nordic Wilderness Tour 2026 arrangeras av GWEF Nordic.\n\nGemensam körning från Norge - Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Norge - Sverige.",
      "links": [
        {
          "label": "gwef.eu",
          "url": "https://gwef.eu/index.php/2026"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Norge+-+Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-vilken-jakla-tur-2026",
      "name": "Vilken jäkla tur!",
      "date": "2026-08-02",
      "dateEnd": "2026-08-02",
      "location": "Hällingsjövägen 2, Hällingsjö",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Gemensam körning från Hällingsjövägen 2, 438 96 Hällingsjö, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/vilken-jakla-tur/",
      "region": "Västra Götaland",
      "source": "bmwklubben.se",
      "descriptionFull": "Vilken jäkla tur! arrangeras av BMW MC-klubben.\n\nGemensam körning från Hällingsjövägen 2, 438 96 Hällingsjö, Sverige. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Hällingsjövägen 2, Hällingsjö.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/vilken-jakla-tur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=H%C3%A4llingsj%C3%B6v%C3%A4gen+2%2C+H%C3%A4llingsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "gavle-norrlandscruisern-2026",
      "name": "Norrlandscruisern",
      "date": "2026-08-02",
      "dateEnd": "2026-08-02",
      "location": "Boulognerskogen, Gävle",
      "type": "Körning",
      "organizer": "Dalarna Riders MC",
      "description": "MC-arrangemang i Boulognerskogen, Gävle. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://druk.se/",
      "region": "Gävleborg",
      "source": "manual",
      "_canary": true,
      "descriptionFull": "Norrlandscruisern arrangeras av Dalarna Riders MC.\n\nMC-arrangemang i Boulognerskogen, Gävle. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Boulognerskogen, Gävle.",
      "links": [
        {
          "label": "druk.se",
          "url": "https://druk.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Boulognerskogen%2C+Gävle",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-08-03",
      "dateEnd": "2026-08-03",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-03/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "smt-vanern-runt-2026",
      "name": "Vänern runt MC-tur",
      "date": "2026-08-03",
      "dateEnd": "2026-08-08",
      "location": "Linköping",
      "type": "Körning",
      "organizer": "Swedish MC Touring",
      "description": "Guidad 6-dagarstur runt Vänern via Halland, Dalsland och Värmland. Fina MC-vägar, dagsetapper 21-30 mil. Hotell och middag ingår. 11 400 kr.",
      "descriptionFull": "En 6-dagars guidad MC-tur runt Vänern med start i Linköping.\n\nTuren går genom Halland, Dalsland och Värmland på utvalda MC-vägar. Dagsetapperna är mellan 21 och 30 mil.\n\nI priset 11 400 kr ingår 5 nätter på hotell, 5 middagar och en erfaren guide som leder gruppen.\n\nSamling i Linköping kl 12.15. Turen avslutas i Järna, sydost om Stockholm vid E4.\n\nMax 8 deltagare per grupp för bästa upplevelse.",
      "links": [
        {
          "label": "swedishmctouring.se",
          "url": "https://www.swedishmctouring.se/vanern-runt-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Linköping",
          "type": "map"
        }
      ],
      "link": "https://www.swedishmctouring.se/vanern-runt-2026/",
      "region": "Östergötland",
      "source": "swedishmctouring.se"
    },
    {
      "id": "smc-kalla-kriget-2026",
      "name": "SMC Travel: Kalla kriget",
      "date": "2026-08-03",
      "dateEnd": "2026-08-07",
      "location": "Haninge till Norrtälje (ca 100 mil)",
      "type": "Körning",
      "organizer": "SMC",
      "description": "Fem dagars MC-resa genom södra Mellansverige. Utforska Kalla krigets hemligheter via museer och försvarsanläggningar. Start Haninge, mål Norrtälje.",
      "descriptionFull": "SMC Travel: Kalla kriget den 3-7 augusti 2026.\n\nEn unik fem dagars MC-resa där spännande historia möter vackra vägar. Resan utforskar Kalla krigets hemligheter (ca 1945-1990) genom besök på utvalda museer och försvarsanläggningar.\n\nStart i Haninge, mål i Norrtälje. Totalt ca 100 mil.\n\nArrangeras av SMC Travel.\n\nPlats: Haninge till Norrtälje.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se/resor2026/kalla-kriget-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Haninge",
          "type": "map"
        }
      ],
      "link": "https://www.svmc.se/resor2026/kalla-kriget-2026/",
      "backImage": "ads/smc-kalla-kriget-back-2026-08-03.jpg",
      "region": "Stockholm",
      "source": "svmc.se"
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-08-04",
      "dateEnd": "2026-08-04",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-04/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, Jämna veckor",
      "date": "2026-08-04",
      "dateEnd": "2026-08-04",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-08-04/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, Jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-08-04/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-08-05",
      "dateEnd": "2026-08-05",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-05/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-08-05",
      "dateEnd": "2026-08-05",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-05/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "treffen-sweden-gwef-2026",
      "name": "Treffen Sweden (GWEF)",
      "date": "2026-08-06",
      "dateEnd": "2026-08-09",
      "location": "Orsa Gronklitt, Dalarna",
      "type": "Träff",
      "organizer": "GoldWing Club Sweden / GWEF",
      "description": "41st International Treffen. Orsa Gronklitt 6-9 augusti 2026. Traffavgift GWEF-medlem SEK700, ej medlem SEK850.",
      "link": "https://media.gwcs.se/2026/01/sv1.jpeg",
      "region": "Dalarna",
      "source": "gwef.eu",
      "frontImage": "ads/gwcs-treffen-2026-08-06.jpg",
      "descriptionFull": "Treffen Sweden (GWEF) arrangeras av GoldWing Club Sweden / GWEF.\n\n41st International Treffen. Orsa Gronklitt 6-9 augusti 2026. Traffavgift GWEF-medlem SEK700, ej medlem SEK850.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Orsa Gronklitt, Dalarna.",
      "links": [
        {
          "label": "media.gwcs.se",
          "url": "https://media.gwcs.se/2026/01/sv1.jpeg"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Orsa+Gronklitt%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "h-dcs-internationella-traffen-ostersund-2026",
      "name": "H-DCS Internationella Träffen Östersund",
      "date": "2026-08-06",
      "dateEnd": "2026-08-09",
      "location": "Östersund",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Östersund. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/h-dcs-internationella-traffen-ostersund/",
      "region": "Jämtland",
      "source": "hdcs.se",
      "descriptionFull": "H-DCS Internationella Träffen Östersund arrangeras av H-DCS.\n\nArrangemang med H-DCS från Östersund. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Östersund.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/h-dcs-internationella-traffen-ostersund/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96stersund",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-08-06",
      "dateEnd": "2026-08-06",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-06/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "h-dcs-internationella-traff-2026",
      "name": "H-DCS internationella träff",
      "date": "2026-08-06",
      "dateEnd": "2026-08-09",
      "location": "Östersund",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Östersund. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/h-dcs-internationella-traff/",
      "region": "Jämtland",
      "source": "hdcs.se",
      "descriptionFull": "H-DCS internationella träff arrangeras av H-DCS.\n\nArrangemang med H-DCS från Östersund. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Östersund.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/h-dcs-internationella-traff/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96stersund",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-08-06",
      "dateEnd": "2026-08-06",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-06/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-08-06",
      "dateEnd": "2026-08-06",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-06/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-08-06",
      "dateEnd": "2026-08-06",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-08-06/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-08-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-08-06",
      "dateEnd": "2026-08-06",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-06/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-kvallstur-fran-bromolla-2026",
      "name": "D15 Kvällstur från Bromölla",
      "date": "2026-08-06",
      "dateEnd": "2026-08-06",
      "location": "Bromölla",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D15 från Bromölla. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-kvallstur-fran-bromolla/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Kvällstur från Bromölla arrangeras av BMW MC-klubben.\n\nKörning med D15 från Bromölla. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Bromölla.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-kvallstur-fran-bromolla/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Brom%C3%B6lla",
          "type": "map"
        }
      ]
    },
    {
      "id": "british-motor-meet-hjo-2026",
      "name": "British Motor Meet Hjo",
      "date": "2026-08-07",
      "dateEnd": "2026-08-09",
      "location": "Hjo Stadspark, Hjo",
      "type": "Show",
      "organizer": "British Motor Meet",
      "description": "Sveriges största träff för klassiska brittiska fordon i Hjo Stadspark. Bilar och motorcyklar, musik, priser och ångbåtsturer på Vättern.",
      "descriptionFull": "British Motor Meet i Hjo den 7-9 augusti 2026.\n\nÅrlig träff för alla som älskar klassiska brittiska fordon. I Hjo Stadspark samlas bilar och motorcyklar från hela Sverige. Musik, prisutdelning och ångbåtsturer på Vättern.\n\nEventet hålls första helgen i augusti varje år och lockar entusiaster från hela landet.\n\nPlats: Hjo Stadspark, Hjo.",
      "links": [
        {
          "label": "britishmotormeet.se",
          "url": "https://britishmotormeet.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hjo+Stadspark+Hjo",
          "type": "map"
        }
      ],
      "link": "https://britishmotormeet.se/",
      "backImage": "ads/british-motor-meet-hjo-back-2026-08-07.jpg",
      "region": "Västra Götaland",
      "source": "britishmotormeet.se"
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-08-08",
      "dateEnd": "2026-08-08",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-08/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-08-08",
      "dateEnd": "2026-08-08",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-08/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-08-08",
      "dateEnd": "2026-08-08",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-08-08/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-08-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "smc-rebusrundan-borlange-2026",
      "name": "Rebusrundan",
      "date": "2026-08-08",
      "dateEnd": "2026-08-08",
      "location": "MCK Tourings klubbstuga, Borlänge",
      "type": "Körning",
      "organizer": "SMC Dalarna",
      "description": "Rebusorientering på MC genom Dalarna. Cirka 15 mil på fina vägar med 10 kontroller. Lös gåtan för att hitta nästa. Start 10-11, 100 kr inkl hamburgare och kaffe.",
      "descriptionFull": "Rebusrundan genom Dalarna!\n\nStart och mål vid MCK Tourings klubbstuga i Borlänge.\n\nCirka 15 mil på fina MC-vägar genom Dalarna med 10 kontroller längs vägen. Vid varje kontroll löser du en gåta som leder dig till nästa.\n\nStart mellan kl 10:00 och 11:00.\n\nDeltagaravgift 100 kr per MC. I priset ingår hamburgare, läsk och kaffe vid målgång.\n\nPrislottning bland alla som genomfört rundan.\n\nArrangör: SMC Dalarna",
      "links": [
        {
          "label": "svmc.se/dalarna",
          "url": "https://www.svmc.se/dalarna/aktiviteter/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=MCK+Tourings+klubbstuga+Borlänge",
          "type": "map"
        }
      ],
      "link": "https://www.svmc.se/dalarna/aktiviteter/",
      "region": "Dalarna",
      "source": "svmc.se"
    },
    {
      "id": "bmw-d8-glasstur-2026",
      "name": "D8 Glasstur",
      "date": "2026-08-08",
      "dateEnd": "2026-08-08",
      "location": "Västra, Götaland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Glasstur med BMW MC-klubben D8 i Västra Götaland. Samling och avfärd enligt plan. Kolla bmwklubben.se för detaljer.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-glasstur/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Glasstur arrangeras av BMW MC-klubben.\n\nGlasstur med BMW MC-klubben D8 i Västra Götaland. Samling och avfärd enligt plan. Kolla bmwklubben.se för detaljer.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Västra, Götaland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-glasstur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4stra%2C+G%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-mcq-sandras-asfaltstur-2026",
      "name": "D11 mcQ Sandras asfaltstur",
      "date": "2026-08-08",
      "dateEnd": "2026-08-08",
      "location": "Älgerumsvägen 35, Mönsterås, Sweden",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D11 från Älgerumsvägen 35, 383 32 Mönsterås, Sweden. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-mcq-sandras-tur/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 mcQ Sandras asfaltstur arrangeras av BMW MC-klubben.\n\nKörning med D11 från Älgerumsvägen 35, 383 32 Mönsterås, Sweden. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Älgerumsvägen 35, Mönsterås, Sweden.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-mcq-sandras-tur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%84lgerumsv%C3%A4gen+35%2C+M%C3%B6nster%C3%A5s%2C+Sweden",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-dagtouring-aug-2026",
      "name": "ÖAMCK Dagtouring",
      "date": "2026-08-08",
      "dateEnd": "2026-08-08",
      "location": "Lelles MC, Uppsala",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/dagtouring-4/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Dagtouring arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lelles MC, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/dagtouring-4/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lelles+MC%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "bsa-summer-camp-oland-2026",
      "name": "BSA Summer Camp Öland",
      "date": "2026-08-08",
      "dateEnd": "2026-08-15",
      "location": "Haga Park Camping, Mörbylånga, Öland",
      "type": "Träff",
      "organizer": "Svenska BSA Klubben",
      "description": "Internationell BSA-träff på Öland. En vecka med utflykter, line-up, Lucas Night Run och BBQ. Registrering senast 15 april.",
      "link": "https://www.bsaoc.org/swe/SummerCamp26/",
      "region": "Kalmar",
      "source": "bsaoc.org",
      "descriptionFull": "BSA Summer Camp Öland arrangeras av Svenska BSA Klubben.\n\nInternationell BSA-träff på Öland. En vecka med utflykter, line-up, Lucas Night Run och BBQ. Registrering senast 15 april.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Haga Park Camping, Mörbylånga, Öland.",
      "links": [
        {
          "label": "bsaoc.org",
          "url": "https://www.bsaoc.org/swe/SummerCamp26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Haga+Park+Camping%2C+M%C3%B6rbyl%C3%A5nga%2C+%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "mxsm-ulricehamn-2026",
      "name": "MXSM Deltävling 5 - Ulricehamn",
      "date": "2026-08-08",
      "dateEnd": "2026-08-09",
      "location": "Ulricehamns MK, Ulricehamn",
      "type": "Racing",
      "organizer": "SVEMO / Ulricehamns MK",
      "description": "Deltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Västra Götaland",
      "source": "svemo.se",
      "descriptionFull": "MXSM Deltävling 5 - Ulricehamn arrangeras av SVEMO / Ulricehamns MK.\n\nDeltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Ulricehamns MK, Ulricehamn.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Ulricehamns+MK%2C+Ulricehamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "viking-tour-2026",
      "name": "Viking Tour 2026",
      "date": "2026-08-09",
      "dateEnd": "2026-08-13",
      "location": "Sverige - Danmark",
      "type": "Körning",
      "organizer": "GWEF Nordic",
      "description": "Gemensam körning från Sverige - Danmark. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://gwef.eu/index.php/2026",
      "region": [
        "Danmark",
        "Sverige"
      ],
      "source": "gwef.eu",
      "descriptionFull": "Viking Tour 2026 arrangeras av GWEF Nordic.\n\nGemensam körning från Sverige - Danmark. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Sverige - Danmark.",
      "links": [
        {
          "label": "gwef.eu",
          "url": "https://gwef.eu/index.php/2026"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige+-+Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-jans-runda-2026",
      "name": "D15 Jans Runda",
      "date": "2026-08-09",
      "dateEnd": "2026-08-09",
      "location": "Skåne",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Skåne. Klassisk körtur i Skåne-regionen. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-jans-runda/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Jans Runda arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Skåne. Klassisk körtur i Skåne-regionen. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-jans-runda/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-glassturen-2026",
      "name": "D14-Glassturen",
      "date": "2026-08-09",
      "dateEnd": "2026-08-09",
      "location": "Jönköping",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Glasstur med BMW MC-klubben D14 i Västernorrland. Samling och avfärd enligt plan. Kolla bmwklubben.se för detaljer.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-glassturen/",
      "region": "Jönköping",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-Glassturen arrangeras av BMW MC-klubben.\n\nGlasstur med BMW MC-klubben D14 i Västernorrland. Samling och avfärd enligt plan. Kolla bmwklubben.se för detaljer.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Jönköping.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-glassturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=J%C3%B6nk%C3%B6ping",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-norra-smaland-runt-2026",
      "name": "D11 Norra Småland Runt",
      "date": "2026-08-09",
      "dateEnd": "2026-08-09",
      "location": "Östra vägen 3, Nässjö",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D11 från Östra vägen 3, 571 38 Nässjö, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-norra-smaland-runt/",
      "region": "Jönköping",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Norra Småland Runt arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D11 från Östra vägen 3, 571 38 Nässjö, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Östra vägen 3, Nässjö.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-norra-smaland-runt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96stra+v%C3%A4gen+3%2C+N%C3%A4ssj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-08-10",
      "dateEnd": "2026-08-10",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-10/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "mototour-of-nations-2026",
      "name": "Mototour of Nations 2026 (FIM)",
      "date": "2026-08-11",
      "dateEnd": "2026-08-14",
      "location": "Imatra, Finland",
      "type": "Körning",
      "organizer": "FIM / GWEF",
      "description": "Gemensam körning från Imatra, Finland. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://gwef.eu/index.php/2026",
      "region": "Finland",
      "source": "gwef.eu",
      "descriptionFull": "Mototour of Nations 2026 (FIM) arrangeras av FIM / GWEF.\n\nGemensam körning från Imatra, Finland. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Imatra, Finland.",
      "links": [
        {
          "label": "gwef.eu",
          "url": "https://gwef.eu/index.php/2026"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Imatra%2C+Finland",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-08-11",
      "dateEnd": "2026-08-11",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-11/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-08-11",
      "dateEnd": "2026-08-11",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-08-11/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-08-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-08-12",
      "dateEnd": "2026-08-12",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-12/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo udda veckor",
      "date": "2026-08-12",
      "dateEnd": "2026-08-12",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-08-12/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-08-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-08-12",
      "dateEnd": "2026-08-12",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-12/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "treffen-denmark-gwef-2026",
      "name": "Treffen Denmark (GWEF)",
      "date": "2026-08-13",
      "dateEnd": "2026-08-16",
      "location": "Lille Skensved, Danmark",
      "type": "Träff",
      "organizer": "Gold Wing Club Denmark / GWEF",
      "description": "MC-arrangemang i Lille Skensved, Danmark. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://gwef.eu/index.php/2026",
      "region": "Danmark",
      "source": "gwef.eu",
      "descriptionFull": "Treffen Denmark (GWEF) arrangeras av Gold Wing Club Denmark / GWEF.\n\nMC-arrangemang i Lille Skensved, Danmark. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Lille Skensved, Danmark.",
      "links": [
        {
          "label": "gwef.eu",
          "url": "https://gwef.eu/index.php/2026"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lille+Skensved%2C+Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "aminnetraffen-2026",
      "name": "Åminneträffen",
      "date": "2026-08-13",
      "dateEnd": "2026-08-16",
      "location": "Åminne Fritid, Slite",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Våran årliga träff i Åminne vid campingen, det bjuds på körningar, musik mm. Träffen startar på torsdagen klockan 12 och slutar söndagen klockan 12.",
      "link": "https://hdcs.se/evenemang/aminnetraffen/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "Åminneträffen arrangeras av H-DCS.\n\nVåran årliga träff i Åminne vid campingen, det bjuds på körningar, musik mm. Träffen startar på torsdagen klockan 12 och slutar söndagen klockan 12.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Åminne Fritid, Slite.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/aminnetraffen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%85minne+Fritid%2C+Slite",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-08-13",
      "dateEnd": "2026-08-13",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-13/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-08-13",
      "dateEnd": "2026-08-13",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-13/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-08-13",
      "dateEnd": "2026-08-13",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-13/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-08-13",
      "dateEnd": "2026-08-13",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-13/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "sikhallsturen-med-overnattning-2026",
      "name": "Sikhallsturen med övernattning",
      "date": "2026-08-14",
      "dateEnd": "2026-08-16",
      "location": "Sikhall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Sikhall. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/sikhallsturen-med-overnattning/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Sikhallsturen med övernattning arrangeras av H-DCS.\n\nKörtur med H-DCS från Sikhall. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Sikhall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/sikhallsturen-med-overnattning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sikhall",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-tjorn-och-orust-2026",
      "name": "D8 Tjörn och Orust",
      "date": "2026-08-14",
      "dateEnd": "2026-08-14",
      "location": "Tjörn",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D8 från Tjörn. Körning genom västra götaland. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-tjorn-och-orust/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Tjörn och Orust arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D8 från Tjörn. Körning genom västra götaland. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Tjörn.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-tjorn-och-orust/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tj%C3%B6rn",
          "type": "map"
        }
      ]
    },
    {
      "id": "torsstraffenen-karlstad-2026",
      "name": "Torsträffen",
      "date": "2026-08-14",
      "dateEnd": "2026-08-16",
      "location": "Dye gård, Karlstad",
      "type": "Träff",
      "organizer": "Tors MC",
      "description": "Träffpunkt i Dye gård, Karlstad. Gemensam fika och MC-umgänge. Återkommande träff alla välkomna!",
      "link": "https://smcboken.svmc.se/traffar/mcträffar",
      "region": "Värmland",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Torsträffen arrangeras av Tors MC.\n\nTräffpunkt i Dye gård, Karlstad. Gemensam fika och MC-umgänge. Återkommande träff alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Dye gård, Karlstad.",
      "links": [
        {
          "label": "smcboken.svmc.se",
          "url": "https://smcboken.svmc.se/traffar/mcträffar"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Dye+g%C3%A5rd%2C+Karlstad",
          "type": "map"
        }
      ]
    },
    {
      "id": "mc-collection-malaren-runt-2026",
      "name": "MC Collection Mälaren Runt-dag",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "Tidö Slott, Västerås",
      "type": "Träff",
      "organizer": "MC Collection",
      "description": "Mälaren Runt-dag på MC Collection. Besök museet i samband med Sveriges största MC-körning.",
      "link": "https://mc-collection.com/kalender/",
      "region": "Västmanland",
      "source": "mc-collection.com",
      "descriptionFull": "MC Collection Mälaren Runt-dag arrangeras av MC Collection.\n\nMälaren Runt-dag på MC Collection. Besök museet i samband med Sveriges största MC-körning.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Tidö Slott, Västerås.",
      "links": [
        {
          "label": "mc-collection.com",
          "url": "https://mc-collection.com/kalender/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Tid%C3%B6+Slott%2C+V%C3%A4ster%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-15/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-15/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-blekingerundan-2026-2026",
      "name": "D11 Blekingerundan 2026",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "Småland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Följ med kors och tvärs på fina vägar såväl asfalt som grus genom Östra Blekinges.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-blekingerundan-2026/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Blekingerundan 2026 arrangeras av BMW MC-klubben.\n\nFölj med kors och tvärs på fina vägar såväl asfalt som grus genom Östra Blekinges.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-blekingerundan-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-vanern-runt-fran-kristinehamn-en-svensk-mc-klassik-2026",
      "name": "Vänern Runt från Kristinehamn - En svensk mc-klassiker",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "Västerlånggatan 4, Kristinehamn",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Västerlånggatan 4, 681 30 Kristinehamn, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/vanern-runt-fran-kristinehamn-en-svensk-mc-klassiker/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "Vänern Runt från Kristinehamn - En svensk mc-klassiker arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Västerlånggatan 4, 681 30 Kristinehamn, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Västerlånggatan 4, Kristinehamn.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/vanern-runt-fran-kristinehamn-en-svensk-mc-klassiker/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4sterl%C3%A5nggatan+4%2C+Kristinehamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-vanern-runt-fran-vanersborg-2026",
      "name": "D8 Vänern runt från Vänersborg",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "Vänersborg",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D8 från Vänersborg. Körning runt Vänern i västra Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-vanern-runt-soder/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Vänern runt från Vänersborg arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D8 från Vänersborg. Körning runt Vänern i västra Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vänersborg.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-vanern-runt-soder/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4nersborg",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-mcq-pa-gronbete-i-goinge-2026",
      "name": "McQ på grönbete i Göinge",
      "date": "2026-08-15",
      "dateEnd": "2026-08-16",
      "location": "Sellerigatan 7, Lund",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Sellerigatan 7, 224 78 Lund, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/mcqhelg-i-goinge/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "McQ på grönbete i Göinge arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Sellerigatan 7, 224 78 Lund, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sellerigatan 7, Lund.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/mcqhelg-i-goinge/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sellerigatan+7%2C+Lund",
          "type": "map"
        }
      ]
    },
    {
      "id": "lulea-midnattssolsransen-2026",
      "name": "Midnattssolsransan",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "Luleå",
      "type": "Körning",
      "organizer": "Norrbottens Vägfarare MC",
      "description": "Gemensam tur genom Norrbotten med Norrbottens Vagfarare MC. Samling i Luleå.",
      "link": "",
      "region": "Norrbotten",
      "source": "web",
      "_canary": true
    },
    {
      "id": "guldrundan-skelleftea-2026",
      "name": "Guldrundan Skellefteå",
      "date": "2026-08-15",
      "dateEnd": "2026-08-15",
      "location": "Bonnstan, Skellefteå",
      "type": "Körning",
      "organizer": "Norrlands Motorhistoriker",
      "description": "Veteranfordonskörning i Skellefteåtrakten. Start och mål i Bonnstan. Bilar, motorcyklar och mopeder 30 år och äldre.",
      "descriptionFull": "Guldrundan i Skellefteå den 15 augusti 2026.\n\nTraditionellt veteranfordonsrally arrangerat av Norrlands Motorhistoriker. Fordon ska vara 30 år eller äldre. Bilar, motorcyklar och mopeder välkomnas.\n\nStart och mål i Bonnstan, Skellefteå. Sträckan är 8-10 mil i vacker natur i Skellefteåtrakten. Första start kl 10:00.\n\nStartavgift: 250 kr (bil), 150 kr (MC).\n\nPlats: Bonnstan, Skellefteå.",
      "links": [
        {
          "label": "nmh.se",
          "url": "https://nmh.se/skellefteasektionen/evenemang-skelleftea/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bonnstan+Skellefte%C3%A5",
          "type": "map"
        }
      ],
      "link": "https://nmh.se/skellefteasektionen/evenemang-skelleftea/",
      "region": "Västerbotten",
      "source": "nmh.se"
    },
    {
      "id": "malaren-runt-2026",
      "name": "Mälaren Runt",
      "date": "2026-08-16",
      "dateEnd": "2026-08-16",
      "location": "Mälardalen",
      "type": "Körning",
      "organizer": "SMC / Mälaren Runt",
      "description": "Arrangemang med SMC i Mälardalen. Mötespunkt för MC-klubben. Mer information på smc.nu.",
      "link": "https://www.svmc.se/club/maelaren-runt/",
      "region": "Stockholm",
      "source": "svmc.se",
      "backImage": "ads/malaren-runt-back-2026-08-16.jpg",
      "descriptionFull": "Mälaren Runt arrangeras av SMC / Mälaren Runt.\n\nArrangemang med SMC i Mälardalen. Mötespunkt för MC-klubben. Mer information på smc.nu.\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Mälardalen.",
      "links": [
        {
          "label": "svmc.se",
          "url": "https://www.svmc.se/club/maelaren-runt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=M%C3%A4lardalen",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-pelles-runda-2026",
      "name": "DOB- Pelles runda",
      "date": "2026-08-16",
      "dateEnd": "2026-08-16",
      "location": "Skåne",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOB från Skåne. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-pelles-runda/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Pelles runda arrangeras av H-DCS.\n\nArrangemang med H-DCS DOB från Skåne. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-pelles-runda/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-dalsland-2026",
      "name": "D8 Dalsland",
      "date": "2026-08-16",
      "dateEnd": "2026-08-16",
      "location": "Dalsland",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D8 från Dalsland. Körning genom dalslands vackra vägar. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-dalsland/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Dalsland arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D8 från Dalsland. Körning genom dalslands vackra vägar. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Dalsland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-dalsland/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Dalsland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-stafsater-glass-gardsbutik-2026",
      "name": "Stafsäter Glass & Gårdsbutik",
      "date": "2026-08-16",
      "dateEnd": "2026-08-16",
      "location": "Stafsäter, Sturefors",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Besök Stafsäter Glass & Gårdsbutik utanför Sturefors. Samåkning från Östenssons Livs i Ullstämma.",
      "link": "https://www.bmwklubben.se/aktiviteter/stafsater-glass-gardsbutik/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "Stafsäter Glass & Gårdsbutik arrangeras av BMW MC-klubben.\n\nBesök Stafsäter Glass & Gårdsbutik utanför Sturefors. Samåkning från Östenssons Livs i Ullstämma.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stafsäter, Sturefors.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/stafsater-glass-gardsbutik/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stafs%C3%A4ter%2C+Sturefors",
          "type": "map"
        }
      ]
    },
    {
      "id": "804-racing-lunda-2026",
      "name": "804 Ultimate Grand Finale",
      "date": "2026-08-16",
      "dateEnd": "2026-08-16",
      "location": "Lunda Flygfält, Alunda",
      "type": "Racing",
      "organizer": "804",
      "description": "Sveriges och Nordens snabbaste gatbilar och motorcyklar gör upp på 804 meter. Halfmile racing på Lunda Flygfält med BBQ, fika och passageraråkning. Publik 300 kr.",
      "descriptionFull": "Lunda Airfield - #NordicHalfmileRacing\n\nSveriges och Nordens absolut snabbaste gatbilar och motorcyklar gör upp på 804 meter. Här gäller det att få så hög sluthastighet som möjligt.\n\nEtt härligt familjenöje där det ges möjlighet att få åka med som passagerare med din favoritbil eller förare, om du törs?\n\nResultat och sluthastighet kan du följa live på stora skärmar på plats eller via 804meter.com under \"Official Ranking\".\n\nBiljetter:\nFörare: Förboka senast 14 augusti: 1 500 kr (exkl. försäkring) på 804meter.com.\n14-16 augusti: 2 500 kr.\nGoodiebag till 20 första förare som förbokar!\n\nPublik: 300 kr vid entré (barn under 14 år gratis med vuxen).\nPassageraråkband: +250 kr.\n\nBBQ och Fika: Grillade rätter och klassisk svensk fika serveras av Lunda Alliansen.\n\nSchema:\n09:00-18:00: Mingel, Race & Kul!\nIncheckning: 09:00-10:00\nFörarmöte: 10:00-10:30\nRacing: 10:30-18:00\n\nRacebanan: 2000x25 meter med separat returbana. 100% action!\n\nFörsäkring: 250 kr/dag (obligatoriskt). Hjälm och heltäckande kläder krävs. Lånehjälmar finns. Ambulans och crew på plats.",
      "backImage": "ads/804-racing-back-2026-08-16.jpg",
      "links": [
        {
          "label": "804meter.com",
          "url": "https://www.804meter.com"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lunda+Flygfält+Alunda",
          "type": "map"
        }
      ],
      "link": "https://www.804meter.com",
      "region": "Uppsala",
      "source": "FB"
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-08-17",
      "dateEnd": "2026-08-17",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-17/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-08-18",
      "dateEnd": "2026-08-18",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-18/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, Jämna veckor",
      "date": "2026-08-18",
      "dateEnd": "2026-08-18",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-08-18/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, Jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-08-18/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-08-19",
      "dateEnd": "2026-08-19",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-19/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-08-19",
      "dateEnd": "2026-08-19",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-19/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "grillkvall-longsjon-augusti-2026",
      "name": "Grillkväll Långsjön augusti",
      "date": "2026-08-19",
      "dateEnd": "2026-08-19",
      "location": "Rastplats väg 40, Långsjön, Ankarsrum",
      "type": "Träff",
      "organizer": "Törnros MC",
      "description": "MC-arrangemang i Rastplats väg 40, Långsjön, Ankarsrum. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://smcboken.svmc.se/traffar/mcträffar",
      "region": "Kalmar",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Grillkväll Långsjön augusti arrangeras av Törnros MC.\n\nMC-arrangemang i Rastplats väg 40, Långsjön, Ankarsrum. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rastplats väg 40, Långsjön, Ankarsrum.",
      "links": [
        {
          "label": "smcboken.svmc.se",
          "url": "https://smcboken.svmc.se/traffar/mcträffar"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Rastplats+v%C3%A4g+40%2C+L%C3%A5ngsj%C3%B6n%2C+Ankarsrum",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-08-20",
      "dateEnd": "2026-08-20",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-20/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-08-20",
      "dateEnd": "2026-08-20",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-20/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-08-20",
      "dateEnd": "2026-08-20",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-20/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-08-20",
      "dateEnd": "2026-08-20",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-08-20/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-08-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-08-20",
      "dateEnd": "2026-08-20",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-20/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-pizzakvall-i-vollsjo-2026",
      "name": "D15 Pizzakväll i Vollsjö",
      "date": "2026-08-20",
      "dateEnd": "2026-08-20",
      "location": "Vollsjö",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Vollsjö. Treffpunkt med pizza och trevligt sällskap. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-pizzakvall-i-vollsjo/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Pizzakväll i Vollsjö arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Vollsjö. Treffpunkt med pizza och trevligt sällskap. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vollsjö.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-pizzakvall-i-vollsjo/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Vollsj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d10-en-god-bit-av-ostfold-26-2026",
      "name": "D10 En God Bit Av Östfold -26",
      "date": "2026-08-21",
      "dateEnd": "2026-08-23",
      "location": "Halden, Østfold fylke, Norge",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D10 från Halden, Østfold fylke, Norge. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d10-en-god-bit-av-ostfold-26/",
      "region": "Norge",
      "source": "bmwklubben.se",
      "descriptionFull": "D10 En God Bit Av Östfold -26 arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D10 från Halden, Østfold fylke, Norge. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Halden, Østfold fylke, Norge.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d10-en-god-bit-av-ostfold-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halden%2C+%C3%98stfold+fylke%2C+Norge",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-ekero-runt-2026",
      "name": "DOA - Ekerö Runt",
      "date": "2026-08-22",
      "dateEnd": "2026-08-22",
      "location": "Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Ekerö. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-ekero-runt/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Ekerö Runt arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Ekerö. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-ekero-runt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-hasses-olandsrunda-22-23-augusti-2026",
      "name": "DOE Hasses Ölandsrunda 22-23 augusti",
      "date": "2026-08-22",
      "dateEnd": "2026-08-23",
      "location": "Öland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Öland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-hasses-olandsrunda-22-23-augusti/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Hasses Ölandsrunda 22-23 augusti arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Öland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Öland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-hasses-olandsrunda-22-23-augusti/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-08-22",
      "dateEnd": "2026-08-22",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-22/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-08-22",
      "dateEnd": "2026-08-22",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-22/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-08-22",
      "dateEnd": "2026-08-22",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-08-22/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-08-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "rockets-chopper-show-kungsbacka-2026",
      "name": "Rockets Chopper Show 2026",
      "date": "2026-08-22",
      "dateEnd": "2026-08-22",
      "location": "Badhusparken, Kungsbacka",
      "type": "Show",
      "organizer": "Rockets Choppers",
      "description": "Andra upplagan av Ride In Chopper Show i Kungsbacka. Choppers, custom builds och hojar i Badhusparken. Start kl 13:00. Alla valkomna!",
      "descriptionFull": "22 augusti 2026, Badhusparken, Kungsbacka\n\nAndra upplagan av Ride In Chopper Show arrangerad av Rockets Choppers. Choppers, custom builds och hojar visas upp i Badhusparken.\n\nStart kl 13:00. Alla med hoj och hojintresse är välkomna!\n\nPlats: Badhusparken, Kungsbacka.",
      "backImage": "ads/rockets-chopper-show-back-2026-08-22.jpg",
      "frontImage": "ads/rockets-chopper-show-front-2026-08-22.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/716595938205472/"
        },
        {
          "label": "visitkungsbacka.se",
          "url": "https://visitkungsbacka.se/evenemang"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Badhusparken+Kungsbacka",
          "type": "map"
        }
      ],
      "link": "https://visitkungsbacka.se/evenemang",
      "region": "Halland",
      "source": "FB"
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-08-24",
      "dateEnd": "2026-08-24",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-24/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "smt-varmland-2026",
      "name": "Värmland MC-tur",
      "date": "2026-08-24",
      "dateEnd": "2026-08-28",
      "location": "Scandic Örebro Väst, Örebro",
      "type": "Körning",
      "organizer": "Swedish MC Touring",
      "description": "Guidad 5-dagarstur genom Värmland, Dalsland och Norge. Fina MC-vägar, dagsetapper 20-26 mil. Hotell och middag ingår. 9 400 kr.",
      "descriptionFull": "En 5-dagars guidad MC-tur genom Värmland med start i Örebro.\n\nTuren går genom Värmland, Dalsland och över till Norge på utvalda MC-vägar. Dagsetapperna är mellan 20 och 26 mil.\n\nI priset 9 400 kr ingår 4 nätter på hotell, 4 middagar och en erfaren guide som leder gruppen.\n\nSamling vid Scandic Örebro Väst, Västhagsgatan 1, Örebro kl 12.15. Turen avslutas vid Trafikplats Biskopskvarn, E20 utanför Strängnäs.\n\nMax 8 deltagare per grupp för bästa upplevelse.",
      "links": [
        {
          "label": "swedishmctouring.se",
          "url": "https://www.swedishmctouring.se/varmland-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Scandic+Örebro+Väst+Västhagsgatan+1+Örebro",
          "type": "map"
        }
      ],
      "link": "https://www.swedishmctouring.se/varmland-2026/",
      "region": "Örebro",
      "source": "swedishmctouring.se"
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-08-25",
      "dateEnd": "2026-08-25",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-25/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-08-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-08-25",
      "dateEnd": "2026-08-25",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-08-25/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-08-25/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-08-26",
      "dateEnd": "2026-08-26",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-26/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-08-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo udda veckor",
      "date": "2026-08-26",
      "dateEnd": "2026-08-26",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-08-26/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-08-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-08-26",
      "dateEnd": "2026-08-26",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-26/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-08-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-08-27",
      "dateEnd": "2026-08-27",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-27/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-08-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-08-27",
      "dateEnd": "2026-08-27",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-27/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-08-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-08-27",
      "dateEnd": "2026-08-27",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-27/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-08-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-08-27",
      "dateEnd": "2026-08-27",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-27/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-08-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "oro-traffen-i-dk-2026",
      "name": "Orö träffen i DK",
      "date": "2026-08-28",
      "dateEnd": "2026-08-30",
      "location": "Orö, Danmark",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Orö, Danmark. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dsb-oro-traffen-i-dk/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "Orö träffen i DK arrangeras av H-DCS.\n\nArrangemang med H-DCS från Orö, Danmark. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Orö, Danmark.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dsb-oro-traffen-i-dk/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Or%C3%B6%2C+Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-upplandsturen-2026",
      "name": "DOA - Upplandsturen",
      "date": "2026-08-29",
      "dateEnd": "2026-08-29",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-upplandsturen/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Upplandsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-upplandsturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-08-29",
      "dateEnd": "2026-08-29",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-29/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-08-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-08-29",
      "dateEnd": "2026-08-29",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-29/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-08-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-limfjorden-runt-med-bmwklubben-danmark-2026",
      "name": "D8 Limfjorden runt med BMWKLUBBEN Danmark",
      "date": "2026-08-29",
      "dateEnd": "2026-08-29",
      "location": "Limfjorden, Danmark",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D8 från Limfjorden, Danmark. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-limfjorden-runt-med-bmwklubben-danmark/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Limfjorden runt med BMWKLUBBEN Danmark arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D8 från Limfjorden, Danmark. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Limfjorden, Danmark.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-limfjorden-runt-med-bmwklubben-danmark/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Limfjorden%2C+Danmark",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-kraftkalas-2026",
      "name": "ÖAMCK Kraftkalas",
      "date": "2026-08-29",
      "dateEnd": "2026-08-29",
      "location": "Stangby, Uppsala",
      "type": "Träff",
      "organizer": "ÖAMCK",
      "description": "Arrangemang med ÖAMCK i Stangby, Uppsala. Gemensamt event för MC-entusiaster. Mer info på oamck.se.",
      "link": "https://oamck.se/events/kraftkalas-2/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Kraftkalas arrangeras av ÖAMCK.\n\nArrangemang med ÖAMCK i Stangby, Uppsala. Gemensamt event för MC-entusiaster. Mer info på oamck.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stangby, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/kraftkalas-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stangby%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "mxsm-gota-ms-2026",
      "name": "MXSM Deltävling 6 - Göteborg",
      "date": "2026-08-29",
      "dateEnd": "2026-08-30",
      "location": "Göta MS, Göteborg",
      "type": "Racing",
      "organizer": "SVEMO / Göta MS",
      "description": "Deltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Västra Götaland",
      "source": "svemo.se",
      "descriptionFull": "MXSM Deltävling 6 - Göteborg arrangeras av SVEMO / Göta MS.\n\nDeltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Göta MS, Göteborg.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=G%C3%B6ta+MS%2C+G%C3%B6teborg",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-emilsturen-2026",
      "name": "DOA - Emilsturen",
      "date": "2026-08-30",
      "dateEnd": "2026-08-30",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-emilsturen/2026-08-30/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Emilsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-emilsturen/2026-08-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-grustur-2026",
      "name": "D15 Grustur",
      "date": "2026-08-30",
      "dateEnd": "2026-08-30",
      "location": "Skåne",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Grustour med D15. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-grustur-2/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Grustur arrangeras av BMW MC-klubben.\n\nGrustour med D15. Körning på grusvägar för äventyrslystna. Samling enligt schema på bmwklubben.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-grustur-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-08-31",
      "dateEnd": "2026-08-31",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-31/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-08-31/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-09-01",
      "dateEnd": "2026-09-01",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-01/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, Jämna veckor",
      "date": "2026-09-01",
      "dateEnd": "2026-09-01",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-09-01/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, Jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-09-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-09-02",
      "dateEnd": "2026-09-02",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-02/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-09-02",
      "dateEnd": "2026-09-02",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-02/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-02/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-09-03",
      "dateEnd": "2026-09-03",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-03/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-09-03",
      "dateEnd": "2026-09-03",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-03/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-09-03",
      "dateEnd": "2026-09-03",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-03/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-09-03",
      "dateEnd": "2026-09-03",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-09-03/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-09-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-09-03",
      "dateEnd": "2026-09-03",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-03/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-unnarydstraffen-2026",
      "name": "DO-C Unnarydsträffen",
      "date": "2026-09-04",
      "dateEnd": "2026-09-06",
      "location": "Vallsnäs Camping, Vallsnäs, Unnaryd",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Vallsnäs Camping, Vallsnäs, 314 52 Unnaryd, Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-unnarydstraffen-2/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Unnarydsträffen arrangeras av H-DCS.\n\nArrangemang med H-DCS från Vallsnäs Camping, Vallsnäs, 314 52 Unnaryd, Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Vallsnäs Camping, Vallsnäs, Unnaryd.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-unnarydstraffen-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Vallsn%C3%A4s+Camping%2C+Vallsn%C3%A4s%2C+Unnaryd",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-stenotraffen-2026",
      "name": "DOG Stenöträffen",
      "date": "2026-09-04",
      "dateEnd": "2026-09-06",
      "location": "Stenö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Stenöträffen + Medlemsmöte 4-6 September Mer info: Laila 072 - 725 12 72 Olle 072.",
      "link": "https://hdcs.se/evenemang/dog-stenotraffen/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG Stenöträffen arrangeras av H-DCS.\n\nStenöträffen + Medlemsmöte 4-6 September Mer info: Laila 072 - 725 12 72 Olle 072.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stenö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-stenotraffen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sten%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-mcq-rikstraff-2026-2026",
      "name": "mcQ Riksträff 2026",
      "date": "2026-09-04",
      "dateEnd": "2026-09-06",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Träffpunkt i Sverige. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/mcq-rikstraff-2026/",
      "region": "Norrbotten",
      "source": "bmwklubben.se",
      "descriptionFull": "mcQ Riksträff 2026 arrangeras av BMW MC-klubben.\n\nTräffpunkt i Sverige. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/mcq-rikstraff-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "motorcycel-inferno-2026",
      "name": "Motorcycel Inferno",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/motorcycel-inferno/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Motorcycel Inferno arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/motorcycel-inferno/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-alandsturen-2026",
      "name": "DOA - Ålandsturen",
      "date": "2026-09-05",
      "dateEnd": "2026-09-06",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-alandsturen/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Ålandsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-alandsturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-05/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-05/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-09-05/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-09-05/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "gokart-2026",
      "name": "Gokart",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Broväg 110, Visby",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Gokartkörning på gokartbanad vid gamla tippen i Visby Broväg 110 Värd Svenne.",
      "link": "https://hdcs.se/evenemang/gokart/",
      "region": "Gotland",
      "source": "hdcs.se",
      "descriptionFull": "Gokart arrangeras av H-DCS.\n\nGokartkörning på gokartbanad vid gamla tippen i Visby Broväg 110 Värd Svenne.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Broväg 110, Visby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/gokart/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Brov%C3%A4g+110%2C+Visby",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d8-mats-tur-2026",
      "name": "D8 Mats tur",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Karl, Gustav",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D8 från Karl Gustav, Hallands län, Sverige. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d8-mats-tur/",
      "region": "Halland",
      "source": "bmwklubben.se",
      "descriptionFull": "D8 Mats tur arrangeras av BMW MC-klubben.\n\nKörning med D8 från Karl Gustav, Hallands län, Sverige. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Karl, Gustav.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d8-mats-tur/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Karl%2C+Gustav",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-tur-fran-bromolla-2026",
      "name": "D15 Tur från Bromölla",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Bromölla",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D15 från Bromölla. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-tur-fran-bromolla/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Tur från Bromölla arrangeras av BMW MC-klubben.\n\nKörning med D15 från Bromölla. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Bromölla.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-tur-fran-bromolla/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Brom%C3%B6lla",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-morka-smaland-2026",
      "name": "D11 - Mörka Småland",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Norremarksvägen, Växjö",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D11 från Norremarksvägen, 352 45 Växjö, Sverige. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-morka-smaland/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 - Mörka Småland arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D11 från Norremarksvägen, 352 45 Växjö, Sverige. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Norremarksvägen, Växjö.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-morka-smaland/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Norremarksv%C3%A4gen%2C+V%C3%A4xj%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-dagtouring-sep-2026",
      "name": "ÖAMCK Dagtouring",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "Lelles MC, Uppsala",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/dagtouring-5/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Dagtouring arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lelles MC, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/dagtouring-5/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lelles+MC%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "jsm-mx-orion-2026",
      "name": "JSM MX Finalomgång - MK Orion",
      "date": "2026-09-05",
      "dateEnd": "2026-09-05",
      "location": "MK, Orion",
      "type": "Racing",
      "organizer": "SVEMO / MK Orion",
      "description": "Deltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Västmanland",
      "source": "svemo.se",
      "descriptionFull": "JSM MX Finalomgång - MK Orion arrangeras av SVEMO / MK Orion.\n\nDeltävling i Svenska Mästerskapen i Motocross. Klasser: MX1, MX2, MX-Women. Mer info på svemo.se.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: MK, Orion.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=MK%2C+Orion",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-09-07",
      "dateEnd": "2026-09-07",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-07/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-07/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "faaker-see-2026-osterrike-2026",
      "name": "Faaker See 2026 Österrike",
      "date": "2026-09-08",
      "dateEnd": "2026-09-13",
      "location": "Österrike",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Österrike. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/faaker-see-2026-osterrike/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "Faaker See 2026 Österrike arrangeras av H-DCS.\n\nArrangemang med H-DCS från Österrike. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Österrike.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/faaker-see-2026-osterrike/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96sterrike",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-09-08",
      "dateEnd": "2026-09-08",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-08/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-09-08",
      "dateEnd": "2026-09-08",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-09-08/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-09-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-09-09",
      "dateEnd": "2026-09-09",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-09/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo udda veckor",
      "date": "2026-09-09",
      "dateEnd": "2026-09-09",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-09-09/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-09-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-09-09",
      "dateEnd": "2026-09-09",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-09/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-09/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-09-10",
      "dateEnd": "2026-09-10",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-10/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-09-10",
      "dateEnd": "2026-09-10",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-10/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-09-10",
      "dateEnd": "2026-09-10",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-10/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-09-10",
      "dateEnd": "2026-09-10",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-10/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "roadracing-sm-anderstorp-2026",
      "name": "Roadracing SM - Anderstorp",
      "date": "2026-09-11",
      "dateEnd": "2026-09-13",
      "location": "Anderstorp",
      "type": "Racing",
      "organizer": "MotoSE / Svemo",
      "description": "MC-arrangemang i Anderstorp, Jönköpings län. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://mcparken.se/calendar/177",
      "region": "Jönköping",
      "source": "mcparken.se",
      "descriptionFull": "Roadracing SM - Anderstorp arrangeras av MotoSE / Svemo.\n\nMC-arrangemang i Anderstorp, Jönköpings län. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: Anderstorp.",
      "links": [
        {
          "label": "mcparken.se",
          "url": "https://mcparken.se/calendar/177"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Anderstorp",
          "type": "map"
        }
      ]
    },
    {
      "id": "tandstickstraffen-i-bankeryd-2026",
      "name": "Tändsticksträffen i Bankeryd",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Bankeryd",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Bankeryd. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/tandstickstraffen-i-bankeryd/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Tändsticksträffen i Bankeryd arrangeras av H-DCS.\n\nArrangemang med H-DCS från Bankeryd. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Bankeryd.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/tandstickstraffen-i-bankeryd/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bankeryd",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-sormlandsturen-2026",
      "name": "DOA - Sörmlandsturen",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-sormlandsturen/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Sörmlandsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-sormlandsturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "do-c-route-154-2026",
      "name": "DO-C Route 154",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Halland/VGötaland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/do-c-route-154-2/",
      "region": "Halland",
      "source": "hdcs.se",
      "descriptionFull": "DO-C Route 154 arrangeras av H-DCS.\n\nArrangemang med H-DCS från Halland/VGötaland. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Halland/VGötaland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/do-c-route-154-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Halland%2FVG%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-tjejtraff-moheda-12-september-2026",
      "name": "DOE Tjejträff Moheda 12 september",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Moheda",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOE i Moheda. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doe-tjejtraff-moheda-12-september/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Tjejträff Moheda 12 september arrangeras av H-DCS.\n\nTräffpunkt för DOE i Moheda. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Moheda.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-tjejtraff-moheda-12-september/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Moheda",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-12/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-12/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d14-bolmenturen-2026",
      "name": "D14-Bolmenturen",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Bolmen/Ljungby",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Körning med D14 från Bolmen/Ljungby. Samling enligt schema. Följ bmwklubben.se för information.",
      "link": "https://www.bmwklubben.se/aktiviteter/d14-bolmenturen/",
      "region": "Gotland",
      "source": "bmwklubben.se",
      "descriptionFull": "D14-Bolmenturen arrangeras av BMW MC-klubben.\n\nKörning med D14 från Bolmen/Ljungby. Samling enligt schema. Följ bmwklubben.se för information.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Bolmen/Ljungby.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d14-bolmenturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Bolmen%2FLjungby",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-oland-experience-2026-2026",
      "name": "D11 Öland Experience 2026",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Öland",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Välkommen till Öland Experience med D11!Välkommen att göra en resa till ett höstfagert Öland, där vi upprepar fjolårets tur, givetvis med lite andra vägval.",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-oland-experience/",
      "region": "Kalmar",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Öland Experience 2026 arrangeras av BMW MC-klubben.\n\nVälkommen till Öland Experience med D11!Välkommen att göra en resa till ett höstfagert Öland, där vi upprepar fjolårets tur, givetvis med lite andra vägval.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Öland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-oland-experience/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96land",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-scaniamus-et-och-tunga-rallyt-2026",
      "name": "Scaniamuséet och ”Tunga Rallyt”",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Krokek Buthagen, Kolmården",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "MC-arrangemang i Krokek Buthagen, 618 92 Kolmården, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://www.bmwklubben.se/aktiviteter/scaniamuseet-och-tunga-rallyt/",
      "region": "Östergötland",
      "source": "bmwklubben.se",
      "descriptionFull": "Scaniamuséet och ”Tunga Rallyt” arrangeras av BMW MC-klubben.\n\nMC-arrangemang i Krokek Buthagen, 618 92 Kolmården, Sverige. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Krokek Buthagen, Kolmården.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/scaniamuseet-och-tunga-rallyt/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Krokek+Buthagen%2C+Kolm%C3%A5rden",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-hemlig-resa-sep-2026",
      "name": "Hemlig resa",
      "date": "2026-09-12",
      "dateEnd": "2026-09-13",
      "location": "Hemligt",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Hemligt. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/hemlig-resa-3/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "Hemlig resa arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Hemligt. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Hemligt.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/hemlig-resa-3/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Hemligt",
          "type": "map"
        }
      ]
    },
    {
      "id": "con-dios-mc-20-årsjubileum-2026",
      "name": "Con Dios MC 20-årsjubileum",
      "date": "2026-09-12",
      "dateEnd": "2026-09-12",
      "location": "Meddelas senare",
      "type": "Träff",
      "organizer": "Con Dios MC",
      "description": "Con Dios MC firar 20 ar! Boka 12 september redan nu. Mer information kommer nar det narmar sig. Alla valkomna!",
      "descriptionFull": "Con Dios MC firar sitt 20-årsjubileum den 12 september 2026.\n\nBoka datumet redan nu! Mer information om plats och program fylls på när det närmar sig.\n\nCon Dios MC är en klubb för bikers med en kristen grundsyn. Alla som vill är välkomna att hänga med.\n\nMer info: condiosmc.com",
      "backImage": "ads/condiosmc-20ar-back-2026-09-12.jpg",
      "links": [
        {
          "label": "FB Event",
          "url": "https://www.facebook.com/events/1431320095362234/"
        },
        {
          "label": "condiosmc.com",
          "url": "http://www.condiosmc.com"
        }
      ],
      "link": "http://www.condiosmc.com",
      "region": "Kronoberg",
      "source": "FB"
    },
    {
      "id": "nykoping-hosttraffen-2026",
      "name": "Hosttraffen Nykoping",
      "date": "2026-09-13",
      "dateEnd": "2026-09-13",
      "location": "Nyköping",
      "type": "Träff",
      "organizer": "Sodermanlands MC-Gille",
      "description": "Avslutar sasongen med hosttreff i Nykoping. Alla valkommen!",
      "link": "",
      "region": "Södermanland",
      "source": "web",
      "_canary": true
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-09-14",
      "dateEnd": "2026-09-14",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-14/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-09-15",
      "dateEnd": "2026-09-15",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-15/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, Jämna veckor",
      "date": "2026-09-15",
      "dateEnd": "2026-09-15",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-09-15/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, Jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-09-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-09-16",
      "dateEnd": "2026-09-16",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-16/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-09-16",
      "dateEnd": "2026-09-16",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-16/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-16/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-09-17",
      "dateEnd": "2026-09-17",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-17/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-09-17",
      "dateEnd": "2026-09-17",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-17/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-09-17",
      "dateEnd": "2026-09-17",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-17/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-09-17",
      "dateEnd": "2026-09-17",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-09-17/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-09-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-09-17",
      "dateEnd": "2026-09-17",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-17/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-fiskelager-i-kulagolen-18-19-september-2026",
      "name": "DOE Fiskeläger i Kulagölen 18-19 september",
      "date": "2026-09-18",
      "dateEnd": "2026-09-19",
      "location": "Småland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-fiskelager-i-kulagolen-18-19-september/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Fiskeläger i Kulagölen 18-19 september arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-fiskelager-i-kulagolen-18-19-september/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "hosttraffenen-kristinehamn-2026",
      "name": "Höstträffen Kristinehamn",
      "date": "2026-09-18",
      "dateEnd": "2026-09-20",
      "location": "Gustavsvik, Kristinehamn",
      "type": "Träff",
      "organizer": "Christinehamns MC klubb",
      "description": "Träffpunkt i Gustavsvik, Kristinehamn. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://cmck.se/",
      "region": "Värmland",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Höstträffen Kristinehamn arrangeras av Christinehamns MC klubb.\n\nTräffpunkt i Gustavsvik, Kristinehamn. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Gustavsvik, Kristinehamn.",
      "links": [
        {
          "label": "cmck.se",
          "url": "https://cmck.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gustavsvik%2C+Kristinehamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "kinnekulleturen-2026",
      "name": "Kinnekulleturen",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "Kinnekulle",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Kinnekulle. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/kinnekulleturen/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Kinnekulleturen arrangeras av H-DCS.\n\nKörtur med H-DCS från Kinnekulle. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Kinnekulle.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/kinnekulleturen/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kinnekulle",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-c-paulas-runda-2026",
      "name": "LO-C Paulas runda",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "Kasernvägen 2, Borås",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "LO Paula guidar oss på en heldags tur med lunch stopp. Kom fulltankade och klara.",
      "link": "https://hdcs.se/evenemang/lo-c-paulas-runda/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "LO-C Paulas runda arrangeras av H-DCS.\n\nLO Paula guidar oss på en heldags tur med lunch stopp. Kom fulltankade och klara.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Kasernvägen 2, Borås.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-c-paulas-runda/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Kasernv%C3%A4gen+2%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-19/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-19/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-09-19/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-09-19/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "mxsm-uddevalla-2026",
      "name": "MXSM Final - Uddevalla",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "BMK Uddevalla, Uddevalla",
      "type": "Racing",
      "organizer": "SVEMO / BMK Uddevalla",
      "description": "Svenska Mästerskapen i Motocross, sista deltävlingen (MX1, MX2, MX-Women, USM 125 U17).",
      "link": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross",
      "region": "Västra Götaland",
      "source": "svemo.se",
      "descriptionFull": "MXSM Final - Uddevalla arrangeras av SVEMO / BMK Uddevalla.\n\nSvenska Mästerskapen i Motocross, sista deltävlingen (MX1, MX2, MX-Women, USM 125 U17).\n\nTävlingsevenemang. Se arrangören för schema och biljetter.\n\nPlats: BMK Uddevalla, Uddevalla.",
      "links": [
        {
          "label": "svemo.se",
          "url": "https://www.svemo.se/vara-sportgrenar/start-motocross/tavlingar-motocross"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=BMK+Uddevalla%2C+Uddevalla",
          "type": "map"
        }
      ]
    },
    {
      "id": "probike-pub-anchor-harley-show-2026",
      "name": "Pub Anchor Harley Show",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "Pub Anchor, Stockholm",
      "type": "Show",
      "organizer": "Probike Stockholm",
      "description": "Harley Show på Pub Anchor i Stockholm. Arrangerat av Probike.",
      "descriptionFull": "Pub Anchor Harley Show 2026\n\nHarley-Davidson show på legendariska Pub Anchor i Stockholm.\n\nArrangerat av Probike Stockholm.",
      "links": [
        {
          "label": "Probike",
          "url": "https://www.probikebutiken.se/event/pub-anchor-harley-show/"
        }
      ],
      "link": "https://www.probikebutiken.se/event/pub-anchor-harley-show/",
      "region": "Stockholm",
      "source": "probikebutiken.se"
    },
    {
      "id": "probike-rock-bottom-summer-sale-2026",
      "name": "Rock Bottom Summer Sale",
      "date": "2026-09-19",
      "dateEnd": "2026-09-19",
      "location": "Probike Stockholm, Göteborg & Malmö",
      "type": "Träff",
      "organizer": "Probike",
      "description": "Oslagbara erbjudanden hos alla Probike butiker. Kampanj på alla hojar, rabatt på delar och tillbehör.",
      "descriptionFull": "Rock Bottom Summer Sale hos Probike!\n\nAlla Probike butiker (Stockholm, Göteborg, Malmö).\n\nOslagbara erbjudanden:\n- Kampanj på alla hojar\n- Reducerat pris på vinterförvaring vid bokning på plats\n- Minst 20% rabatt på alla delar och tillbehör i lager\n\nKl. 11:00.",
      "links": [
        {
          "label": "Probike",
          "url": "https://www.probikebutiken.se/event/rock-bottom-summer-sale/"
        }
      ],
      "link": "https://www.probikebutiken.se/event/rock-bottom-summer-sale/",
      "region": "Sverige",
      "source": "probikebutiken.se"
    },
    {
      "id": "doa-emilsturen-2026",
      "name": "DOA - Emilsturen",
      "date": "2026-09-20",
      "dateEnd": "2026-09-20",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-emilsturen/2026-09-20/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Emilsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-emilsturen/2026-09-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-09-21",
      "dateEnd": "2026-09-21",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-21/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-21/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-09-22",
      "dateEnd": "2026-09-22",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-22/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-ekero-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ekerö, udda veckor",
      "date": "2026-09-22",
      "dateEnd": "2026-09-22",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-09-22/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ekerö, udda veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning Varannan vecka guidar DO/LO Lennart oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-ekero-udda-veckor-2/2026-09-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-09-23",
      "dateEnd": "2026-09-23",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-23/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-osmo-udda-veckor-2026",
      "name": "DOA - Kvällskörning Ösmo udda veckor",
      "date": "2026-09-23",
      "dateEnd": "2026-09-23",
      "location": "Ösmo Plantshop, Växtstigen 1, Ösmo",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-09-23/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Ösmo udda veckor arrangeras av H-DCS.\n\nVälkomna på onsdagskörning varannan vecka LO Petra guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Ösmo Plantshop, Växtstigen 1, Ösmo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-osmo-udda-veckor-2/2026-09-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=%C3%96smo+Plantshop%2C+V%C3%A4xtstigen+1%2C+%C3%96smo",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-09-23",
      "dateEnd": "2026-09-23",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-23/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-23/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-dagstur-ekero-2026",
      "name": "DOA - Dagstur Ekerö",
      "date": "2026-09-24",
      "dateEnd": "2026-09-24",
      "location": "OKQ8, Åkerstigen 22, Ekerö",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-24/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Dagstur Ekerö arrangeras av H-DCS.\n\nVälkommen till Lennarts dagstur! Samling kl. 09:45 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OKQ8, Åkerstigen 22, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-dagstur-ekero-2/2026-09-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OKQ8%2C+%C3%85kerstigen+22%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-uppsala-2026",
      "name": "DOA - Kvällskörning Uppsala",
      "date": "2026-09-24",
      "dateEnd": "2026-09-24",
      "location": "Circle K, Rapsgatan 1H, Uppsala",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-24/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Uppsala arrangeras av H-DCS.\n\nVälkomna på torsdagskörning LO Joakim guidar oss på en tur. Samling kl. 17:00 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Circle K, Rapsgatan 1H, Uppsala.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-uppsala-4/2026-09-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Circle+K%2C+Rapsgatan+1H%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallstur-skogskyrkogarden-2026",
      "name": "DOA - Kvällstur Skogskyrkogården",
      "date": "2026-09-24",
      "dateEnd": "2026-09-24",
      "location": "Skogskyrkogården, vid Lillys Blommor, Sköndal",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-24/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällstur Skogskyrkogården arrangeras av H-DCS.\n\nVälkomna på torsdagskörning Varannan vecka guidar LO Göran och LO Petra oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Skogskyrkogården, vid Lillys Blommor, Sköndal.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallstur-skogskyrkogarden/2026-09-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Skogskyrkog%C3%A5rden%2C+vid+Lillys+Blommor%2C+Sk%C3%B6ndal",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-torsdagskörningar-sundsvall-2026",
      "name": "LO-I Torsdagskörningar Sundsvall",
      "date": "2026-09-24",
      "dateEnd": "2026-09-24",
      "location": "HD-Sundsvall, Sundsvall",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.",
      "link": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-24/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO-I Torsdagskörningar Sundsvall arrangeras av H-DCS.\n\nVi samlas 17:45 på ny startplats HD-Sundsvall. Turens längd och resmål bestäms på plats eller läggs ut på Facebook samma vecka.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: HD-Sundsvall, Sundsvall.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-torsdagskörningar-sundsvall-2-2/2026-09-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=HD-Sundsvall%2C+Sundsvall",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-kramfors-2026",
      "name": "LO I Lördagskörningar Kramfors",
      "date": "2026-09-26",
      "dateEnd": "2026-09-26",
      "location": "OK Kramfors, Kramfors",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi kör en tur som bestäms på plats eller publiceras innan via Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-26/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Kramfors arrangeras av H-DCS.\n\nVi kör en tur som bestäms på plats eller publiceras innan via Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: OK Kramfors, Kramfors.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-kramfors/2026-09-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=OK+Kramfors%2C+Kramfors",
          "type": "map"
        }
      ]
    },
    {
      "id": "lo-i-lordagskörningar-ornskoldsvik-2026",
      "name": "LO I Lördagskörningar Örnsköldsvik",
      "date": "2026-09-26",
      "dateEnd": "2026-09-26",
      "location": "Vägkrogen Örnen, Örnsköldsvik",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Vi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.",
      "link": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-26/",
      "region": "Västernorrland",
      "source": "hdcs.se",
      "descriptionFull": "LO I Lördagskörningar Örnsköldsvik arrangeras av H-DCS.\n\nVi samlas 10:45 och åker 11:00 från Vägkrogen Örnen.Turens längd och resmål bestäms på plats eller annonseras innan på Facebook.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Vägkrogen Örnen, Örnsköldsvik.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/lo-i-lordagskörningar-ornskoldsvik/2026-09-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4gkrogen+%C3%96rnen%2C+%C3%96rnsk%C3%B6ldsvik",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-sista-rundan-2026",
      "name": "DOB- Sista Rundan",
      "date": "2026-09-27",
      "dateEnd": "2026-09-27",
      "location": "Skåne",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOB från Skåne. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-sista-rundan/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Sista Rundan arrangeras av H-DCS.\n\nArrangemang med H-DCS DOB från Skåne. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-sista-rundan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nacka-2026",
      "name": "DOA - Kvällskörning Nacka",
      "date": "2026-09-28",
      "dateEnd": "2026-09-28",
      "location": "McDonalds Nacka, Nacka",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-28/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nacka arrangeras av H-DCS.\n\nVälkomna på måndagskörning Varannan vecka guidar LO Mahlén och LO Göran oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: McDonalds Nacka, Nacka.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nacka-4/2026-09-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=McDonalds+Nacka%2C+Nacka",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-09-29",
      "dateEnd": "2026-09-29",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-29/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-09-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-nockeby-jamna-veckor-2026",
      "name": "DOA - Kvällskörning Nockeby, Jämna veckor",
      "date": "2026-09-29",
      "dateEnd": "2026-09-29",
      "location": "Nockeby, Bromma",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-09-29/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Nockeby, Jämna veckor arrangeras av H-DCS.\n\nVälkomna på tisdagskörning jämna veckor, OBS samlingsplatsen LO Michael guidar oss på en tur. Samling kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Nockeby, Bromma.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-nockeby-jamna-veckor-2/2026-09-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nockeby%2C+Bromma",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-taby-2026",
      "name": "DOA - Kvällskörning Täby",
      "date": "2026-09-30",
      "dateEnd": "2026-09-30",
      "location": "Probike Täby Åkerivägen 6, Täby",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-30/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Täby arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Martin guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Probike Täby Åkerivägen 6, Täby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-taby-2/2026-09-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Probike+T%C3%A4by+%C3%85keriv%C3%A4gen+6%2C+T%C3%A4by",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-kvallskörning-norrtalje-2026",
      "name": "DOA - Kvällskörning Norrtälje",
      "date": "2026-09-30",
      "dateEnd": "2026-09-30",
      "location": "Biltema Norrtälje, Norrtälje",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Välkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.",
      "link": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-30/",
      "region": "Uppsala",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Kvällskörning Norrtälje arrangeras av H-DCS.\n\nVälkomna på onsdagskörning LO Tommy guidar oss på en tur. Samling kl. 17:30 Planerad avfärd kl.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Biltema Norrtälje, Norrtälje.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-kvallskörning-norrtalje/2026-09-30/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Biltema+Norrt%C3%A4lje%2C+Norrt%C3%A4lje",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-10-01",
      "dateEnd": "2026-10-01",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-10-01/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-10-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-sista-rundan-3-oktober-2026",
      "name": "DOE Sista rundan 3 oktober",
      "date": "2026-10-03",
      "dateEnd": "2026-10-03",
      "location": "Småland",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-sista-rundan-3-oktober/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Sista rundan 3 oktober arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-sista-rundan-3-oktober/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-asian-food-varnamo-3-oktober-2026",
      "name": "DOE Asian Food Värnamo 3 oktober",
      "date": "2026-10-03",
      "dateEnd": "2026-10-03",
      "location": "Värnamo",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Värnamo. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-asian-food-varnamo-3-oktober/",
      "region": "Jönköping",
      "source": "hdcs.se",
      "descriptionFull": "DOE Asian Food Värnamo 3 oktober arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Värnamo. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Värnamo.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-asian-food-varnamo-3-oktober/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4rnamo",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-10-03",
      "dateEnd": "2026-10-03",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-10-03/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-10-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "medlemsmote-planeringsmote-2027-2026",
      "name": "Medlemsmöte/Planeringsmöte 2027",
      "date": "2026-10-03",
      "dateEnd": "2026-10-04",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/medlemsmote-planeringsmote-2027/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Medlemsmöte/Planeringsmöte 2027 arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/medlemsmote-planeringsmote-2027/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-dagtouring-okt-2026",
      "name": "ÖAMCK Dagtouring",
      "date": "2026-10-03",
      "dateEnd": "2026-10-03",
      "location": "Lelles MC, Uppsala",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/dagtouring-6/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Dagtouring arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Lelles MC, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Lelles MC, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/dagtouring-6/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lelles+MC%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-10-06",
      "dateEnd": "2026-10-06",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-06/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-06/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "dob-grill-rovarkulan-2026",
      "name": "DOB- Grill Rövarkulan",
      "date": "2026-10-11",
      "dateEnd": "2026-10-11",
      "location": "Skåne",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOB från Skåne. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/dob-grill-rovarkulan/",
      "region": "Skåne",
      "source": "hdcs.se",
      "descriptionFull": "DOB- Grill Rövarkulan arrangeras av H-DCS.\n\nArrangemang med H-DCS DOB från Skåne. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Skåne.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dob-grill-rovarkulan/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sk%C3%A5ne",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-emilsturen-2026",
      "name": "DOA - Emilsturen",
      "date": "2026-10-11",
      "dateEnd": "2026-10-11",
      "location": "Stockholm",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-emilsturen/2026-10-11/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Emilsturen arrangeras av H-DCS.\n\nKörtur med H-DCS DOA från Stockholm. Gemensam MC-körning. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-emilsturen/2026-10-11/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-medlemsmote-okt-2026",
      "name": "Sormlandswingarnas Medlemsmote (host)",
      "date": "2026-10-11",
      "dateEnd": "2026-10-11",
      "location": "Sigridslunds Cafe, Ardala",
      "type": "Träff",
      "organizer": "Sormlandswingarna / GWCS",
      "description": "MC-arrangemang i Sigridslunds Cafe, Ardala. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://gwcs.se/events/sormlandswingarnas-medlemsmote-3/?occurrence=2026-10-11",
      "region": "Södermanland",
      "source": "gwcs.se",
      "descriptionFull": "Sormlandswingarnas Medlemsmote (host) arrangeras av Sormlandswingarna / GWCS.\n\nMC-arrangemang i Sigridslunds Cafe, Ardala. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sigridslunds Cafe, Ardala.",
      "links": [
        {
          "label": "gwcs.se",
          "url": "https://gwcs.se/events/sormlandswingarnas-medlemsmote-3/?occurrence=2026-10-11"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sigridslunds+Cafe%2C+Ardala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-10-13",
      "dateEnd": "2026-10-13",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-13/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-13/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-10-15",
      "dateEnd": "2026-10-15",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-10-15/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-10-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-10-17",
      "dateEnd": "2026-10-17",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-10-17/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-10-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-10-20",
      "dateEnd": "2026-10-20",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-20/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-20/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "medlemsmote-2026",
      "name": "Medlemsmöte",
      "date": "2026-10-24",
      "dateEnd": "2026-10-24",
      "location": "Langs  Väg 2, Visby, Visby",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Medlemsmöte med efterföljande bowling och lite mat Väl Mött Styrelsen, Kenneth.",
      "link": "https://hdcs.se/evenemang/medlemsmote/",
      "region": "Gotland",
      "source": "hdcs.se",
      "descriptionFull": "Medlemsmöte arrangeras av H-DCS.\n\nMedlemsmöte med efterföljande bowling och lite mat Väl Mött Styrelsen, Kenneth.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Langs  Väg 2, Visby, Visby.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/medlemsmote/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Langs++V%C3%A4g+2%2C+Visby%2C+Visby",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-alandkryss-2026",
      "name": "Alandkryss med Eckerolinjen (Viltbord)",
      "date": "2026-10-24",
      "dateEnd": "2026-10-24",
      "location": "Grisslehamn",
      "type": "Körning",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Grisslehamn. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/alandkryss-med-eckerolinjen-viltbord/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "Alandkryss med Eckerolinjen (Viltbord) arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Grisslehamn. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Grisslehamn.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/alandkryss-med-eckerolinjen-viltbord/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Grisslehamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-10-27",
      "dateEnd": "2026-10-27",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-27/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-10-27/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-10-29",
      "dateEnd": "2026-10-29",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-10-29/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-10-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-10-31",
      "dateEnd": "2026-10-31",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-10-31/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-10-31/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "pannkaksfsesten-boras-2026",
      "name": "Pannkaksfesten",
      "date": "2026-10-31",
      "dateEnd": "2026-11-01",
      "location": "Eagle Riders MC, Borås",
      "type": "Träff",
      "organizer": "Eagle Riders MC Borås",
      "description": "MC-arrangemang i Eagle Riders MC, Borås. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "http://eagleriders.se/",
      "region": "Västra Götaland",
      "source": "smcboken.svmc.se",
      "descriptionFull": "Pannkaksfesten arrangeras av Eagle Riders MC Borås.\n\nMC-arrangemang i Eagle Riders MC, Borås. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Eagle Riders MC, Borås.",
      "links": [
        {
          "label": "eagleriders.se",
          "url": "http://eagleriders.se/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Eagle+Riders+MC%2C+Bor%C3%A5s",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-11-03",
      "dateEnd": "2026-11-03",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-03/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-03/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-aktivitet-i-oskarshamn-7-november-2026",
      "name": "DOE Aktivitet i Oskarshamn 7 november",
      "date": "2026-11-07",
      "dateEnd": "2026-11-07",
      "location": "Oskarshamn",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Oskarshamn. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-aktivitet-i-oskarshamn-7-november/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Aktivitet i Oskarshamn 7 november arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Oskarshamn. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Oskarshamn.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-aktivitet-i-oskarshamn-7-november/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Oskarshamn",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-11-10",
      "dateEnd": "2026-11-10",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-10/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-11-12",
      "dateEnd": "2026-11-12",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-11-12/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-11-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-11-14",
      "dateEnd": "2026-11-14",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-11-14/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-11-14/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "pubkvall-2026",
      "name": "Pubkväll",
      "date": "2026-11-14",
      "dateEnd": "2026-11-14",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Pubkväll med H-DCS i Sverige. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!",
      "link": "https://hdcs.se/evenemang/pubkvall/",
      "region": "Sverige",
      "source": "hdcs.se",
      "descriptionFull": "Pubkväll arrangeras av H-DCS.\n\nPubkväll med H-DCS i Sverige. Träffas och umgås med andra Harley-entusiaster. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/pubkvall/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d11-planeringstraff-2026-2026",
      "name": "D11 Planeringsträff 2026",
      "date": "2026-11-14",
      "dateEnd": "2026-11-14",
      "location": "Småland",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Träffpunkt med D11 i Småland. Gemensam fika och MC-prat. Alla medlemmar välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/d11-planeringstraff-2026/",
      "region": "Kronoberg",
      "source": "bmwklubben.se",
      "descriptionFull": "D11 Planeringsträff 2026 arrangeras av BMW MC-klubben.\n\nTräffpunkt med D11 i Småland. Gemensam fika och MC-prat. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d11-planeringstraff-2026/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-fikatraff-sormland-nov-2026",
      "name": "Sormlandswingarnas fikatraff (host)",
      "date": "2026-11-15",
      "dateEnd": "2026-11-15",
      "location": "Sigridslunds Cafe, Ardala",
      "type": "Träff",
      "organizer": "Sormlandswingarna / GWCS",
      "description": "Träffpunkt i Sigridslunds Cafe, Ardala. Gemensam fika och MC-umgänge. Alla välkomna!",
      "link": "https://gwcs.se/events/sormlandswingarnas-fikatraff-sigridslunds-cafe-3/?occurrence=2026-11-15",
      "region": "Södermanland",
      "source": "gwcs.se",
      "descriptionFull": "Sormlandswingarnas fikatraff (host) arrangeras av Sormlandswingarna / GWCS.\n\nTräffpunkt i Sigridslunds Cafe, Ardala. Gemensam fika och MC-umgänge. Alla välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sigridslunds Cafe, Ardala.",
      "links": [
        {
          "label": "gwcs.se",
          "url": "https://gwcs.se/events/sormlandswingarnas-fikatraff-sigridslunds-cafe-3/?occurrence=2026-11-15"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sigridslunds+Cafe%2C+Ardala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-11-17",
      "dateEnd": "2026-11-17",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-17/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-17/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "oamck-flipperturnering-2026",
      "name": "ÖAMCK Flipperturnering",
      "date": "2026-11-21",
      "dateEnd": "2026-11-21",
      "location": "Stangby, Uppsala",
      "type": "Träff",
      "organizer": "ÖAMCK",
      "description": "Dagstur med ÖAMCK från Stangby, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.",
      "link": "https://oamck.se/events/flipperturnering-2/",
      "region": "Uppsala",
      "source": "oamck.se",
      "descriptionFull": "ÖAMCK Flipperturnering arrangeras av ÖAMCK.\n\nDagstur med ÖAMCK från Stangby, Uppsala. Trevlig grupptur på vägar genom Sverige. Samling enligt evenemangsinformation.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stangby, Uppsala.",
      "links": [
        {
          "label": "oamck.se",
          "url": "https://oamck.se/events/flipperturnering-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stangby%2C+Uppsala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-11-24",
      "dateEnd": "2026-11-24",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-24/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-11-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-11-26",
      "dateEnd": "2026-11-26",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-11-26/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-11-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-julbordskryssning-2026",
      "name": "DOA - Julbordskryssning",
      "date": "2026-11-28",
      "dateEnd": "2026-11-29",
      "location": "Stockholm",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOA från Stockholm. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doa-julbordskryssning/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Julbordskryssning arrangeras av H-DCS.\n\nArrangemang med H-DCS DOA från Stockholm. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Stockholm.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-julbordskryssning/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Stockholm",
          "type": "map"
        }
      ]
    },
    {
      "id": "doe-medlemsmote-julbord-28-29-november-2026",
      "name": "DOE Medlemsmöte/Julbord 28-29 november",
      "date": "2026-11-28",
      "dateEnd": "2026-11-29",
      "location": "Småland",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.",
      "link": "https://hdcs.se/evenemang/doe-medlemsmote-julbord-28-29-november/",
      "region": "Östergötland",
      "source": "hdcs.se",
      "descriptionFull": "DOE Medlemsmöte/Julbord 28-29 november arrangeras av H-DCS.\n\nArrangemang med H-DCS DOE från Småland. Mötespunkt för Harley-klubben. Mer info på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Småland.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doe-medlemsmote-julbord-28-29-november/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sm%C3%A5land",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-11-28",
      "dateEnd": "2026-11-28",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-11-28/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-11-28/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-12-01",
      "dateEnd": "2026-12-01",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-01/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-01/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "gwcs-jultallrik-2026",
      "name": "Stor Jultallrik Sigridslunds Cafe",
      "date": "2026-12-06",
      "dateEnd": "2026-12-06",
      "location": "Sigridslunds Cafe, Ardala",
      "type": "Träff",
      "organizer": "Sormlandswingarna / GWCS",
      "description": "MC-arrangemang i Sigridslunds Cafe, Ardala. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.",
      "link": "https://gwcs.se/events/stor-jultallrik-pa-sigridslunds-cafe/?occurrence=2026-12-06",
      "region": "Södermanland",
      "source": "gwcs.se",
      "descriptionFull": "Stor Jultallrik Sigridslunds Cafe arrangeras av Sormlandswingarna / GWCS.\n\nMC-arrangemang i Sigridslunds Cafe, Ardala. Gemensamt event för MC-entusiaster. Mer information på arrangörens hemsida.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sigridslunds Cafe, Ardala.",
      "links": [
        {
          "label": "gwcs.se",
          "url": "https://gwcs.se/events/stor-jultallrik-pa-sigridslunds-cafe/?occurrence=2026-12-06"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sigridslunds+Cafe%2C+Ardala",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-12-08",
      "dateEnd": "2026-12-08",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-08/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-08/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-12-10",
      "dateEnd": "2026-12-10",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-12-10/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-12-10/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-d15-luciamys-i-lundagard-2026",
      "name": "D15 Luciamys i Lundagård",
      "date": "2026-12-10",
      "dateEnd": "2026-12-12",
      "location": "Lundagård, Lund",
      "type": "Träff",
      "organizer": "BMW MC-klubben",
      "description": "Arrangemang med BMW MC-klubben D15 från Lundagård, Lund. Mer info på bmwklubben.se.",
      "link": "https://www.bmwklubben.se/aktiviteter/d15-luciamys-i-lundagard/",
      "region": "Skåne",
      "source": "bmwklubben.se",
      "descriptionFull": "D15 Luciamys i Lundagård arrangeras av BMW MC-klubben.\n\nArrangemang med BMW MC-klubben D15 från Lundagård, Lund. Mer info på bmwklubben.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Lundagård, Lund.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/d15-luciamys-i-lundagard/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Lundag%C3%A5rd%2C+Lund",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-12-12",
      "dateEnd": "2026-12-12",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-12-12/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-12-12/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "julbord-2026",
      "name": "Julbord",
      "date": "2026-12-12",
      "dateEnd": "2026-12-12",
      "location": "Sverige",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Arrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.",
      "link": "https://hdcs.se/evenemang/julbord/",
      "region": "Sverige",
      "source": "hdcs.se",
      "descriptionFull": "Julbord arrangeras av H-DCS.\n\nArrangemang med H-DCS från Sverige. Mötespunkt för Harley-klubben. Mer information på hdcs.se.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/julbord/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-12-15",
      "dateEnd": "2026-12-15",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-15/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-15/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-12-22",
      "dateEnd": "2026-12-22",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-22/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-22/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "doh-fika-lycksele-2026",
      "name": "DOH-Fika Lycksele",
      "date": "2026-12-24",
      "dateEnd": "2026-12-24",
      "location": "Nobody's MC Lappland, Lycksele",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-12-24/",
      "region": "Värmland",
      "source": "hdcs.se",
      "descriptionFull": "DOH-Fika Lycksele arrangeras av H-DCS.\n\nTräffpunkt för DOH i Nobody's MC Lappland, Lycksele. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Nobody's MC Lappland, Lycksele.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doh-fika-lycksele-2/2026-12-24/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Nobody%27s+MC+Lappland%2C+Lycksele",
          "type": "map"
        }
      ]
    },
    {
      "id": "dog-fikatraff-pa-radis-2026",
      "name": "DOG  FIKATRÄFF på Rådis",
      "date": "2026-12-26",
      "dateEnd": "2026-12-26",
      "location": "Rådis, Dalarna",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Träffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!",
      "link": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-12-26/",
      "region": "Dalarna",
      "source": "hdcs.se",
      "descriptionFull": "DOG  FIKATRÄFF på Rådis arrangeras av H-DCS.\n\nTräffpunkt för DOG i Rådis, Dalarna. Gemensam fika och MC-umgänge. Alla medlemmar välkomna!\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Rådis, Dalarna.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/dog-fikatraff-pa-radis/2026-12-26/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=R%C3%A5dis%2C+Dalarna",
          "type": "map"
        }
      ]
    },
    {
      "id": "doa-tisdagslunch-2026",
      "name": "DOA - Tisdagslunch",
      "date": "2026-12-29",
      "dateEnd": "2026-12-29",
      "location": "Väringgården, Träkvista torg, Ekerö",
      "type": "Träff",
      "organizer": "H-DCS",
      "description": "Vi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.",
      "link": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-29/",
      "region": "Stockholm",
      "source": "hdcs.se",
      "descriptionFull": "DOA - Tisdagslunch arrangeras av H-DCS.\n\nVi ses på Väringgården’s Kök och Bar och äter Raggmunk varje tisdag, året om.\n\nEn träffpunkt för MC-entusiaster. Alla är välkomna oavsett vilken hoj du kör.\n\nPlats: Väringgården, Träkvista torg, Ekerö.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/doa-tisdagslunch/2026-12-29/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4ringg%C3%A5rden%2C+Tr%C3%A4kvista+torg%2C+Eker%C3%B6",
          "type": "map"
        }
      ]
    },
    {
      "id": "sista-turen-for-aret-2026",
      "name": "Sista turen för året",
      "date": "2026-12-31",
      "dateEnd": "2026-12-31",
      "location": "Sverige",
      "type": "Körning",
      "organizer": "H-DCS",
      "description": "Körtur med H-DCS från Sverige. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.",
      "link": "https://hdcs.se/evenemang/sista-turen-for-aret/",
      "region": "Västra Götaland",
      "source": "hdcs.se",
      "descriptionFull": "Sista turen för året arrangeras av H-DCS.\n\nKörtur med H-DCS från Sverige. Gemensam MC-körning för alla medlemmar. Samling enligt evenemangsinformation på hdcs.se.\n\nGemensam MC-tur. Kom i tid för samling. Kontrollera evenemangssidan för exakt avfärdstid och rutt.\n\nPlats: Sverige.",
      "links": [
        {
          "label": "hdcs.se",
          "url": "https://hdcs.se/evenemang/sista-turen-for-aret/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Sverige",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-region-vast-subventionerat-trafiksakerhetsarbete-2-2026",
      "name": "Region Väst Subventionerat Trafiksäkerhetsarbete 2026 - 2",
      "date": "2026-12-31",
      "dateEnd": "2026-12-31",
      "location": "Västra, Götaland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Gemensam körning från Västra Götaland. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/region-vast-subventionerat-trafiksakerhetsarbete-2026-2/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "Region Väst Subventionerat Trafiksäkerhetsarbete 2026 - 2 arrangeras av BMW MC-klubben.\n\nGemensam körning från Västra Götaland. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Västra, Götaland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/region-vast-subventionerat-trafiksakerhetsarbete-2026-2/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4stra%2C+G%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bmw-region-vast-subventionerat-trafiksakerhetsarbete-2-2026",
      "name": "Region Väst Subventionerat Trafiksäkerhetsarbete 2026 - 1",
      "date": "2026-12-31",
      "dateEnd": "2026-12-31",
      "location": "Västra, Götaland",
      "type": "Körning",
      "organizer": "BMW MC-klubben",
      "description": "Gemensam körning från Västra Götaland. Samling enligt evenemangsinformation. Alla bikerare välkomna!",
      "link": "https://www.bmwklubben.se/aktiviteter/region-vast-subventionerat-trafiksakerhetsarbete-2026-1/",
      "region": "Värmland",
      "source": "bmwklubben.se",
      "descriptionFull": "Region Väst Subventionerat Trafiksäkerhetsarbete 2026 - 1 arrangeras av BMW MC-klubben.\n\nGemensam körning från Västra Götaland. Samling enligt evenemangsinformation. Alla bikerare välkomna!\n\nGemensam körning på utvalda vägar. Kontrollera evenemangssidan för detaljer.\n\nPlats: Västra, Götaland.",
      "links": [
        {
          "label": "bmwklubben.se",
          "url": "https://www.bmwklubben.se/aktiviteter/region-vast-subventionerat-trafiksakerhetsarbete-2026-1/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=V%C3%A4stra%2C+G%C3%B6taland",
          "type": "map"
        }
      ]
    },
    {
      "id": "bonneville-lansering-upplands-vasby-2026",
      "name": "Bonneville lansering!",
      "date": "2026-03-21",
      "dateEnd": "2026-03-21",
      "location": "Triumph Stockholm Norr, Upplands Väsby",
      "type": "Show",
      "organizer": "Triumph Stockholm Norr",
      "description": "Exklusivt lanseringsevent for nya Triumph Modern Classics. Provkör nya modellerna och träffa andra Triumph-entusiaster.",
      "backImage": "ads/bonneville-lansering-back-2026-03-21.jpg",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/1267483558130059/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Karins+v%C3%A4g+5+194+61+Upplands+V%C3%A4sby",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1267483558130059/",
      "region": "Stockholm",
      "source": "facebook.com",
      "descriptionFull": "Triumph Stockholm Norr bjuder in till ett exklusivt lanseringsevent for de nya modellerna inom Triumph Modern Classics.\n\nKom och kolla in nyheterna, provkor och snacka med andra MC-entusiaster. Triumph Stockholm Norr ar din Triumph-handlare i Stockholm.\n\nDatum: Lordag 21 mars\nTid: kl. 11:00\n\nPlats: Karins vag 5, 194 61 Upplands Vasby."
    },
    {
      "id": "mce-manadsmoete-april-jarfalla-2026",
      "name": "MCE Månadsmöte",
      "date": "2026-04-13",
      "dateEnd": "2026-04-13",
      "location": "MCE Klubblokalen, Järfälla",
      "type": "Träff",
      "organizer": "MCE - motorcykelentusiasterna",
      "description": "Månadsmöte hos MCE i Järfälla. Diskussion om verksamheten, nyval av medlemmar. Alla hojintresserade välkomna.",
      "backImage": "ads/mce-manadsmoete-back-2026-04-13.jpg",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/1919105001975075/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Snutenv%C3%A4gen+11+177+61+J%C3%A4rf%C3%A4lla",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1919105001975075/",
      "region": "Stockholm",
      "source": "facebook.com",
      "descriptionFull": "MCE - motorcykelentusiasterna bjuder in till månadsmöte i april.\n\nVi går igenom vad som hänt och vad som är på gång. Diskuterar verksamheten och väljer in nya medlemmar. Alla medlemmar och klubbnyfikna är välkomna.\n\nMCE i Järfälla är klubben för alla hojintresserade. Typ av hoj spelar ingen roll, inte ens krav på att man ska ha en hoj.\n\nTid: Måndag 13 april kl. 18:00 - 21:00\nPlats: Snutenvägen 11, 177 61 Järfälla."
    },
    {
      "id": "avrostning-mc-tur-oppet-hus-vallentuna-2026",
      "name": "Avrostning / MC-tur & Öppet Hus",
      "date": "2026-05-01",
      "dateEnd": "2026-05-01",
      "location": "Svensk Trafikutbildning, Vallentuna",
      "type": "Körning",
      "organizer": "Svensk Trafikutbildning",
      "description": "Avrostning och MC-tur for gamla MC-elever. Körning, häng och hoj-snack. Alla gamla elever välkomna till trafikskolan.",
      "backImage": "ads/avrostning-mc-tur-back-2026-05-01.jpg",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/1530418525067996/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Gillinge+55+186+91+Vallentuna",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1530418525067996/",
      "region": "Stockholm",
      "source": "facebook.com",
      "descriptionFull": "Svensk Trafikutbildning bjuder in alla gamla MC-elever till en dag med körning, häng och massa hoj-snack!\n\nDet är äntligen dags för avrostning och öppet hus. Kom och träffa andra MC-entusiaster, kör en tur och njut av stämningen.\n\nSvensk Trafikutbildning är en modern trafikskola som utbildar för MC och tungt släp, samt hyr ut motorcyklar.\n\nTid: Fredag 1 maj kl. 09:00 - 13:00\nPlats: Gillinge 55, 186 91 Vallentuna."
    },
    {
      "id": "provkorning-zontes-cafe-rosenhill-2026",
      "name": "Provkörning: Zontes",
      "date": "2026-06-03",
      "dateEnd": "2026-06-03",
      "location": "Café Rosenhill, Botkyrka",
      "type": "Show",
      "organizer": "Café Rosenhill",
      "description": "Provkörningskväll med Zontes motorcyklar. Pauls Motor visar upp modeller. Fika och MC-snack på Café Rosenhill.",
      "backImage": "ads/provkorning-zontes-back-2026-06-03.jpg",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/839357122492882/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=Rosenhill+23+Botkyrka+Gr%C3%B6dinge",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/839357122492882/",
      "region": "Stockholm",
      "source": "facebook.com",
      "descriptionFull": "Café Rosenhill bjuder in till en provkörningskväll!\n\nGrabbarna bakom Pauls Motor kommer och visar upp Zontes motorcyklar. Kom och ta en fika, kika på fina motorcyklar och provkör om du vill.\n\nZontes är ett spännande märke med prisvärda och moderna modeller.\n\nTid: Onsdag 3 juni kl. 16:00 - 19:00\nPlats: Rosenhill 23, Botkyrka (Grödinge)."
    },
    {
      "id": "nationaldagsturen-haninge-2026",
      "name": "Nationaldagsturen Haninge 2026",
      "date": "2026-06-06",
      "dateEnd": "2026-06-06",
      "location": "GULF Tungelsta, Haninge",
      "type": "Körning",
      "organizer": "Kari Lindh",
      "description": "Nationaldagskörning i Haninge. Alla sorters MC välkomna. Rutten är 55 km lång genom Haninges vackraste vägar.",
      "backImage": "ads/nationaldagsturen-haninge-back-2026-06-06.jpg",
      "links": [
        {
          "label": "Facebook",
          "url": "https://www.facebook.com/events/1132728105638779/"
        },
        {
          "label": "Karta",
          "url": "https://www.google.com/maps/search/?api=1&query=GULF+Tungelsta+S%C3%B6dert%C3%A4ljev%C3%A4gen+19+Tungelsta",
          "type": "map"
        }
      ],
      "link": "https://www.facebook.com/events/1132728105638779/",
      "region": "Stockholm",
      "source": "facebook.com",
      "descriptionFull": "Välkommen att köra nationaldagsturen Haninge 2026!\n\nAlla sorters MC får vara med. Sträckningen är knappt 5,5 mil lång och går genom större delen av Haninges vackraste vägar.\n\nStart och mål: GULF Tungelsta. Samling för gemensam avfärd.\n\nTid: Lördag 6 juni kl. 14:00\nPlats: GULF Tungelsta, Södertäljevägen 19, 137 56 Tungelsta."
    }
  ]
};
